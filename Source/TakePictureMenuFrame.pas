...
type
  THideMenuType=(hmtNone,hmtHasPicture,hmtHasFile);
  TTakedPictureEvent=procedure(Sender:TObject;ABitmap:TBitmap) of object;

  TPickFileEvent=procedure(Sender:TObject;AFileNameList:TStringList) of object;
  ...
  TFrameTakePictureMenu = class(TFrame,
    ...
    procedure ActionTakeFileFromDeviceDidFinishTaking;
  ...
  public
    FRawBitmap: TBitmap;
    OnTakedPicture:TTakedPictureEvent;  //拍照
    FOnCustomPickPhoto:TNotifyEvent;    //选择照片

    FFileNameList: TStringList;
    OnPickFile:TPickFileEvent;     //选择文件

    procedure HideMenu;
    procedure ShowMenu;

    //选择文件、文件夹回调事件
    procedure OnPathResult(AResult:Boolean;AFilePath:String;ARequestCode:Integer);
  public
...
// 从设备选择文件
procedure TFrameTakePictureMenu.TakeFileFromDevice;
var
  OpenDialog: TOpenDialog;
  FileName: string;
begin
  ...
  {$IFDEF MSWINDOWS}
    ...
    try
      ...
      OpenDialog.Title:='选择附件';
//      OpenDialog.Filter:='所有文件 (*.*)|*.*';
//      OpenDialog.Filter := TBitmapCodecManager.GetFilterString;
      OpenDialog.Options:=OpenDialog.Options+[TOpenOption.ofAllowMultiSelect]; // 多选
      ...
      begin
        ...
        ActionTakeFileFromDeviceDidFinishTaking;
  ...
  {$IFDEF ANDROID}
    GetFileOpenDialog(OnPathResult,0,GetSDCardPath(0));
  {$ENDIF}
...
// 选择文件返回
procedure TFrameTakePictureMenu.ActionTakeFileFromDeviceDidFinishTaking;
begin

  //隐藏弹出菜单
  HideMenuType:=hmtHasFile;
  HideMenu;
  ...
  begin
      ...
      case HideMenuType of
        ...
        begin
          ...
          if Assigned(OnPickFile) then
          begin
            OnPickFile(Self, FFileNameList);
          end;
...
const
  ...
  Langs_Button_Caption_Device : array[Low(TLangKind)..High(TLangKind)] of string =
      ('选择文件','Select File','');
...
begin
  ...
  Self.lbMenu.Properties.Items.FindItemByName('选择文件').Caption:=Langs_Button_Caption_Device[ALangKind];
...
//      HideMenu;
  ...
  if TSkinItem(Sender).Name='选择文件' then
  begin
    TakeFileFromDevice;
  end;
...
procedure TFrameTakePictureMenu.OnPathResult(AResult: Boolean;
  AFilePath: String; ARequestCode: Integer);
begin

  if AResult then
  begin

    if ARequestCode=0 then
    begin
      //数据文件路径
//      ShowMessage(AFilePath);

      FFileNameList.Clear;
      FFileNameList.Add(AFilePath);
      ActionTakeFileFromDeviceDidFinishTaking;

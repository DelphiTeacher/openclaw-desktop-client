...
//  uThirdPartyAccountAuth,
  ...
  uSkinFireMonkeyMemo, FMX.WebBrowser, uProcessNativeControlModalShowPanel;

type
  TFrameEditMail = class(TFrame)
    edtRecv: TSkinFMXEdit;
    pnlToolBar: TSkinFMXPanel;
    btnReturn: TSkinFMXButton;
    pnlRecv: TSkinFMXPanel;
    pnlSubject: TSkinFMXPanel;
...
//    FPetHeadPicFrame:TFrameAddPictureListSub;
  ...
  public
    //图片附件上传
    FAddPhotoFrame:TFrameAddPictureListSub;
  public
...
var
  GlobalEditMailFrame: TFrameEditMail;
...
procedure TFrameEditMail.btnSaveClick(Sender: TObject);
begin
  FScriptGate.Eval(
...
procedure TFrameEditMail.btnSelectAreaClick(Sender: TObject);
begin
  HideFrame;
  //选择区域
...
procedure TFrameEditMail.btnSendCaptchaClick(Sender: TObject);
begin
//  HideVirtualKeyboard;
//
...
procedure TFrameEditMail.Clear;//(AText:String);
begin
//  //用户头像
...
constructor TFrameEditMail.Create(AOwner: TComponent);
begin
  inherited;
...
//  Self.detCompleteDate.DateTime:=0;
  FScriptGate:=TScriptGate.Create(Self,Self.webBody,'delphi');



  FAddPhotoFrame:=TFrameAddPictureListSub.Create(AOwner);
  //要设置Name,不然会报错
  SetFrameName(FAddPhotoFrame);
  FAddPhotoFrame.Parent:=Self;//.sbcClient;
  FAddPhotoFrame.Align:=TAlignLayout.Bottom;
  FAddPhotoFrame.pnlToolBar.Visible:=False;
  FAddPhotoFrame.lvPictures.Align:=TAlignLayout.Client;
  FAddPhotoFrame.lvPictures.Margins.Left:=0;
  FAddPhotoFrame.Init(
                      '',
                      [],//
                      [],//
...
//  FAddPhotoFrame.OnChange:=DoAddPhotoFrameChange;
//  FAddPhotoFrame.OnCustomUploadVideoFile:=DoAddPhotoFrameCustomUploadVideoFile;
//  FAddPhotoFrame.OnSelectMediaResult:=DoAddPhotoFrameSelectMediaResult;

  //多选图片问题多,先不添加
//  FAddPhotoFrame.IsUseMultiSelectPicture:=True;
  FAddPhotoFrame.IsCanNotEditPicture:=True;//不能编辑图片,也就是要先删除再添加

//  FAddPhotoFrame.Visible:=False;



  //启用缓存图模式来显示地图
  ProcessNativeControlModalShowPanel1.IsEnableModalShow:=True;



end;

procedure TFrameEditMail.DoSaveBillExecute(ATimerTask: TObject);
var
...
procedure TFrameEditMail.DoSaveBillExecuteEnd(ATimerTask: TObject);
var
...
procedure TFrameEditMail.DoReturnFrameFromSelectArea(AFrame: TFrame);
begin
...
procedure TFrameEditMail.DoSendRegisterCaptchaExecute(ATimerTask: TObject);
begin
//  // 出错
//  TTimerTask(ATimerTask).TaskTag := 1;
//    try
...
procedure TFrameEditMail.DoSendRegisterCaptchaExecuteEnd(ATimerTask: TObject);
//var
//  ASuperObject: ISuperObject;
begin
...
//function TFrameEditMail.GetCurrentPorcessControl(AFocusedControl: TControl)
//  : TControl;
//begin
//  Result := AFocusedControl;
//end;
//
//function TFrameEditMail.GetVirtualKeyboardControlParent: TControl;
//begin
//  Result := Self;
//end;

procedure TFrameEditMail.Load(ADataJson:ISuperObject);//(AUserName, AUserHeadPicFilePath, AWxOpenID,
//  AWxAuthToken: String;
//  AAlipayOpenID:String;
//  AAlipayAuthToken:String);
//var
...
//  Self.pnlEmail.Visible:=(LangKind<>lkZH);
//
//  Self.webBody.LoadFromStrings('<HTML>'
//                                +'<BODY></BODY>'
//                                +'</HTML>','');
//
  Self.webBody.LoadFromStrings('<HTML>'
...
//procedure TFrameEditMail.Load(ABindCode: String; AIntroducerPhone: String);
//begin
//
//  FBindCode := ABindCode;
...
procedure TFrameEditMail.OnModalResultFromRegisterSucc(Frame: TObject);
begin
...
procedure TFrameEditMail.btnReturnClick(Sender: TObject);
begin
...
procedure TFrameEditMail.tmrSendCaptchaCheckTimer(Sender: TObject);
begin

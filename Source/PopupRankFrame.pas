...
//  uVirtualListDataController,
  ...
  ListItemStyleFrame_CustomerRank,
...
const
  Const_PopupRankWidth:Double=320;
...
//                                      var AIsOfThisCategory:Boolean) of object;
  ...
  TFramePopupRank = class(TFrame,IFrameHistroyVisibleEvent)
    lbMenus: TSkinFMXTreeView;
    BackRectangle: TRectangle;
    dspBottomButton: TSkinFMXItemDesignerPanel;
    btnReset: TSkinFMXButton;
    ...
    procedure lbMenusPrepareDrawItem(Sender: TObject; ACanvas: TDrawCanvas;
...
var
  GlobalPopupRankFrame:TFramePopupRank;
...
procedure TFramePopupRank.BackRectangleClick(Sender: TObject);
begin
  //按空白区域则返回上一页
...
procedure TFramePopupRank.Clear;
begin
  ModalResult:='';
  ModalResultName:='';
  ModalResultItem:=nil;
...
constructor TFramePopupRank.Create(AOwner: TComponent);
begin
  inherited;
...
//  //记住默认宽度
//  MenuWidth:=Self.pnlPopupRank.Width;
...
destructor TFramePopupRank.Destroy;
begin
//  FreeAndNil(ListDataController);
  inherited;
end;

//procedure TFramePopupRank.DoFilter;
//var
//  I: Integer;
//  AFilter:String;
//begin
...
procedure TFramePopupRank.DoHide;
begin
...
//procedure TFramePopupRank.DoListDataController_LoadDataToUIEnd(Sender: TObject);
//begin
...
procedure TFramePopupRank.DoShow;
begin
...
procedure TFramePopupRank.edtFilterChangeTracking(Sender: TObject);
begin
end;

//procedure TFramePopupRank.FilterSelectedCategory;
//var
//  I: Integer;
...
procedure TFramePopupRank.FrameClick(Sender: TObject);
begin
  //没有点击到列表项
  HidePopupRank;
end;

procedure TFramePopupRank.FrameResize(Sender: TObject);
//var
//  AMenuCount:Integer;
begin
...
//
//
//  Self.pnlPopupRank.Height:=Self.pnlTopDevide.Height
//                            +AMenuCount*Self.lbMenus.Prop.ItemHeight;
//  if Self.lblCaption.Visible then
//  begin
//    Self.pnlPopupRank.Height:=Self.pnlPopupRank.Height+Self.lblCaption.Height;
//  end;
//  if Self.edtFilter.Visible then
//  begin
//    Self.pnlPopupRank.Height:=Self.pnlPopupRank.Height+Self.edtFilter.Height;
//  end;
end;

procedure TFramePopupRank.HidePopupRank;
begin
...
//procedure TFramePopupRank.Init(const ACaption: String;
//                                AOnGetDataJsonEvent: TTimerTaskNotify;
//                                ADataJsonArrayKey: String;
//                                AItemCaptionBindingFieldName:String;
//                                AItemStyle:String='';
...
//end;
//
//procedure TFramePopupRank.Init(
//  const ACaption: String;
//  AMenuCaptions: array of String;
//  AItemStyle: String;
//  AIsShowFilter:Boolean;
...
procedure TFramePopupRank.lbMenusClickItem(Sender: TSkinItem);
begin
...
procedure TFramePopupRank.lbMenusClickItemDesignerPanelChild(Sender: TObject;
  AItem: TBaseSkinItem; AItemDesignerPanel: TSkinFMXItemDesignerPanel;
  AChild: TFmxObject);
var
  I:Integer;
...
begin
  ...
  // 升序
  if AChild.Name='btnRankUp' then
  begin
    if TSkinItem(AItem).Detail = '升序' then
      TSkinItem(AItem).Detail:= ''
    else
      TSkinItem(AItem).Detail:= '升序';
  end;

  // 降序
  if AChild.Name='btnRankDown' then
  begin
    if TSkinItem(AItem).Detail = '降序' then
      TSkinItem(AItem).Detail:= ''
    else
      TSkinItem(AItem).Detail:= '降序';
  end;

  // 重置
  if AChild.Name='btnReset' then
  begin

    for I := 0 to Self.lbMenus.Prop.Items.Count-1 do
    begin
      ANowItem:=Self.lbMenus.Prop.Items[I];
      ANowItem.Detail:= '';
    end;
  ...
  // 确认
  if AChild.Name='btnConfirm' then
  begin
//    ShowMessage('确认');
    HidePopupRank;
  end;
...
// 绘制列表项样式
procedure TFramePopupRank.lbMenusPrepareDrawItem(Sender: TObject;
  ACanvas: TDrawCanvas; AItemDesignerPanel: TSkinFMXItemDesignerPanel;
  AItem: TSkinItem; AItemDrawRect: TRect);
var
  AFrame:TFrameCustomerRankListItemStyle;
begin

  if AItem.ItemType=sitDefault then
  begin
    AFrame:=TFrameCustomerRankListItemStyle(AItemDesignerPanel.Parent);

    if AItem.Detail = '' then
    begin
      AFrame.btnRankUp.Material.DrawCaptionParam.FontColor:= TAlphaColorRec.Black;
      AFrame.btnRankUp.Material.BackColor.BorderColor.Color:= TAlphaColorRec.Lightgray;
      AFrame.btnRankUp.Material.BackColor.FillColor.Color:= TAlphaColorRec.White;

      AFrame.btnRankDown.Material.DrawCaptionParam.FontColor:= TAlphaColorRec.Black;
      AFrame.btnRankDown.Material.BackColor.BorderColor.Color:= TAlphaColorRec.Lightgray;
      AFrame.btnRankDown.Material.BackColor.FillColor.Color:= TAlphaColorRec.White;

    end;

    // 如果当前项选择了，就绘制按钮
    if AItem.Detail = '升序' then
    begin
      AFrame.btnRankUp.Material.DrawCaptionParam.FontColor:= TAlphaColorRec.White;
      AFrame.btnRankUp.Material.BackColor.BorderColor.Color:= $FF1681FF;
      AFrame.btnRankUp.Material.BackColor.FillColor.Color:= $FF1681FF;

      AFrame.btnRankDown.Material.DrawCaptionParam.FontColor:= TAlphaColorRec.Black;
      AFrame.btnRankDown.Material.BackColor.BorderColor.Color:= TAlphaColorRec.Lightgray;
      AFrame.btnRankDown.Material.BackColor.FillColor.Color:= TAlphaColorRec.White;
    end;
    if AItem.Detail = '降序' then
    begin
      AFrame.btnRankDown.Material.DrawCaptionParam.FontColor:= TAlphaColorRec.White;
      AFrame.btnRankDown.Material.BackColor.BorderColor.Color:= $FF1681FF;
      AFrame.btnRankDown.Material.BackColor.FillColor.Color:= $FF1681FF;
...
procedure TFramePopupRank.lbMenusStayClick(Sender: TObject);
var
  I:Integer;
  ANowItem:TSkinItem;
begin
...
procedure TFramePopupRank.PrepareForShow(const ACaption: String;
                                          AItemStyle:String;
                                          AIsShowFilter:Boolean;
                                          AWidth:Double);
begin
...
//  Self.pnlPopupRank.Width:=300;//MenuWidth;
////  SetProp(0,0);
//  //标题为空时不显示标题及分割线
//  if ACaption='' then
//  begin
...
//
//
//  Self.pnlPopupRank.Width:=AWidth;
//
...
//procedure TFramePopupRank.SetProp(AFontHorzAlign, AWidth: Integer);
//begin
//  //标题FontHorzAlign
...
//
//  //设置菜单框宽度  0不修改
//  if (AWidth<>0) AND (AWidth<>Self.pnlPopupRank.Width) then
//  begin
//    Self.pnlPopupRank.Width:=AWidth;
//  end;
//end;
...
//procedure TFramePopupRank.InitCategory(
//        //分类
//        ACategoryCaptions:Array of String;
//        ACategoryNames:Array of String;
//        //判断列表项属于哪个分类的事件

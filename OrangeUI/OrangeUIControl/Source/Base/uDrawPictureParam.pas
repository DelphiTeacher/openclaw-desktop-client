...
//    function SaveToJson(ASuperObject:ISuperObject):Boolean;override;
  ...
  published
    property FixedColor:TDrawColor read FFixedColor write SetFixedColor;
    /// <summary>
    ///   <para>
    ///     旋转角度
    ///   </para>
...
//    function CustomSaveToJson(ASuperObject:ISuperObject):Boolean;override;
  ...
  public
    ...
    function CurrentEffectFixedColor:TDrawColor;
...
begin
  FFixedColor:=TDrawColor.Create('FixedColor','SVG填充颜色');
//  {$IFDEF VCL}
...
function TBaseDrawPictureParam.CurrentEffectFixedColor: TDrawColor;
begin
  Result:=Self.FFixedColor;
  if (CurrentEffect<>nil) and (dppetFixedColorChange in TDrawPictureParamEffect(CurrentEffect).FEffectTypes) then
  begin
//    case TDrawPictureParamEffect(CurrentEffect).FFillColorChangeType of
//      cctNone:
          Result:=TDrawPictureParamEffect(CurrentEffect).FFixedColor;
//      cctBrightness:
//        begin
////          FTempFixedColor.Assign(FFixedColor);
//          FTempFixedColor.FColor:=FFixedColor.Color;
//          FTempFixedColor.FColor:=BrightnessColor(FTempFixedColor.Color,20);
//          Result:=FTempFixedColor;
//        end;
//      cctDarkness:
//        begin
////          FTempFixedColor.Assign(FFixedColor);
//          FTempFixedColor.FColor:=FFixedColor.Color;
//          FTempFixedColor.FColor:=DarknessColor(FTempFixedColor.Color,20);
//          Result:=FTempFixedColor;
//        end;
//    end;
  end;
...
begin
  ...
  ABTNode:=ADocNode.AddChildNode_Color('FixedColor','填充颜色');
...
//    DestObject.FIsDrawSquareCenterBlock:=FIsDrawSquareCenterBlock;
    ...
    DestObject.FFixedColor.Assign(FFixedColor);
  end;
...
//  FFixedColor.Color:=Const_DefaultFillColor;
//  {$IFDEF VCL}
//  FFixedColor.StoredDefaultColor:=TColors.SysDefault;
//  FFixedColor.FColor:=TColors.SysDefault;
//  {$ENDIF}
...
procedure TBaseDrawPictureParam.SetFixedColor(const Value: TDrawColor);
begin
...
function TBaseDrawPictureParam.SetVariable(AVariableNames:TStringList;AValueStr: String):Boolean;
begin
  Result:=inherited;
  ...
  begin
      if AVariableNames[0]='FixedColor' then
      begin
        Self.FixedColor.Color:=ColorNameToColor(AValueStr);
        Result:=True;
      end
      else if AVariableNames[0]='PictureHorzAlign' then
      begin
        Self.FPictureHorzAlign:=GetPictureHorzAlign(AValueStr);
        Result:=True;
      end
      else if AVariableNames[0]='PictureVertAlign' then
      begin
        Self.FPictureVertAlign:=GetPictureVertAlign(AValueStr);
        Result:=True;
      end
...
destructor TDrawPictureParamEffect.Destroy;
begin
  FreeAndNil(FFixedColor);
...
begin
  ...
  ABTNode:=ADocNode.AddChildNode_Color('FixedColor','字体颜色');
  ABTNode.ConvertNode_Color.Data.Assign(FFixedColor);
...
procedure TDrawPictureParamEffect.SetFixedColor(const Value: TDrawColor);
begin
  FFixedColor.Assign(Value);
end;

function TDrawPictureParamEffect.SetVariable(AVariableNames: TStringList;
...
begin
  ...
  begin
    ...
    begin
      Self.FixedColor.Color:=ColorNameToColor(AValueStr);
      FEffectTypes:=FEffectTypes+[dppetFixedColorChange];
    end
    else if AVariableNames[0]='RotateAngle' then
    begin
      Self.FRotateAngle:=StrToInt(AValueStr);
    end
    else if AVariableNames[0]='ImageIndex' then
    begin
      Self.FImageIndex:=StrToInt(AValueStr);
      FEffectTypes:=FEffectTypes+[dppetImageIndexChange];
    end
    else if AVariableNames[0]='ImageName' then
    begin
      Self.FImageName:=AValueStr;
      FEffectTypes:=FEffectTypes+[dppetImageNameChange];
    end
    else if AVariableNames[0]='EffectTypes' then
    begin
      Self.EffectTypes:=GetSetFromStr_TDPPEffectTypes(AValueStr);
    end;
  end;

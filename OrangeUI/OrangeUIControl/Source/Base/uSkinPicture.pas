...
//    {$ENDIF FMX}
    ...
    procedure LoadSVGFromStream(Stream: TStream);
  protected
...
//    function GetGraphicType:String;
    function IsNeedStored:Boolean;virtual;
...
//    property SkinThemeColorChange:Boolean read FSkinThemeColorChange write SetSkinThemeColorChange;
  ...
  TCreateSVGObjectEvent=function(Sender:TObject;ASVGStream:TStream):TObject;


var
  GlobalChangedColorBySkinThemePictureList:TBaseList;
  GlobalOnCreateSVGObject:TCreateSVGObjectEvent;
  GlobalIsHintOnCreateSVGObject:Boolean;
...
//  end;
//
//  //用了skia之后 ，调用AndroidRoundBitmap画出来 的图片会变d
////  {$IFDEF ANDROID}
...
//      ResultCanvas:=TCanvasManager.CreateFromBitmap(Result,TCanvasQuality.SystemDefault);
...
function TSVGHeader_Check(Stream: TStream): Boolean;
var
  SVGHeader		: array[0..3] of byte;
//  ColorCount		: integer;
  Position		: integer;
begin
  Result:=False;
  ...
  ReadCheck(Stream, SVGHeader, sizeof(SVGHeader));
//  if (uppercase(SVGHeader.Signature) <> 'GIF') then
    if ((SVGHeader[0] = Ord('<'))
        or (SVGHeader[0] = Ord('<')))
//    and ((SVGHeader[1] = Ord('S'))
//        or (SVGHeader[1] = Ord('s')))
//    and ((SVGHeader[2] = Ord('V'))
//        or (SVGHeader[2] = Ord('v')))
//    and ((SVGHeader[3] = Ord('G'))
//        or (SVGHeader[3] = Ord('g')))
  then
  begin
    Result:=True;
  end;
...
function TSkinPicture._AddRef: Longint;{$IFNDEF WINDOWS}cdecl{$ELSE}stdcall{$ENDIF};
begin
  Result := InterlockedIncrement(FRefCount);
end;


function TSkinPicture._Release: Longint;{$IFNDEF WINDOWS}cdecl{$ELSE}stdcall{$ENDIF};
begin
...
function TSkinPicture.QueryInterface({$IFDEF FPC_HAS_CONSTREF}constref{$ELSE}const{$ENDIF} IID: TGUID; out Obj): HResult;{$IFNDEF WINDOWS}cdecl{$ELSE}stdcall{$ENDIF};
begin
...
var
  ...
  ASVGFileDataStream:TMemoryStream;
begin
  Result:=False;
  ...
  ASVGFileDataStream:=TMemoryStream.Create;
...
//  ABTNode.ConvertNode_Int32.Data:=FSkinThemeColor;
    ...
    else if Copy(ABTNode.NodeName,1,Length('SVGFileData'))='SVGFileData' then
    begin
      //加载SVG图片
      ABTNode.ConvertNode_Binary.SaveToStream(ASVGFileDataStream);
    end
    ;
  end;
  ...
  //加载图片
  if (AGraphicType<>'') and (AGraphicStream.Size>0) and (ASVGFileDataStream.Size=0) then
  begin
    {$IFDEF VCL}
  ...
  //加载SVG
  ASVGFileDataStream.Position:=0;
  //Self.FSVGStream.LoadFromStream(ASVGFileDataStream);
  Self.LoadSVGFromStream(ASVGFileDataStream);



  FreeAndNil(AGraphicStream);
  FreeAndNil(AGIFFileDataStream);
  FreeAndNil(ASVGFileDataStream);
...
//  ABTNode.ConvertNode_WideString.Data:=FName;
  ...
  if (AGraphicType<>'') and (Not Self.IsEmpty) and (Self.FSVGObject=nil) then
  begin
    //保存图片数据
  ...
  //保存SVG图片数据
  ABTNode:=ADocNode.AddChildNode_Binary('SVGFileData');//,'SVG图片数据');
  ABTNode.ConvertNode_Binary.LoadFromStream(FSVGStream);
...
procedure TSkinPicture.SaveToJson(AJson: ISuperObject);
begin
//  if (Not Self.IsEmpty) then
//  begin
//    //保存图片类型
...
begin
  ...
  begin
      ...
      Self.FGIFStream.Size:=0;
      Self.FGIFStream.LoadFromStream(TSkinPicture(Source).FGIFStream);
      TSkinPicture(Source).FGIFStream.Position:=0;

      //SVGFile赋值,SVG图片需要把流保存起来
//      TSkinPicture(Source).FSVGStream.Position:=0;
//      Self.FSVGStream.Size:=0;
//      Self.FSVGStream.LoadFromStream(TSkinPicture(Source).FSVGStream);
//      TSkinPicture(Source).FSVGStream.Position:=0;
//       if TSkinPicture(Source).FSVGStream.Size>0 then
//       begin
        TSkinPicture(Source).FSVGStream.Position:=0;
        Self.LoadSVGFromStream(TSkinPicture(Source).FSVGStream);
        TSkinPicture(Source).FSVGStream.Position:=0;
//       end;
...
begin
  ...
  FSVGStream:=TMemoryStream.Create;
...
begin
  ...
  begin
//    {$IFDEF FMX}
    ARoundBitmap:=GlobalDrawCanvasClass.RoundSkinPicture(Self,
                                  Self.FClipRoundXRadis,
                                  Self.FClipRoundYRadis);//,
//                                  Self.FClipRoundCorners);
    try
      //必须用CopyBitmap,不然会花屏,在10.2,10.3下面
...
begin
  ...
  begin
    FSkinPictureEngine:=CreateCurrentEngineSkinPictureEngine(Self,Self.GIFSupport);
  end;
  Result:=FSkinPictureEngine;
end;

//function TSkinPicture.GetGraphicType: String;
//begin
//  Result:='';
//  if (Not Self.IsEmpty) then
//  begin
...
//    {$IFDEF VCL}
//    //VCL要保存图片类型，因为每个图片用不同的类型
//    Result:=GetGraphicTypeByGraphicClass(TGraphicClass(Self.Graphic.ClassType));
////    if AGraphicType<>'' then
////    begin
////      ABTNode:=ADocNode.AddChildNode_WideString('GraphicType');//,'图片类型');
////      ABTNode.ConvertNode_WideString.Data:=AGraphicType;
...
begin
  ...
  try
    ...
    //尝试加载SVG
    Self.LoadSVGFromStream(AFileStream);
...
procedure TSkinPicture.LoadFromJson(AJson: ISuperObject);
begin
...
begin
  ...
  //<svg
  Stream.Position:=0;
  Self.LoadSVGFromStream(Stream);
  if TSVGHeader_Check(Stream) then
  begin
    Exit;
  end;
...
procedure TSkinPicture.LoadSVGFromStream(Stream: TStream);
var
  Position:Integer;
begin
  FreeAndNil(FSVGObject);
  ...
  begin
    ...
    begin
      Stream.Position:=Position;
      FSVGStream.LoadFromStream(Stream);
      Stream.Position:=Position;

      if not Assigned(GlobalOnCreateSVGObject) and not GlobalIsHintOnCreateSVGObject then
      begin
        GlobalIsHintOnCreateSVGObject:=True;
        {$IFDEF VCL}
        raise Exception.Create('Please use uVCLSVGSupport.pas');
        {$ENDIF}
        {$IFDEF FMX}
        raise Exception.Create('Please use uFMXSVGSupport.pas');
        {$ENDIF}
      end;

      if Assigned(GlobalOnCreateSVGObject) then
      begin
        FSVGObject:=GlobalOnCreateSVGObject(Self,FSVGStream);
      end;
...
begin
  {$IFDEF VCL}
  Result:=((Graphic=nil) or (Graphic.Empty)) and not (Self.FSVGObject<>nil);
  {$ENDIF}
  {$IFDEF FMX}
  Result:=(Inherited IsEmpty) and not (Self.FSVGObject<>nil);
  {$ENDIF}
end;

function TSkinPicture.IsNeedStored: Boolean;
begin
  Result:=not IsEmpty;
end;
...
//  Filer.DefineBinaryProperty('Data', ReadData, WriteData, DoWrite);
  ...
  Filer.DefineBinaryProperty('SVGFileData', ReadSVGData, WriteSVGData,(FSVGStream<>nil) and (FSVGStream.Size>0));
...
procedure TSkinPicture.ReadSVGData(Stream: TStream);
begin
//  uBaseLog.HandleException(nil,'TSkinPicture.ReadGIFData Begin');
...
procedure TSkinPicture.WriteSVGData(Stream: TStream);
begin
//  uBaseLog.HandleException(nil,'TSkinPicture.WriteGIFData Begin');

  if (FSVGStream<>nil) and (FSVGStream.Size>0) then
  begin
    FSVGStream.Position:=0;
    Stream.CopyFrom(Self.FSVGStream,FSVGStream.Size);
  end;

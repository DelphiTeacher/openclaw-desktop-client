...
//  TProtectedUrlCacheItem=class(TUrlCacheItem)
//
//  end;
...
//    procedure SetItem(Index:Integer; const Value: TUrlCacheItem);
  protected
    ...
    function CanSaveToDocNode(ABinaryObject:TObject):Boolean;override;//TInterfacedPersistent):Boolean;override;
  public
...
//                              AClipRoundYRadis:Double=0
  ...
  protected
    FNotifyUrlCacheItem:TUrlCacheItem;
    procedure DoUrlCacheItemNotifyDownloadStateChange;
  protected
...
//    procedure SetItem(Index:Integer; const Value: TUrlCacheItem);
  ...
  private
    FCustomLoadUrlCacheItem:TUrlCacheItem;
    FCustomLoadResponseStream:TStream;
    procedure DoUrlCacheItemLoadPictureDownloadedStream;
    //决定保存成什么后缀
  ...
  private
    FLoadUrlPicture:TUrlPicture;
    //不下载网络图片(用于延迟加载)
    procedure DoLoadItemInThread(ATimerTask:TObject);override;
    procedure DoUrlPictureLoadPictureDownloaded;
  public
...
begin
  ...
  try
      ...
      {$ENDIF}
      {$IFDEF FPC}
      FNotifyUrlCacheItem:=AUrlCacheItem;
      TThread.Synchronize(nil,DoUrlCacheItemNotifyDownloadStateChange);
      {$ENDIF}
...
procedure TDefaultDownloadManager.DoUrlCacheItemNotifyDownloadStateChange;
begin
  FNotifyUrlCacheItem.DoNotifyDownloadStateChange(False);
...
function TUrlCacheItemList.CanSaveToDocNode(ABinaryObject: TObject):Boolean;//TInterfacedPersistent): Boolean;
begin
  Result:=(TUrlCacheItem(ABinaryObject).State=dpsDownloadSucc);
end;

function TUrlCacheItemList.CreateBinaryObject(const AClassName:String=''): TObject;//TInterfacedPersistent;
begin
...
procedure TBaseDownloadPictureManager.DoUrlCacheItemLoadPictureDownloadedStream;
begin
  //在线程中加载图片
  TUrlPicture(FCustomLoadUrlCacheItem).LoadPictureDownloadedStream(FCustomLoadResponseStream);
...
begin
  ...
  begin
        ...
        {$IFDEF FPC}
          FCustomLoadUrlCacheItem:=AUrlCacheItem;
          FCustomLoadResponseStream:=AResponseStream;
          TThread.Synchronize(nil,DoUrlCacheItemLoadPictureDownloadedStream);
        {$ENDIF}
...
//  {$ENDIF MSWINDOWS}
  ...
  {$IFDEF FPC}
  TThread.Synchronize(nil,DoUrlPictureLoadPictureDownloaded);
  {$ENDIF}
...
procedure TBaseDownloadPictureManager.DoUrlPictureLoadPictureDownloaded;
begin
  //在线程中加载图片
  FLoadUrlPicture.LoadPictureDownloaded;

end;

function TBaseDownloadPictureManager.DownloadPicture(const AUrl: String;
  const ADrawPicture: TObject; const AData: Pointer;
  const ASavedFilePath: String; const AFileExt: String;
  const ALogCaption: String;
  AOnDownloadStateChange: TDownloadProgressStateChangeEvent;
...
function TUrlPictureList.CreateBinaryObject(
  const AClassName: String): TObject;//TInterfacedPersistent;
begin

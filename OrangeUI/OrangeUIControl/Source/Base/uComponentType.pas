...
const
  ...
  IID_IBindSkinItemArrayControl:TGUID='{6542912B-3D24-44DC-9495-CEF99BC51928}';
...
//  TOnGetParamValueTypeEvent=function(AValueFrom:String;AParamName:String):TVarType of object;
...
//  TOnGetDrawPictureEvent=function(AImageName:String;var AIsGeted:Boolean):TDrawPicture of object;
  ...
  TBindDataSource=class(TComponent,IDataSource,IControlForPageFramework)
  private
//    FOnGetParamValueType: TOnGetParamValueTypeEvent;
...
//    FOnGetDrawPicture: TOnGetDrawPictureEvent;
  public
    FActive: Boolean;
    //是否已经获取到了数据
    FIsGetedData:Boolean;
    FBindedControlList:TBaseList;
    ...
    //获取到了数据,给页面上绑定的控件赋值
    procedure DoGetedData;virtual;
    procedure SetControlFieldValue(AControl:TPersistent);

    procedure BindControl(AControl:TPersistent);
    procedure UnBindControl(AControl:TPersistent);
    //本地数据,比如医院显示屏项目,保存办公室fid
    //FDataJson:ISuperObject;
//    function GetValueArray(AParamName:String):ISuperArray;virtual;abstract;
//    function GetValue(AParamName:String):Variant;virtual;abstract;
//    function GetValueObject(AParamName:String):TObject;virtual;abstract;
    procedure SetValue(AParamName:String;AValue:Variant);virtual;abstract;
    //获取图片,比如列表项的图片
//    function GetDrawPicture(AImageName:String):TDrawPicture;virtual;abstract;
  ...
  protected
    //保存页面的时候要能保存到json里面 IControlForPageFramework
    //针对页面框架的控件接口
...
//    procedure SetProp(APropName:String;APropValue:Variant);
  ...
  public
    ...
    //获取字段列表,绑定的时候需要供用户选择
    function GetFieldDefList(ABindItemFieldName:String):TFieldDefs;virtual;abstract;
    //根据绑定的FieldName获取Item的值,然后赋给绑定的控件
    function GetValueTypeByBindItemField(AFieldName:String):TVarType;virtual;
    function GetValueByBindItemField(AFieldName:String):Variant;virtual;abstract;
...
//    property OnGetParamValueType:TOnGetParamValueTypeEvent read FOnGetParamValueType write FOnGetParamValueType;

    property OnSetParamValue:TOnSetParamValueEvent read FOnSetParamValue write FOnSetParamValue;
  end;
  TBindDataSourceList=class(TBaseList)
  private
    function GetItem(Index: Integer): TBindDataSource;
  public
    function GetVaribleValue(AVariableName:String):String;
    function Find(AName:String):TBindDataSource;
    property Items[Index:Integer]:TBindDataSource read GetItem;default;
  end;

  {$REGION '列表项绑定'}

  TBindItemFieldSetting=class(TCollectionItem)
  private
  ...
  TBindItemFieldSettingList=class(TCollection)
...
//    function GetBindFieldSettings:TBindItemFieldSettingList;
...
//    //返回哪些属性绑定了哪些字段，[属性1,字段1，属性2，字段2]
//    function GetControlPropertyValueBindings:TStringDynArray;
    //给控件赋值
  ...
  IBindSkinItemArrayControl=interface
    ['{6542912B-3D24-44DC-9495-CEF99BC51928}']
    //给控件赋数组
...
//  IBindSkinItemColorControl=interface
//  ['{EB4BC032-AD58-4E44-A87B-156FF6402067}']
//    /// <summary>
//    ///   <para>
//    ///     绑定列表项Item的颜色属性
//    ///   </para>
//    ///   <para>
//    ///     Bind property Color of Item
//    ///   </para>
//    /// </summary>
//    procedure BindingItemColor(AItemColor:TDrawColor;AIsDrawItemInteractiveState:Boolean);
//  end;
  ...
  protected
    ...
    procedure SaveToJson(ASuperObject:ISuperObject);virtual;
    procedure LoadFromJson(ASuperObject:ISuperObject);virtual;
  protected
    /// <summary>
    ///   <para>
    ///     重绘控件
...
var
  ...
  GlobalBindDataSourceList:TBindDataSourceList;
...
//添加控件的释放通知
//ASubComponent释放了通知AMainComponent
{$IFDEF FMX}
...
function GetParentIsScrollControl(
            AChild:TComponent;
            var AParentIsScrollControl:Boolean;
            var AParentScrollControl:TControl;
            ACheckLevel:Integer=8
            ):Boolean;
...
function GetParentIsScrollControl(AChild:TComponent;
                                    var AParentIsScrollControl:Boolean;
                                    var AParentScrollControl:TControl;
                                    ACheckLevel:Integer):Boolean;
var
...
begin
  ...
  if (AChild is TSkinScrollControl) then
  begin
    //如果这个ScrollBox的滚动条都不能动,那么只是一个普通的Panel控件而已
    AScrollControl:=TSkinScrollControl(AChild);
    if AScrollControl.Prop.VertScrollBarShowType<>TScrollBarShowType.sbstNone then
    begin
      Exit;
    end;
  end
...
//      end;
      ...
      begin
          AScrollControl:=TSkinScrollControl(AParent);


//          if AScrollBox.Prop.VertScrollBarShowType<>TScrollBarShowType.sbstNone then//不能这样写,不然水平滚动框就传递不下去了
//          begin
              Result:=True;
              AParentScrollControl:=TControl(AParent);
              Break;
//          end
//          else
//          begin
...
begin
  ...
  begin
    if BParent<>nil then
    begin
      if BParent is TControl then
      begin
        Result:=Result+GetControlLeft(TControl(BParent));
      end
      else if BParent is TForm then
      begin
        Exit;
      end;
...
begin
  ...
  begin
    ...
    begin
      if BParent is TControl then
      begin
        Result:=Result+GetControlTop(TControl(BParent));
      end
      else if BParent is TForm then
      begin
        Exit;
...
begin
  Result:=CreateCurrentEngineDrawPicture(AName,ACaption,AGroup);
...
procedure TSkinControlProperties.LoadFromJson(ASuperObject: ISuperObject);
begin
...
function TSkinControlProperties.GetOwner: TPersistent;
begin
  Result:=FSkinControl;
end;

procedure TSkinControlProperties.SaveToJson(ASuperObject: ISuperObject);
begin
...
begin
  ...
  begin
    ...
    begin
        ...
        {$ELSE}
        OffsetRectF(AControlWindowRect,ADrawRect.Left,ADrawRect.Top);
        {$ENDIF}
...
function TBindDataSourceList.Find(AName: String): TBindDataSource;
var
  I: Integer;
begin
  Result:=nil;
...
function TBindDataSourceList.GetItem(
  Index: Integer): TBindDataSource;
begin
  Result:=TBindDataSource(Inherited Items[Index]);

end;

function TBindDataSourceList.GetVaribleValue(AVariableName: String): String;
var
...
begin
  Result:='';
  //{dsLogin.Data.appid}
  //{dsLogin.Params.appid}
  //先找出属于哪个数据集
  ADotIndex:=Pos('.',AVariableName);
  if ADotIndex>0 then
  begin
    ADataSourceName:=Copy(AVariableName,2,ADotIndex-2);

    AVariableName:=Copy(AVariableName,ADotIndex+1,Length(AVariableName)-(ADotIndex+1));
    ADataSource:=Self.Find(ADataSourceName);
    if ADataSource<>nil then
    begin
      Result:=ADataSource.GetValueByBindItemField(AVariableName);
    end;
  end;
...
procedure TBindDataSource.BindControl(AControl: TPersistent);
begin
  if FBindedControlList.IndexOf(AControl)=-1 then
  begin
    FBindedControlList.Add(AControl);


    if AControl is TComponent then
    begin
      //DataSource释放了通知AControl
      uComponentType.AddFreeNotification(TComponent(AControl),Self);
    end;


    //给控件赋值
    SetControlFieldValue(AControl);
  end;
end;

function TBindDataSource.CanOpen: Boolean;
begin
  Result:=True;
end;

procedure TBindDataSource.Close;
begin
  FActive:=False;

end;

constructor TBindDataSource.Create(AComponent: TComponent);
begin
  inherited;
  FBindedControlList:=TBaseList.Create(ooReference);
  FFieldDefs:=TFieldDefs.Create(nil);
  //它是放在Page上面的
  GlobalBindDataSourceList.Add(Self);
end;

destructor TBindDataSource.Destroy;
begin
  GlobalBindDataSourceList.Remove(Self,False);
  FreeAndNil(FFieldDefs);
  FreeAndNil(FBindedControlList);
  inherited;
end;

procedure TBindDataSource.DoGetedData;
var
  I: Integer;
begin
  Self.FIsGetedData:=True;
  for I := 0 to FBindedControlList.Count-1 do
  begin
    SetControlFieldValue(TControl(FBindedControlList[I]));
  end;
end;

//function TBindDataSource.GetObjectByBindItemField(
//  AFieldName: String): TObject;
//begin
//  Result:=GetValueObject(AFieldName);
//end;
//
//function TBindDataSource.GetValueArrayByBindItemField(
//  AFieldName: String): ISuperArray;
//begin
//  Result:=GetValueArray(AFieldName);
//end;
//
//function TBindDataSource.GetValueByBindItemField(
//  AFieldName: String): Variant;
//begin
//  Result:=GetValue(AFieldName);
//end;

function TBindDataSource.GetValueTypeByBindItemField(
  AFieldName: String): TVarType;
begin
  Result:=varString;
end;

procedure TBindDataSource.SaveToJson(AJson: ISuperObject);
begin

end;

procedure TBindDataSource.SetControlFieldValue(AControl:TPersistent);
var
  AFieldName:String;
  AValueType:TVarType;
  ...
  ABindSkinItemArrayControlIntf:IBindSkinItemArrayControl;
begin
  try
      if not FIsGetedData then
      begin
        Exit;
      end;
      //看这个控件绑定了哪些属性,然后给它赋值
      if not AControl.GetInterface(IID_ISkinItemBindingControl,ASkinItemBindingControlIntf) then
      begin
        Exit;
      end;
      ...
      AValueType:=GetValueTypeByBindItemField(AFieldName);
      case AValueType of
        varArray:
        begin
          if not AControl.GetInterface(IID_IBindSkinItemArrayControl,ABindSkinItemArrayControlIntf) then
          begin
            Exit;
          end;
          ABindSkinItemArrayControlIntf.SetControlArrayByBindItemField(AFieldName,
                                                GetValueArrayByBindItemField(AFieldName),
                                                '',
                                                nil,
                                                nil,
                                                False);
        ...
        varObject:
        begin
          if not AControl.GetInterface(IID_IBindSkinItemObjectControl,ABindSkinItemObjectControlIntf) then
          begin
            Exit;
          end;
          ABindSkinItemObjectControlIntf.SetControlObjectByBindItemField(AFieldName,
                                                GetObjectByBindItemField(AFieldName),
                                                '',
                                                nil,
                                                nil,
                                                False);
        ...
        begin
          if not AControl.GetInterface(IID_IBindSkinItemValueControl,ABindSkinItemValueControlIntf) then
          begin
            Exit;
          end;
          ABindSkinItemValueControlIntf.SetControlValueByBindItemField(AFieldName,
                                                GetValueByBindItemField(AFieldName),
                                                '',
                                                nil,
                                                nil,
                                                False);
    ...
    on E:Exception do
    begin
      uBaseLog.HandleException(E,'TBindDataSource.SetControlFieldValue')
    end;
  end;
end;

procedure TBindDataSource.UnBindControl(AControl: TPersistent);
begin
  if FBindedControlList.IndexOf(AControl)<>-1 then
  begin
    FBindedControlList.Remove(AControl,False);
    if (AControl is TComponent) then
    begin
      uComponentType.RemoveFreeNotification(TComponent(AControl),Self);
    end;
  end;
end;

procedure TBindDataSource.SetActive(const Value: Boolean);
begin
...
procedure TBindDataSource.Open;
//var
//  ADesc:String;
...
function TBindDataSource.GetPropJsonStr:String;
var
  ASuperObject:ISuperObject;
begin
  ASuperObject:=TSuperObject.Create;
...
procedure TBindDataSource.SetPropJsonStr(AJsonStr:String);
var
  ASuperObject:ISuperObject;
begin
...
function TBindDataSource.GetPostValue(ASetting:TFieldControlSetting;APageDataDir: String;ASetRecordFieldValueIntf:ISetRecordFieldValue;
                            var AErrorMessage:String): Variant;
begin
end;


function TBindDataSource.LoadFromFieldControlSetting(ASetting: TFieldControlSetting;AFieldControlSettingMap:TObject): Boolean;
begin

end;

procedure TBindDataSource.LoadFromJson(AJson: ISuperObject);
begin

end;

procedure TBindDataSource.SetControlValue(ASetting:TFieldControlSetting;APageDataDir, AImageServerUrl: String;AValue: Variant;AValueCaption:String;
                            //要设置多个值,整个字段的记录
                            AGetDataIntfResultFieldValueIntf:IGetDataIntfResultFieldValue);
begin
end;
...
procedure TBindDataSource.DoReturnFrame(AFromFrame:TFrame);
begin
...
initialization
  ...
  GlobalBindDataSourceList:=TBindDataSourceList.Create(ooReference);
...
finalization
  ...
  FreeAndNil(GlobalBindDataSourceList);

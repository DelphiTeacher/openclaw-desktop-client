...
//                    patGetRegion
  ...
  private
    ...
    procedure CalcAreaRectX(const Value: Double);
    procedure CalcAreaRectY(const Value: Double);
    ...
    procedure SetX3(const Value: Double);
    procedure SetY3(const Value: Double);
...
//    property SizeChange:Double read FSizeChange write FSizeChange;
  ...
  public
    ...
    procedure SetPoints(APoints:array of TPointF);
    ...
    //得到终点
    function GetArcStartPoint(Const ADrawRect:TRectF):TPointF;
    function GetArcStopPoint(Const ADrawRect:TRectF):TPointF;

    function GetX3(Const ADrawRect:TRectF):Double;
    function GetY3(Const ADrawRect:TRectF):Double;
  published
    //点
    //画线条时使用,添加矩形框时使用
    ...
    property X3:Double read FX3 write SetX3;
    property Y3:Double read FY3 write SetY3;
  ...
  public
    ...
    procedure Clear;
    procedure CalcRoundRect(APoints:array of TPointF);
...
//    function GetAreaRect(ADrawRect:TRectF):TRectF;
  end;
  ...
  private
    ...
    function IsFillColorChangeTypeStored: Boolean;
  protected
    procedure AssignTo(Dest: TPersistent); override;
  public
    FIsFill: Boolean;
...
//    function SaveToJson(ASuperObject:ISuperObject):Boolean;override;
  ...
  public
    property FillColorChangeType:TColorChangeType read FFillColorChangeType write SetFillColorChangeType stored IsFillColorChangeTypeStored;
  published
  ...
  private
    ...
    FFillGradientColor1:TDrawColor;
//    function GetFillGradientColor1: TDrawColor;
    procedure SetBrushKind(const Value: TDRPBrushKind);
    function IsGradientStored: Boolean;
    procedure SetFillGradientColor1(const Value: TDrawColor);
...
//    function CurrentEffectIsFill:Boolean;
    ...
    ///   </para>
    /// </summary>
    function CurrentEffectPenWidth:Double;
    /// <summary>
    ///   <para>
    ///     当前效果的边框颜色
    ///   </para>
  ...
  public
    ...
    destructor Destroy;override;
    property StaticPenWidth:Double read FPenWidth write FPenWidth;
  public
    property FillGradientColor1: TDrawColor read FFillGradientColor1 write SetFillGradientColor1 stored IsGradientStored;
    property GradientStyle: TGradientStyle read FGradientStyle write SetGradientStyle stored IsGradientStored;
    ...
    ///   </para>
    /// </summary>
    property PenWidth:Double read FPenWidth write SetPenWidth stored IsRectParamPenWidthStored;
...
//根据多边形的轮廓,计算出一个矩形的区域，用于地图图表
function CalcAreaRect(APointFArray:array of TPointF):TRectF;
function CalculatePolygonCenter(const points: array of TPointF): TPointF;
function GetGravityPoint(lstPol: array of TPointF):TPointF;
...
var
  I: Integer;
begin
  Result:=RectF(MaxInt,MaxInt,-MaxInt,-MaxInt);
  for I := 0 to Length(APointFArray)-1 do
  begin

    if APointFArray[I].X<Result.Left then
    begin
      Result.Left:=APointFArray[I].X;
    end;
    if APointFArray[I].X>Result.Right then
    begin
      Result.Right:=APointFArray[I].X;
    end;

    if APointFArray[I].Y<Result.Top then
    begin
      Result.Top:=APointFArray[I].Y;
    end;
    if APointFArray[I].Y>Result.Bottom then
    begin
      Result.Bottom:=APointFArray[I].Y;
    end;
...
begin
  ...
  Result.X := sumX / Length(points);
  Result.Y := sumY / Length(points);
end;

function GetGravityPoint(lstPol: array of TPointF):TPointF;
var
  n, i:Integer;
  s, x1, y1, x2, y2, x3, y3, sum_x, sum_y, sum_s: Double;
  k: Integer;
begin
  sum_x := 0;
  sum_y := 0;
  ...
  //ShowMessage(Format('重心为：%.2f, %.2f', []));
  Result:=PointF(sum_x / sum_s / 3.0, sum_y / sum_s / 3.0);
end;
...
begin
  case APathActionType of
    ...
    patClose: Result:='Close';
    patStart: Result:='Start';
    patStop: Result:='Stop';
    patDrawAndFillPath: Result:='DrawAndFillPath';
  end;
end;
...
begin
  ...
  if SameText(APathActionTypeStr,'Close') then
  begin
    Result:=patClose;
  end;
  if SameText(APathActionTypeStr,'Start') then
  begin
    Result:=patStart;
  end;
  if SameText(APathActionTypeStr,'Stop') then
  begin
    Result:=patStop;
  end;
  if SameText(APathActionTypeStr,'DrawAndFillPath') then
  begin
    Result:=patDrawAndFillPath;
  end;
end;
...
function TBaseDrawPathParam.CurrentEffectPenWidth:Double;
begin
...
procedure TBaseDrawPathParam.SetFillGradientColor1(const Value: TDrawColor);
begin
//  {$IFDEF FMX}
//  FGradient.Color1:=Value.FColor;
//  {$ENDIF FMX}
  FFillGradientColor1.Assign(Value);
end;

procedure TBaseDrawPathParam.SetGradientStyle(const Value: TGradientStyle);
begin
...
//function TBaseDrawPathParam.GetFillGradientColor1: TDrawColor;
//begin
//  Result:=FFillGradientColor1.Color;
////  {$IFDEF FMX}
...
begin
  ...
  FreeAndNil(FFillGradientColor1);
...
function TBaseDrawPathParam.IsGradientStored: Boolean;
begin
//  {$IFDEF FMX}
...
procedure TBaseDrawPathParam.SetBrushKind(const Value: TDRPBrushKind);
begin
...
procedure TBaseDrawPathParam.SetPenWidth(const Value: Double);
begin
...
begin
  ...
  FFillGradientColor1:=CreateDrawColor('FillGradientColor1','渐变颜色');
...
function TDrawPathParamEffect.IsFillColorChangeTypeStored: Boolean;
begin
  Result:=FFillColorChangeType<>cctNone;
end;
...
procedure TPathActionItem.CalcAreaRectX(const Value: Double);
begin
  if Collection<>nil then
  begin
    if Value<TPathActionCollection(Collection).FAreaRect.Left then
    begin
      TPathActionCollection(Collection).FAreaRect.Left:=Value;
    end;
    if Value>TPathActionCollection(Collection).FAreaRect.Right then
    begin
      TPathActionCollection(Collection).FAreaRect.Right:=Value;
    end;
  end;

end;

procedure TPathActionItem.CalcAreaRectY(const Value: Double);
begin
  if Collection<>nil then
  begin
    if Value<TPathActionCollection(Collection).FAreaRect.Top then
    begin
      TPathActionCollection(Collection).FAreaRect.Top:=Value;
    end;
    if Value>TPathActionCollection(Collection).FAreaRect.Bottom then
    begin
      TPathActionCollection(Collection).FAreaRect.Bottom:=Value;
    end;
  end;
...
begin
  ...
  begin
    TPathActionCollection(Self.Collection).FDrawPathParam.DoChange(nil);
  end;

end;

function TPathActionItem.GetArcStartPoint(Const ADrawRect:TRectF): TPointF;
var
  ARect:TRectF;
  ACenter:TPointF;
  ARadius:Double;
begin
  ARect:=RectF(
              ADrawRect.Left+Self.GetX(ADrawRect),
              ADrawRect.Top+Self.GetY(ADrawRect),
              ADrawRect.Left+Self.GetX1(ADrawRect),
              ADrawRect.Top+Self.GetY1(ADrawRect)
              );

  //半径
  ARadius:=ARect.Width/2;
  ...
  Result:=CalcArcStopPoint(ACenter,ARadius,Self.FStartAngle);

end;

function TPathActionItem.GetArcStopPoint(Const ADrawRect:TRectF): TPointF;
var
  ARect:TRectF;
  ACenter:TPointF;
  ARadius:Double;
begin
  ...
  ACenter.X:=ARect.Left+ARect.Width/2;
  ACenter.Y:=ARect.Top+ARect.Height/2;
  Result:=CalcArcStopPoint(ACenter,ARadius,Self.FStartAngle+Self.FSweepAngle);
...
function TPathActionItem.GetX3(const ADrawRect: TRectF): Double;
begin
  case Self.FSizeType of
    dpstPixel: Result:=FX3;
    dpstPercent: Result:=ControlSize(FX3/100*(RectWidthF(ADrawRect)));
  end;
end;

function TPathActionItem.GetY3(const ADrawRect: TRectF): Double;
begin
  case Self.FSizeType of
    dpstPixel: Result:=FY3;
    dpstPercent: Result:=ControlSize(FY3/100*(RectHeightF(ADrawRect)));
  end;
end;
...
procedure TPathActionItem.SetPoints(APoints: array of TPointF);
var
  I:Integer;
begin
  ...
  begin
    FPoints[I]:=APoints[I];
    //计算出范围的矩形
    CalcAreaRectX(APoints[I].X);
    CalcAreaRectY(APoints[I].Y);
  end;
  ...
  begin
    ...
    TPathActionCollection(Collection).CalcRoundRect(APoints);
...
procedure TPathActionItem.SetX3(const Value: Double);
begin
  if FX3<>Value then
  begin
    FX3 := Value;
...
procedure TPathActionItem.SetY3(const Value: Double);
begin
  if FY3<>Value then
  begin
    FY3 := Value;
...
procedure TPathActionCollection.CalcRoundRect(APoints: array of TPointF);
var
  I:Integer;
  ARadius:Double;
//  AMinX,AMinY:Double;
begin
  ...
  //计算出中心点和半径
  FAreaCenter.X:=FAreaRect.Left+FAreaRect.Width/2;
  FAreaCenter.Y:=FAreaRect.Top+FAreaRect.Height/2;

  //其实半径这个东西,应该和所有的点去计算的,而不是根据左上角的距离来的
  //计算出半径
//  ARoundRadius:=Sqrt(Sqr(ARoundCenter.X - APathActions.FAreaRect.Left) + Sqr(ARoundCenter.Y - APathActions.FAreaRect.Top));

  //根据点计算半径
//  FAreaRadius:=0;
//  for I := 0 to Length(APoints)-1 do
//  begin
//    ARadius:=Sqrt(Sqr(FAreaCenter.X - APoints[I].X) + Sqr(FAreaCenter.Y - APoints[I].Y));
//    if ARadius>FAreaRadius then
//    begin
//      FAreaRadius:=ARadius;
//    end;
//  end;
  ...
  //其实半径这个东西,应该和所有的点去计算的,而不是根据左上角的距离来的
  //计算出半径
  FAreaRadius:=Sqrt(Sqr(FAreaCenter.X - FAreaRect.Left) + Sqr(FAreaCenter.Y - FAreaRect.Top));

  //计算出圆型
  FRoundRect:=
              RectF(FAreaCenter.X-FAreaRadius,
                    FAreaCenter.Y-FAreaRadius,
                    FAreaCenter.X+FAreaRadius,
                    FAreaCenter.Y+FAreaRadius);

end;

procedure TPathActionCollection.Clear;
begin
  Inherited Clear;
  FAreaRect:=RectF(MaxInt,MaxInt,-MaxInt,-MaxInt);
end;
...
//function TPathActionCollection.GetAreaRect(ADrawRect:TRectF):TRectF;
//var
//  I: Integer;
//  ATempValue:Double;
//begin
...
//  begin
//    if Items[I].GetX(ADrawRect)<ATempValue then
//    begin
//
//    end;
//  end;

...
const
  ...
  DefaultColor = {$IFDEF VCL}clDefault{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Null{$ENDIF};
...
//  end;
  ...
  private
    procedure SetStaticAlpha(const Value: Byte);
  protected
    FName:String;
    FCaption:String;
  ...
  TDrawColorList=class(TBaseList)
  private
    function GetItem(Index: Integer): TDrawColor;
  public
    property Items[Index:Integer]:TDrawColor read GetItem;default;
  end;
...
function StrToColor(AColorName:String):TDelphiColor;
function ColorToStr(AColor:TDelphiColor):String;

//function StrToDrawColor(AColorName:String):TDelphiColor;
//function DrawColorToStr(AColor:TDelphiColor):String;
...
//                      ctThemeColor3
//                      );
function GetUseThemeColorTypeStr(AUseThemeColorType:TUseThemeColorType):String;
function GetUseThemeColorTypeByStr(AUseThemeColorType:String):TUseThemeColorType;
...
begin
  Result:=ctNone;
  if SameText(AUseThemeColorType,'ThemeColor') then
  begin
    Result:=ctThemeColor;
  end
  else if SameText(AUseThemeColorType,'ThemeColor1') then
  begin
    Result:=ctThemeColor1;
  end
  else if SameText(AUseThemeColorType,'NavigationBarColor') then
  begin
    Result:=ctNavigationBarColor;
  end
  else if SameText(AUseThemeColorType,'NavigationBarFontColor') then
  begin
    Result:=ctNavigationBarFontColor;
  end
  else if SameText(AUseThemeColorType,'ThemeColor2') then
  begin
    Result:=ctThemeColor2;
  end
  else if SameText(AUseThemeColorType,'ThemeColor3') then
  begin
    Result:=ctThemeColor3;
  end
  ;
end;
...
begin
  Result:='';
  case AUseThemeColorType of
    ctNone: ;
    ctThemeColor: Result:='ThemeColor';
    ctThemeColor1: Result:='ThemeColor1';
    ctNavigationBarColor: Result:='NavigationBarColor';
    ctNavigationBarFontColor: Result:='NavigationBarFontColor';
    ctThemeColor2: Result:='ThemeColor2';
    ctThemeColor3: Result:='ThemeColor3';
  end;
end;
...
function ColorToStr(AColor:TDelphiColor):String;
begin
  Result:='';// = {$IFDEF VCL}0{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Null{$ENDIF};

  {$IFDEF DELPHI}
  if AColor=AliceblueColor then Result:='Aliceblue';// = AliceblueColor = {$IFDEF VCL}clWebAliceblue{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Aliceblue{$ENDIF};
  if AColor=AntiquewhiteColor then Result:='Antiquewhite';//={$IFDEF_VCL}clWebAntiquewhite{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Antiquewhite{$ENDIF};
  if AColor=AquaColor then Result:='Aqua';//={$IFDEF_VCL}clAqua{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Aqua{$ENDIF};
  if AColor=AquamarineColor then Result:='Aquamarine';//={$IFDEF_VCL}clWebAquamarine{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Aquamarine{$ENDIF};
  if AColor=AzureColor then Result:='Azure';//={$IFDEF_VCL}clWebAzure{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Azure{$ENDIF};
  if AColor=BeigeColor then Result:='Beige';//={$IFDEF_VCL}clWebBeige{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Beige{$ENDIF};
  if AColor=BisqueColor then Result:='Bisque';//={$IFDEF_VCL}clWebBisque{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Bisque{$ENDIF};
  if AColor=BlackColor then Result:='Black';//={$IFDEF_VCL}clBlack{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Black{$ENDIF};
  if AColor=BlanchedalmondColor then Result:='Blanchedalmond';//={$IFDEF_VCL}clWebBlanchedalmond{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Blanchedalmond{$ENDIF};
  if AColor=BlueColor then Result:='Blue';//={$IFDEF_VCL}clBlue{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Blue{$ENDIF};
  if AColor=BluevioletColor then Result:='Blueviolet';//={$IFDEF_VCL}clWebBlueviolet{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Blueviolet{$ENDIF};
  if AColor=BrownColor then Result:='Brown';//={$IFDEF_VCL}clWebBrown{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Brown{$ENDIF};
  if AColor=BurlywoodColor then Result:='Burlywood';//={$IFDEF_VCL}clWebBurlywood{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Burlywood{$ENDIF};
  if AColor=CadetblueColor then Result:='Cadetblue';//={$IFDEF_VCL}clWebCadetblue{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Cadetblue{$ENDIF};
  if AColor=ChartreuseColor then Result:='Chartreuse';//={$IFDEF_VCL}clWebChartreuse{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Chartreuse{$ENDIF};
  if AColor=ChocolateColor then Result:='Chocolate';//={$IFDEF_VCL}clWebChocolate{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Chocolate{$ENDIF};
  if AColor=CoralColor then Result:='Coral';//={$IFDEF_VCL}clWebCoral{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Coral{$ENDIF};
  if AColor=CornflowerblueColor then Result:='Cornflowerblue';//={$IFDEF_VCL}clWebCornflowerblue{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Cornflowerblue{$ENDIF};
  if AColor=CornsilkColor then Result:='Cornsilk';//={$IFDEF_VCL}clWebCornsilk{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Cornsilk{$ENDIF};
  if AColor=CrimsonColor then Result:='Crimson';//={$IFDEF_VCL}clWebCrimson{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Crimson{$ENDIF};
  if AColor=CyanColor then Result:='Cyan';//={$IFDEF_VCL}clWebCyan{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Cyan{$ENDIF};
  if AColor=DarkblueColor then Result:='Darkblue';//={$IFDEF_VCL}clWebDarkblue{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Darkblue{$ENDIF};
  if AColor=DarkcyanColor then Result:='Darkcyan';//={$IFDEF_VCL}clWebDarkcyan{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Darkcyan{$ENDIF};
  if AColor=DarkgoldenrodColor then Result:='Darkgoldenrod';//={$IFDEF_VCL}clWebDarkgoldenrod{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Darkgoldenrod{$ENDIF};
  if AColor=DarkgrayColor then Result:='Darkgray';//={$IFDEF_VCL}clWebDarkgray{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Darkgray{$ENDIF};
  if AColor=DarkgreenColor then Result:='Darkgreen';//={$IFDEF_VCL}clWebDarkgreen{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Darkgreen{$ENDIF};
//  if AColor=AliceblueColor then Result:='DarkgreyColor={$IFDEF_VCL}clWebDarkgrey{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Darkgrey{$ENDIF};
  if AColor=DarkkhakiColor then Result:='Darkkhaki';//={$IFDEF_VCL}clWebDarkkhaki{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Darkkhaki{$ENDIF};
  if AColor=DarkmagentaColor then Result:='Darkmagenta';//={$IFDEF_VCL}clWebDarkmagenta{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Darkmagenta{$ENDIF};
  if AColor=DarkolivegreenColor then Result:='Darkolivegreen';//={$IFDEF_VCL}clWebDarkolivegreen{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Darkolivegreen{$ENDIF};
  if AColor=DarkorangeColor then Result:='Darkorange';//={$IFDEF_VCL}clWebDarkorange{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Darkorange{$ENDIF};
  if AColor=DarkorchidColor then Result:='Darkorchid';//={$IFDEF_VCL}clWebDarkorchid{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Darkorchid{$ENDIF};
  if AColor=DarkredColor then Result:='Darkred';//={$IFDEF_VCL}clWebDarkred{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Darkred{$ENDIF};
  if AColor=DarksalmonColor then Result:='Darksalmon';//{$IFDEF_VCL}clWebDarksalmon{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Darksalmon{$ENDIF};
  if AColor=DarkseagreenColor then Result:='Darkseagreen';//{$IFDEF_VCL}clWebDarkseagreen{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Darkseagreen{$ENDIF};
  if AColor=DarkslateblueColor then Result:='Darkslateblue';//{$IFDEF_VCL}clWebDarkslateblue{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Darkslateblue{$ENDIF};
  if AColor=DarkslategrayColor then Result:='Darkslategray';//{$IFDEF_VCL}clWebDarkslategray{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Darkslategray{$ENDIF};
//  if AColor=AliceblueColor then Result:='Darkslategrey';//{$IFDEF_VCL}clWebDarkslategrey{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Darkslategrey{$ENDIF};
  if AColor=DarkturquoiseColor then Result:='Darkturquoise';//{$IFDEF_VCL}clWebDarkturquoise{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Darkturquoise{$ENDIF};
  if AColor=DarkvioletColor then Result:='Darkviolet';//{$IFDEF_VCL}clWebDarkviolet{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Darkviolet{$ENDIF};
  if AColor=DeeppinkColor then Result:='Deeppink';//{$IFDEF_VCL}clWebDeeppink{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Deeppink{$ENDIF};
  if AColor=DeepskyblueColor then Result:='Deepskyblue';//{$IFDEF_VCL}clWebDeepskyblue{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Deepskyblue{$ENDIF};
  if AColor=DimgrayColor then Result:='Dimgray';//{$IFDEF_VCL}clWebDimgray{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Dimgray{$ENDIF};
//  if AColor=AliceblueColor then Result:='Dimgrey';//{$IFDEF_VCL}clWebDimgrey{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Dimgrey{$ENDIF};
  if AColor=DodgerblueColor then Result:='Dodgerblue';//{$IFDEF_VCL}clWebDodgerblue{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Dodgerblue{$ENDIF};
  if AColor=FirebrickColor then Result:='Firebrick';//{$IFDEF_VCL}clWebFirebrick{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Firebrick{$ENDIF};
  if AColor=FloralwhiteColor then Result:='Floralwhite';//{$IFDEF_VCL}clWebFloralwhite{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Floralwhite{$ENDIF};
  if AColor=ForestgreenColor then Result:='Forestgreen';//{$IFDEF_VCL}clWebForestgreen{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Forestgreen{$ENDIF};
  if AColor=FuchsiaColor then Result:='Fuchsia';//{$IFDEF_VCL}clWebFuchsia{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Fuchsia{$ENDIF};
  if AColor=GainsboroColor then Result:='Gainsboro';//{$IFDEF_VCL}clWebGainsboro{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Gainsboro{$ENDIF};
  if AColor=GhostwhiteColor then Result:='Ghostwhite';//{$IFDEF_VCL}clWebGhostwhite{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Ghostwhite{$ENDIF};
  if AColor=GoldColor then Result:='Gold';//{$IFDEF_VCL}clWebGold{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Gold{$ENDIF};
  if AColor=GoldenrodColor then Result:='Goldenrod';//{$IFDEF_VCL}clWebGoldenrod{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Goldenrod{$ENDIF};
  if AColor=GrayColor then Result:='Gray';//{$IFDEF_VCL}clGray{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Gray{$ENDIF};
  if AColor=GreenColor then Result:='Green';//{$IFDEF_VCL}clGreen{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Green{$ENDIF};
  if AColor=GreenyellowColor then Result:='Greenyellow';//{$IFDEF_VCL}clWebGreenyellow{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Greenyellow{$ENDIF};
//  if AColor=AliceblueColor then Result:='Grey';//{$IFDEF_VCL}clWebGrey{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Grey{$ENDIF};
  if AColor=HoneydewColor then Result:='Honeydew';//{$IFDEF_VCL}clWebHoneydew{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Honeydew{$ENDIF};
  if AColor=HotpinkColor then Result:='Hotpink';//{$IFDEF_VCL}clWebHotpink{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Hotpink{$ENDIF};
  if AColor=IndianredColor then Result:='Indianred';//{$IFDEF_VCL}clWebIndianred{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Indianred{$ENDIF};
  if AColor=IndigoColor then Result:='Indigo';//{$IFDEF_VCL}clWebIndigo{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Indigo{$ENDIF};
  if AColor=IvoryColor then Result:='Ivory';//{$IFDEF_VCL}clWebIvory{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Ivory{$ENDIF};
  if AColor=KhakiColor then Result:='Khaki';//{$IFDEF_VCL}clWebKhaki{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Khaki{$ENDIF};
  if AColor=LavenderColor then Result:='Lavender';//{$IFDEF_VCL}clWebLavender{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Lavender{$ENDIF};
  if AColor=LavenderblushColor then Result:='Lavenderblush';//{$IFDEF_VCL}clWebLavenderblush{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Lavenderblush{$ENDIF};
  if AColor=LawngreenColor then Result:='Lawngreen';//{$IFDEF_VCL}clWebLawngreen{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Lawngreen{$ENDIF};
  if AColor=LemonchiffonColor then Result:='Lemonchiffon';//{$IFDEF_VCL}clWebLemonchiffon{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Lemonchiffon{$ENDIF};
  if AColor=LightblueColor then Result:='Lightblue';//{$IFDEF_VCL}clWebLightblue{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Lightblue{$ENDIF};
  if AColor=LightcoralColor then Result:='Lightcoral';//{$IFDEF_VCL}clWebLightcoral{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Lightcoral{$ENDIF};
  if AColor=LightcyanColor then Result:='Lightcyan';//{$IFDEF_VCL}clWebLightcyan{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Lightcyan{$ENDIF};
  if AColor=LightgoldenrodyellowColor then Result:='Lightgoldenrodyellow';//{$IFDEF_VCL}clWebLightgoldenrodyellow{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Lightgoldenrodyellow{$ENDIF};
  if AColor=LightgrayColor then Result:='Lightgray';//{$IFDEF_VCL}clgray{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Lightgray{$ENDIF};
//  if AColor=AliceblueColor then Result:='Lightgray';//{$IFDEF_VCL}clWebLightgray{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Lightgray{$ENDIF};
  if AColor=LightgreenColor then Result:='Lightgreen';//{$IFDEF_VCL}clWebLightgreen{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Lightgreen{$ENDIF};
  if AColor=LightgreyColor then Result:='Lightgrey';//{$IFDEF_VCL}clWebLightgrey{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Lightgrey{$ENDIF};
  if AColor=LightpinkColor then Result:='Lightpink';//{$IFDEF_VCL}clWebLightpink{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Lightpink{$ENDIF};
  if AColor=LightsalmonColor then Result:='Lightsalmon';//{$IFDEF_VCL}clWebLightsalmon{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Lightsalmon{$ENDIF};
  if AColor=LightseagreenColor then Result:='Lightseagreen';//{$IFDEF_VCL}clWebLightseagreen{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Lightseagreen{$ENDIF};
  if AColor=LightskyblueColor then Result:='Lightskyblue';//{$IFDEF_VCL}clWebLightskyblue{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Lightskyblue{$ENDIF};
  if AColor=LightslategrayColor then Result:='Lightslategray';//{$IFDEF_VCL}clWebLightslategray{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Lightslategray{$ENDIF};
//  if AColor=AliceblueColor then Result:='Lightslategrey';//{$IFDEF_VCL}clWebLightslategrey{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Lightslategrey{$ENDIF};
  if AColor=LightsteelblueColor then Result:='Lightsteelblue';//{$IFDEF_VCL}clWebLightsteelblue{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Lightsteelblue{$ENDIF};
  if AColor=LightyellowColor then Result:='Lightyellow';//{$IFDEF_VCL}clWebLightyellow{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Lightyellow{$ENDIF};
  if AColor=LtGrayColor then Result:='LtGray';//{$IFDEF_VCL}clLtGray{$ENDIF}{$IFDEF FMX}TAlphaColorRec.LtGray{$ENDIF};
  if AColor=MedGrayColor then Result:='MedGray';//{$IFDEF_VCL}clMedGray{$ENDIF}{$IFDEF FMX}TAlphaColorRec.MedGray{$ENDIF};
  if AColor=DkGrayColor then Result:='DkGray';//{$IFDEF_VCL}clDkGray{$ENDIF}{$IFDEF FMX}TAlphaColorRec.DkGray{$ENDIF};
  if AColor=MoneyGreenColor then Result:='MoneyGreen';//{$IFDEF_VCL}clMoneyGreen{$ENDIF}{$IFDEF FMX}TAlphaColorRec.MoneyGreen{$ENDIF};
//  if AColor=AliceblueColor then Result:='LegacySkyBlue';//{$IFDEF_VCL}clWebLegacySkyBlue{$ENDIF}{$IFDEF FMX}TAlphaColorRec.LegacySkyBlue{$ENDIF};
  if AColor=CreamColor then Result:='Cream';//{$IFDEF_VCL}clCream{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Cream{$ENDIF};
  if AColor=LimeColor then Result:='Lime';//{$IFDEF_VCL}clLime{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Lime{$ENDIF};
  if AColor=LimegreenColor then Result:='Limegreen';//{$IFDEF_VCL}clWebLimegreen{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Limegreen{$ENDIF};
  if AColor=LinenColor then Result:='Linen';//{$IFDEF_VCL}clWebLinen{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Linen{$ENDIF};
  if AColor=MagentaColor then Result:='Magenta';//{$IFDEF_VCL}clWebMagenta{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Magenta{$ENDIF};
  if AColor=MaroonColor then Result:='Maroon';//{$IFDEF_VCL}clMaroon{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Maroon{$ENDIF};
  if AColor=MediumaquamarineColor then Result:='Mediumaquamarine';//{$IFDEF_VCL}clWebMediumaquamarine{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Mediumaquamarine{$ENDIF};
  if AColor=MediumblueColor then Result:='Mediumblue';//{$IFDEF_VCL}clWebMediumblue{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Mediumblue{$ENDIF};
  if AColor=MediumorchidColor then Result:='Mediumorchid';//{$IFDEF_VCL}clWebMediumorchid{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Mediumorchid{$ENDIF};
  if AColor=MediumpurpleColor then Result:='Mediumpurple';//{$IFDEF_VCL}clWebMediumpurple{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Mediumpurple{$ENDIF};
  if AColor=MediumseagreenColor then Result:='Mediumseagreen';//{$IFDEF_VCL}clWebMediumseagreen{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Mediumseagreen{$ENDIF};
  if AColor=MediumslateblueColor then Result:='Mediumslateblue';//{$IFDEF_VCL}clWebMediumslateblue{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Mediumslateblue{$ENDIF};
  if AColor=MediumspringgreenColor then Result:='Mediumspringgreen';//{$IFDEF_VCL}clWebMediumspringgreen{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Mediumspringgreen{$ENDIF};
  if AColor=MediumturquoiseColor then Result:='Mediumturquoise';//{$IFDEF_VCL}clWebMediumturquoise{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Mediumturquoise{$ENDIF};
  if AColor=MediumvioletredColor then Result:='Mediumvioletred';//{$IFDEF_VCL}clWebMediumvioletred{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Mediumvioletred{$ENDIF};
  if AColor=MidnightblueColor then Result:='Midnightblue';//{$IFDEF_VCL}clWebMidnightblue{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Midnightblue{$ENDIF};
  if AColor=MintcreamColor then Result:='Mintcream';//{$IFDEF_VCL}clWebMintcream{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Mintcream{$ENDIF};
  if AColor=MistyroseColor then Result:='Mistyrose';//{$IFDEF_VCL}clWebMistyrose{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Mistyrose{$ENDIF};
  if AColor=MoccasinColor then Result:='Moccasin';//{$IFDEF_VCL}clWebMoccasin{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Moccasin{$ENDIF};
  if AColor=NavajowhiteColor then Result:='Navajowhite';//{$IFDEF_VCL}clWebNavajowhite{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Navajowhite{$ENDIF};
  if AColor=NavyColor then Result:='Navy';//{$IFDEF_VCL}clNavy{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Navy{$ENDIF};
  if AColor=OldlaceColor then Result:='Oldlace';//{$IFDEF_VCL}clWebOldlace{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Oldlace{$ENDIF};
  if AColor=OliveColor then Result:='Olive';//{$IFDEF_VCL}clOlive{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Olive{$ENDIF};
  if AColor=OlivedrabColor then Result:='Olivedrab';//{$IFDEF_VCL}clWebOlivedrab{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Olivedrab{$ENDIF};
  if AColor=OrangeColor then Result:='Orange';//{$IFDEF_VCL}clWebOrange{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Orange{$ENDIF};
  if AColor=OrangeredColor then Result:='Orangered';//{$IFDEF_VCL}clWebOrangered{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Orangered{$ENDIF};
  if AColor=OrchidColor then Result:='Orchid';//{$IFDEF_VCL}clWebOrchid{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Orchid{$ENDIF};
  if AColor=PalegoldenrodColor then Result:='Palegoldenrod';//{$IFDEF_VCL}clWebPalegoldenrod{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Palegoldenrod{$ENDIF};
  if AColor=PalegreenColor then Result:='Palegreen';//{$IFDEF_VCL}clWebPalegreen{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Palegreen{$ENDIF};
  if AColor=PaleturquoiseColor then Result:='Paleturquoise';//{$IFDEF_VCL}clWebPaleturquoise{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Paleturquoise{$ENDIF};
  if AColor=PalevioletredColor then Result:='Palevioletred';//{$IFDEF_VCL}clWebPalevioletred{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Palevioletred{$ENDIF};
  if AColor=PapayawhipColor then Result:='Papayawhip';//{$IFDEF_VCL}clWebPapayawhip{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Papayawhip{$ENDIF};
  if AColor=PeachpuffColor then Result:='Peachpuff';//{$IFDEF_VCL}clWebPeachpuff{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Peachpuff{$ENDIF};
  if AColor=PeruColor then Result:='Peru';//{$IFDEF_VCL}clWebPeru{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Peru{$ENDIF};
  if AColor=PinkColor then Result:='Pink';//{$IFDEF_VCL}clWebPink{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Pink{$ENDIF};
  if AColor=PlumColor then Result:='Plum';//{$IFDEF_VCL}clWebPlum{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Plum{$ENDIF};
  if AColor=PowderblueColor then Result:='Powderblue';//{$IFDEF_VCL}clWebPowderblue{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Powderblue{$ENDIF};
  if AColor=PurpleColor then Result:='Purple';//{$IFDEF_VCL}clPurple{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Purple{$ENDIF};
  if AColor=RedColor then Result:='Red';//{$IFDEF_VCL}clRed{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Red{$ENDIF};
  if AColor=RosybrownColor then Result:='Rosybrown';//{$IFDEF_VCL}clWebRosybrown{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Rosybrown{$ENDIF};
  if AColor=RoyalblueColor then Result:='Royalblue';//{$IFDEF_VCL}clWebRoyalblue{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Royalblue{$ENDIF};
  if AColor=SaddlebrownColor then Result:='Saddlebrown';//{$IFDEF_VCL}clWebSaddlebrown{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Saddlebrown{$ENDIF};
  if AColor=SalmonColor then Result:='Salmon';//{$IFDEF_VCL}clWebSalmon{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Salmon{$ENDIF};
  if AColor=SandybrownColor then Result:='Sandybrown';//{$IFDEF_VCL}clWebSandybrown{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Sandybrown{$ENDIF};
  if AColor=SeagreenColor then Result:='Seagreen';//{$IFDEF_VCL}clWebSeagreen{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Seagreen{$ENDIF};
  if AColor=SeashellColor then Result:='Seashell';//{$IFDEF_VCL}clWebSeashell{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Seashell{$ENDIF};
  if AColor=SiennaColor then Result:='Sienna';//{$IFDEF_VCL}clWebSienna{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Sienna{$ENDIF};
  if AColor=SilverColor then Result:='Silver';//{$IFDEF_VCL}clSilver{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Silver{$ENDIF};
  if AColor=SkyblueColor then Result:='Skyblue';//{$IFDEF_VCL}clSkyblue{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Skyblue{$ENDIF};
  if AColor=SlateblueColor then Result:='Slateblue';//{$IFDEF_VCL}clWebSlateblue{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Slateblue{$ENDIF};
  if AColor=SlategrayColor then Result:='Slategray';//{$IFDEF_VCL}clWebSlategray{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Slategray{$ENDIF};
//  if AColor=AliceblueColor then Result:='Slategrey';//{$IFDEF_VCL}clWebSlategrey{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Slategrey{$ENDIF};
  if AColor=SnowColor then Result:='Snow';//{$IFDEF_VCL}clWebSnow{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Snow{$ENDIF};
  if AColor=SpringgreenColor then Result:='Springgreen';//{$IFDEF_VCL}clWebSpringgreen{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Springgreen{$ENDIF};
  if AColor=SteelblueColor then Result:='Steelblue';//{$IFDEF_VCL}clWebSteelblue{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Steelblue{$ENDIF};
  if AColor=TanColor then Result:='Tan';//{$IFDEF_VCL}clWebTan{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Tan{$ENDIF};
  if AColor=TealColor then Result:='Teal';//{$IFDEF_VCL}clTeal{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Teal{$ENDIF};
  if AColor=ThistleColor then Result:='Thistle';//{$IFDEF_VCL}clWebThistle{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Thistle{$ENDIF};
  if AColor=TomatoColor then Result:='Tomato';//{$IFDEF_VCL}clWebTomato{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Tomato{$ENDIF};
  if AColor=TurquoiseColor then Result:='Turquoise';//{$IFDEF_VCL}clWebTurquoise{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Turquoise{$ENDIF};
  if AColor=VioletColor then Result:='Violet';//{$IFDEF_VCL}clWebViolet{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Violet{$ENDIF};
  if AColor=WheatColor then Result:='Wheat';//{$IFDEF_VCL}clWebWheat{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Wheat{$ENDIF};
  if AColor=WhiteColor then Result:='White';//{$IFDEF_VCL}clWhite{$ENDIF}{$IFDEF FMX}TAlphaColorRec.White{$ENDIF};
  if AColor=WhitesmokeColor then Result:='Whitesmoke';//{$IFDEF_VCL}clWebWhitesmoke{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Whitesmoke{$ENDIF};
  if AColor=YellowColor then Result:='Yellow';//{$IFDEF_VCL}clYellow{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Yellow{$ENDIF};
  if AColor=YellowgreenColor then Result:='Yellowgreen';//{$IFDEF_VCL}clWebYellowgreen{$ENDIF}{$IFDEF FMX}TAlphaColorRec.Yellowgreen{$ENDIF};
...
function StrToColor(AColorName:String):TDelphiColor;
begin
  Result:=ColorNameToColor(AColorName);
end;
...
begin
  {$IFDEF FMX}
  //在FMX下面Color是TAlphaColorRec类型有透明度,不需要保存Alpha
  Result:=False;
  {$ENDIF}
  {$IFDEF VCL}
  Result:=(FAlpha<>255);
...
function TDrawColor.IsNeedStored: Boolean;
begin
  Result:=IsAlphaStored or IsColorStored or IsAlphaStored;
end;
...
procedure TDrawColor.LoadFromJson(AJson: ISuperObject);
begin
//    //是否使用主题色
//    property UseThemeColor:TUseThemeColorType read FUseThemeColor write SetUseThemeColor stored IsUseThemeColorStored;
//    //透明度
...
//    //颜色
//    property Color:TDelphiColor read GetColor write SetColor stored IsColorStored;
  if AJson.Contains('UseThemeColor') then
  begin
    FUseThemeColor:=GetUseThemeColorTypeByStr(AJson.S['UseThemeColor']);
  end;
  if AJson.Contains('Color') then
  begin
    FColor:=StrToColor(AJson.S['Color']);
  end;
  if AJson.Contains('Alpha') then
  begin
    AJson.I['Alpha']:=FAlpha;
  end;

end;

procedure TDrawColor.SaveToJson(AJson: ISuperObject);
begin
  if IsUseThemeColorStored then
  begin
    AJson.S['UseThemeColor']:=GetUseThemeColorTypeStr(FUseThemeColor);
  end;
  if IsColorStored then
  begin
    AJson.S['Color']:=ColorToStr(FColor);
  end;
  if IsAlphaStored then
  begin
    FAlpha:=AJson.I['Alpha'];
  end;
...
begin
  ...
  //  begin
      FColor:=Value;
      FAlpha:=TAlphaColorRec(FColor).A;
  //  end;
    {$ENDIF}
...
procedure TDrawColor.SetStaticAlpha(const Value: Byte);
begin
  {$IFDEF VCL}
  FAlpha:=Value;
  {$ENDIF}
...
function TDrawColorList.GetItem(Index: Integer): TDrawColor;
begin
  Result:=TDrawColor(Inherited Items[Index]);
end;

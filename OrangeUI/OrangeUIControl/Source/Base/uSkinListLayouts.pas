...
//  if (FSkinListIntf.GetOwnerProperties<>nil) and (csLoading in TSkinControlProperties(FSkinListIntf.GetOwnerProperties).SkinControl.ComponentState) then
//  begin
//    Exit;
//  end;
...
begin
  ...
  if (Self.FMouseOverItem=AItem) or (AItem.GetIsMouseOver) then
  begin
...
begin
  ...
  begin
      ...
      {$ENDIF}
      {$IFDEF VCL}
      Result:=ScreenScaleSize(AItem.Height);
      {$ENDIF}
  end;
...
begin
  ...
  begin
      ...
      {$IFDEF VCL}
      //要放大
      Result:=ScreenScaleSize(AItem.Width);
      {$ENDIF}
  end;

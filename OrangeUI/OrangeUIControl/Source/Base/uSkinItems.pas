...
const
  ...
  IID_IDataSource:TGUID='{D6D2A233-8F73-4084-8679-25BC09A06FC3}';
...
type
  ...
  IDataSource=interface
    ['{D6D2A233-8F73-4084-8679-25BC09A06FC3}']
    //获取字段列表,绑定的时候需要供用户选择
    function GetFieldDefList(ABindItemFieldName:String):TFieldDefs;
    //根据绑定的FieldName获取Item的值,然后赋给绑定的控件
    function GetValueTypeByBindItemField(AFieldName:String):TVarType;
    function GetValueByBindItemField(AFieldName:String):Variant;
    function GetObjectByBindItemField(AFieldName:String):TObject;
  end;


//  //数据绑定对象接口
//  IBindDataObject=interface
//    ['{95231C10-F89A-4800-9A09-B3AF85FF7880}']
//    //根据绑定的FieldName获取Item的值,然后赋给绑定的控件
//    function GetValueTypeByBindItemField(AFieldName:String):TVarType;
//    function GetValueByBindItemField(AFieldName:String):Variant;
//    function GetObjectByBindItemField(AFieldName:String):TObject;
//    procedure SetValueByBindItemField(AFieldName:String;AValue:Variant);
//
//  end;
//
//  IBindDataObjectList=interface
//    ['{11EFEC66-202F-4F02-A3F0-E44FECB60E54}']
//    function GetFieldNameList:TStringList;
//    function GetValueByBindItemField(AObjectIndex:Integer;AFieldName:String):Variant;
//    function GetObjectByBindItemField(AObjectIndex:Integer;AFieldName:String):TObject;
//    procedure SetValueByBindItemField(AObjectIndex:Integer;AFieldName:String;AValue:Variant);
//  end;
  ...
  TBaseSkinItem=class(TBinaryObject,ISkinItem, IDataSource)
  protected
    //是否选中
    FSelected:Boolean;
...
//    function GetListLayoutsManager:TSkinListLayoutsManager;virtual;
  protected
    ...
    procedure LoadFromJson(AJson:ISuperObject);virtual;
    procedure SaveToJson(AJson:ISuperObject);virtual;
  public
    //IDataSource
    function GetFieldDefList(ABindItemFieldName:String):TFieldDefs;
    //根据绑定的FieldName获取Item的值,然后赋给绑定的控件
    ...
    function GetValueArrayByBindItemField(AFieldName:String):ISuperArray;virtual;abstract;
...
//    constructor Create(AOwner: TBaseSkinItems);overload;virtual;
  ...
  private
    ...
    function GetOwner: TBaseSkinItems;
//  protected
//    //在列表中的顺序
//    function GetIndex:Integer;
  public
    ...
    //获取选中的
    procedure GetSelectedChlids(ASelectedList:TList);virtual;
    /// <summary>
    ///   <para>
    ///     在列表中的下标
    ///   </para>
...
//    property Index:Integer read GetIndex;
    ...
    ///   </para>
    /// </summary>
    property Owner:TBaseSkinItems read GetOwner write SetOwner;
...
//  TBaseSkinItems=class(TBinaryObjectList,ISkinList,ISkinItems)
  TBaseSkinItems=class(TBinaryCollection,ISkinList,ISkinItems)
  protected
    { IInterface }
//    function QueryInterface({$IFDEF FPC}constref{$ELSE}const{$ENDIF} IID: TGUID; out Obj): {$IFDEF LINUX}longint{$ELSE}HResult{$ENDIF}; virtual; {$IFDEF LINUX}stdcall{$ELSE}stdcall{$ENDIF};
...
//    procedure InitSkinItemClass;virtual;
    function GetDefaultSkinItemClass:TBaseSkinItemClass;virtual;
  protected
    //ISkinItems接口,用于设计时编辑
    function GetItems:TBaseSkinItems;
  public
    SkinItemClass:TBaseSkinItemClass;
    constructor Create(const AObjectOwnership:TObjectOwnership=ooOwned;
...
//    function GetOwnerProperties:TObject;
  ...
  public
    ...
    procedure DoDelete(AObject:TObject);override;
    procedure DoInsert(AObject:TObject);override;
  ...
  public
    ...
    procedure LoadFromJsonArray(ASuperArray:ISuperArray);
    procedure SaveToJsonArray(ASuperArray:ISuperArray);
...
//    procedure ClearData(AItemDataType:TSkinItemDataType);virtual;
  ...
  published
    ...
    property ItemStyle:String read FItemStyle write SetItemStyle;
  public
    //抽象属性
    /// <summary>
    ///   <para>
...
//    procedure ClearData(AFieldName:String);override;
  ...
  TRealSkinItems=class(TSkinItems)
  private
    function GetItem(Index: Integer): TRealSkinItem;
    procedure SetItem(Index: Integer; const Value: TRealSkinItem);
  protected
//    function CreateBinaryObject(const AClassName:String=''):TInterfacedPersistent;override;
//    procedure InitSkinItemClass;override;
    function GetDefaultSkinItemClass:TBaseSkinItemClass;override;
  public
    function Add:TRealSkinItem;overload;
    function Insert(Index:Integer):TRealSkinItem;overload;
    property Items[Index:Integer]:TRealSkinItem read GetItem write SetItem;default;
  end;
...
//  TSkinListViewItems=TRealSkinItems;
...
//    procedure AfterConstruction; override;
  ...
  public
    //获取选中的
    procedure GetSelectedChlids(ASelectedList:TList);override;
    //获取所有子节点是否都已经选中
    function GetAllChildSelected:Boolean;
    procedure SetAllChildSelected(ASelected:Boolean);
    procedure SetSelectedByAllChildSelected;
    //用于设置器中编辑
    function GetItems:TBaseSkinItems;
...
//    procedure CollapseAll(Level:Integer=-1);
  public
    //搜索,支持搜索到子节点
    function FindItemByData(const AData:Pointer):TRealSkinTreeViewItem;overload;
    function FindItemByCaption(const ACaption:String):TRealSkinTreeViewItem;overload;
    function FindItemByName(const AName:String):TRealSkinTreeViewItem;overload;
    function FindItemByDetail(const ADetail:String):TRealSkinTreeViewItem;overload;
    function FindItemByDetail1(const ADetail1:String):TRealSkinTreeViewItem;overload;
  end;
...
function TSkinItems.GetDefaultSkinItemClass: TBaseSkinItemClass;
begin
  Result:=TSkinItem;
end;
...
function TBaseSkinItems._AddRef: Integer;
begin
...
function TBaseSkinItems._Release: Integer;
begin
//  if FOwnerInterface <> nil then
...
function TBaseSkinItems.QueryInterface({$IFDEF FPC}constref{$ELSE}const{$ENDIF} IID: TGUID;
...
//  Result:=Items[Index] as ISkinItem;
  { TODO : 这里要处理1 }
  Items[Index].QueryInterface(IID_ISkinItem,Result);
end;

function TBaseSkinItems.GetDefaultSkinItemClass: TBaseSkinItemClass;
begin
...
//function TBaseSkinItems.GetOwnerProperties: TObject;
//begin
//  Result:=FOwnerProperties;
//end;
...
procedure TBaseSkinItems.LoadFromJsonArray(ASuperArray: ISuperArray);
var
  I: Integer;
...
begin
  ...
  try
    ...
    begin
      ASkinItem:=TBaseSkinItem(CreateBinaryObject);
      Add(ASkinItem);
      ASkinItem.LoadFromJson(ASuperArray.O[I]);
    end;
  finally
    Self.EndUpdate;
  end;
...
procedure TBaseSkinItems.SaveToJsonArray(ASuperArray: ISuperArray);
var
  I: Integer;
begin
  for I := 0 to Count-1 do
  begin
    Items[I].SaveToJson(ASuperArray.O[I]);
  end;
end;
...
begin
  SkinItemClass:=GetDefaultSkinItemClass;
//  FOwnerProperties:=AOwnerProperties;

//  inherited Create(AObjectOwnership,AIsCreateObjectChangeManager);
  inherited Create(SkinItemClass);

end;

destructor TBaseSkinItems.Destroy;
begin
  
  //去除引用
  if (FListLayoutsManager<>nil) then
  begin
    FListLayoutsManager.SetSkinListIntf(nil);
  end;

  inherited Destroy;
end;

function TBaseSkinItems.CreateBinaryObject(const AClassName:String=''): TObject;//TInterfacedPersistent;
var
...
begin
  ...
  begin
    ...
    begin
      Result:=ASkinItemClass.Create(Self);
    end
    else
    begin
      raise Exception.Create('Can not find item class:'+AClassName);
    end;
  end
  else
  begin
    if SkinItemClass<>nil then
    begin
       Result:=SkinItemClass.Create(Self);
    end
    else
    begin
      raise Exception.Create('SkinItemClass is nil');
    end;
  end;
end;
...
procedure TBaseSkinItems.DoInsert(AObject: TObject);
begin
  inherited;
...
procedure TBaseSkinItems.DoDelete(AObject: TObject);
begin
...
//  end;
  ...
  //if (FOwnerProperties<>nil)
  //  and not (csDesigning in TSkinControlProperties(Self.FOwnerProperties).SkinControl.ComponentState) then
  //begin
  ...
  begin
    ...
    if GetListLayoutsManager<>nil then
    begin
      //VisibleChange应该放前面，因为VisibleItems的话，需要先计算出来，才能一一进行ItemSizeChange，不然Item已经释放但是还在VisibleItems的情况下进行SizeChage的操作会报错的
      Self.GetListLayoutsManager.DoItemVisibleChange(nil,not AIsDoItemVisibleChange);
      //Self.GetListLayoutsManager.DoItemSizeChange(nil);
...
begin
  ...
  {$ELSE}
  //低版本Delphi VCL,String没有Substring方法
  //ItemSubItems
  ASubItemsIndexStr:=Copy(AFieldName,13,MaxInt);
  {$ENDIF}

  if {$IFDEF DELPHIXE8}ASubItemsIndexStr.Chars[0]{$ELSE}ASubItemsIndexStr[1]{$ENDIF}<>'_' then
  begin
      //下标
...
begin
  ...
  else if {$IFDEF DELPHIXE8}AFieldName.Substring(0,12){$ELSE}Copy(AFieldName,1,12){$ENDIF}='ItemSubItems' then
  begin
...
begin
      ...
      if {$IFDEF DELPHIXE8}ASubItemsIndexStr.Chars[0]{$ELSE}AFieldName[1]{$ENDIF}<>'_' then
      begin
          //是下标
...
constructor TBaseSkinItem.Create(ACollection:TCollection);
begin
...
begin
  ...
  if (Owner<>nil) and (Owner.GetListLayoutsManager<>nil) then
  begin
    Owner.GetListLayoutsManager.DoItemPropChange(Self);
  end;
end;
...
begin
  ...
  if (Owner<>nil) and (Owner.GetListLayoutsManager<>nil) then
  begin
    Owner.GetListLayoutsManager.DoItemSizeChange(Self);
  end;
end;
...
begin
  if (Owner<>nil) and (Owner.GetListLayoutsManager<>nil) then
  begin
    Owner.GetListLayoutsManager.DoItemVisibleChange(Self);
  end;
end;

function TBaseSkinItem.GetFieldDefList(ABindItemFieldName:String): TFieldDefs;
begin
  Result:=nil;
end;
...
procedure TBaseSkinItem.GetSelectedChlids(ASelectedList:TList);
begin
  if FSelected then
  begin
    ASelectedList.Add(Self);
  end;
end;
...
function TBaseSkinItem.GetOwner: TBaseSkinItems;
begin
  Result:=TBaseSkinItems(Collection);
end;
...
procedure TBaseSkinItem.LoadFromJson(AJson: ISuperObject);
begin
  if AJson.Contains('Height') then FHeight:=AJson.F['Height'];
  if AJson.Contains('Width') then FWidth:=AJson.F['Width'];
  if AJson.Contains('Selected') then FSelected:=AJson.B['Selected'];
...
procedure TBaseSkinItem.SaveToJson(AJson: ISuperObject);
begin
  if FHeight<>-1 then AJson.F['Height']:=FHeight;
  if FWidth<>-1 then AJson.F['Width']:=FWidth;
  if FSelected then AJson.B['Selected']:=FSelected;
  if not Visible then AJson.B['Visible']:=FVisible;
...
begin
  ...
  begin
      ...
      begin
        //取消选中
        Owner.GetListLayoutsManager.DoItemSelected(nil);
      end;
  end;
  ...
  begin
      ...
      begin
        //选中
        Owner.GetListLayoutsManager.DoItemSelected(Self);
      end;
  end;
...
begin
  ...
  begin
    ...
    if FSelected and (Owner<>nil) then
    begin
        if (Owner.GetListLayoutsManager<>nil) then
        begin
          //选中
          Owner.GetListLayoutsManager.DoItemSelected(Self);
        end;
...
function TBaseSkinItem.GetIsMouseOver: Boolean;
begin
  Result:=FIsMouseOver;
end;
...
constructor TSkinItem.Create(ACollection:TCollection=nil);
begin
  FColor:=TDrawColor.Create('Color','颜色');
  FColor.FAlpha:=255;
  FColor.FColor:=0;
...
begin
  if (Owner<>nil) and (Owner.GetListLayoutsManager<>nil) then
  begin
    Owner.GetListLayoutsManager.DoGetItemIconDefaultSkinImageList(Sender,ASkinImageList);
  end;
end;
...
begin
  if (Owner<>nil) and (Owner.GetListLayoutsManager<>nil) then
  begin
    Owner.GetListLayoutsManager.DoGetItemIconDefaultDownloadPictureManager(Sender,ADownloadPictureManager);
  end;
end;
...
begin
  ...
  begin
    ...
    else if ABTNode.NodeName='ItemStyle' then
    begin
      FItemStyle:=ABTNode.ConvertNode_WideString.Data;
    end
    ...
    else if ABTNode.NodeName='AutoSizeWidth' then
    begin
      FAutoSizeWidth:=ABTNode.ConvertNode_Bool32.Data;
    end
    else if ABTNode.NodeName='AutoSizeHeight' then
    begin
      FAutoSizeHeight:=ABTNode.ConvertNode_Bool32.Data;
    end
...
procedure TSkinItem.SetItemStyle(const Value: String);
begin
...
begin
  ...
  ABTNode:=ADocNode.AddChildNode_WideString('ItemStyle');//,'名称');
  ABTNode.ConvertNode_WideString.Data:=FItemStyle;
  ...
  ABTNode:=ADocNode.AddChildNode_Bool32('AutoSizeWidth');//,'自动尺寸');
  ABTNode.ConvertNode_Bool32.Data:=FAutoSizeWidth;

  ABTNode:=ADocNode.AddChildNode_Bool32('AutoSizeHeight');//,'自动尺寸');
  ABTNode.ConvertNode_Bool32.Data:=FAutoSizeHeight;

  Result:=True;

end;

procedure TSkinItem.SaveToJson(AJson: ISuperObject);
begin
  inherited;


  if FName<>'' then AJson.S['Name']:=FName;
  if FTag<>0 then AJson.I['Tag']:=FTag;
  if FTag1<>0 then AJson.I['Tag1']:=FTag1;
  if FChecked then AJson.B['Checked']:=FChecked;
//  AJson.S['ItemType']:=FName;
  if FItemStyle<>'' then AJson.S['ItemStyle']:=FItemStyle;
  if (FIcon<>nil) and FIcon.IsNeedStored then Icon.SaveToJson(AJson.O['Icon']);
  if (FPic<>nil) and FIcon.IsNeedStored then Pic.SaveToJson(AJson.O['Pic']);
  if Color<>DefaultColor then AJson.S['Color']:=ColorToStr(Color);
  if AutoSizeWidth then AJson.B['AutoSizeWidth']:=FAutoSizeWidth;
  if AutoSizeHeight then AJson.B['AutoSizeHeight']:=FAutoSizeHeight;

  if Caption<>'' then AJson.S['Caption']:=Caption;
  if Detail<>'' then AJson.S['Detail']:=Detail;
  if Detail1<>'' then AJson.S['Detail1']:=Detail1;
  if Detail2<>'' then AJson.S['Detail2']:=Detail2;
  if Detail3<>'' then AJson.S['Detail3']:=Detail3;
  if Detail4<>'' then AJson.S['Detail4']:=Detail4;
  if Detail5<>'' then AJson.S['Detail5']:=Detail5;
  if Detail6<>'' then AJson.S['Detail6']:=Detail6;
end;
...
//  begin
//    Self.Icon.RefDrawPicture:=GetGlobalBindDataSourceManager.GetDrawPicture(ASetting.icon_image_name);
//  end;
...
procedure TSkinItem.LoadFromJson(AJson: ISuperObject);
begin
  inherited;
  if AJson.Contains('Name') then FName:=AJson.S['Name'];
  if AJson.Contains('Tag') then FTag:=AJson.I['Tag'];
  if AJson.Contains('Tag1') then FTag1:=AJson.I['Tag1'];
  if AJson.Contains('Checked') then FChecked:=AJson.B['Checked'];
//  AJson.S['ItemType']:=FName;
  if AJson.Contains('ItemStyle') then FItemStyle:=AJson.S['ItemStyle'];
  if AJson.Contains('Icon') then Icon.LoadFromJson(AJson.O['Icon']);
  if AJson.Contains('Pic') then Pic.LoadFromJson(AJson.O['Pic']);
  if AJson.Contains('Color') then AJson.S['Color']:=ColorToStr(Color);
  if AJson.Contains('AutoSizeWidth') then FAutoSizeWidth:=AJson.B['AutoSizeWidth'];
  if AJson.Contains('AutoSizeHeight') then FAutoSizeHeight:=AJson.B['AutoSizeHeight'];

  if AJson.Contains('Caption') then Caption:=AJson.S['Caption'];
  if AJson.Contains('Detail') then Detail:=AJson.S['Detail'];
  if AJson.Contains('Detail1') then Detail1:=AJson.S['Detail1'];
  if AJson.Contains('Detail2') then Detail2:=AJson.S['Detail2'];
  if AJson.Contains('Detail3') then Detail3:=AJson.S['Detail3'];
  if AJson.Contains('Detail4') then Detail4:=AJson.S['Detail4'];
  if AJson.Contains('Detail5') then Detail5:=AJson.S['Detail5'];
  if AJson.Contains('Detail6') then Detail6:=AJson.S['Detail6'];
...
begin
  ...
  begin
    //uSkinImageType.
    SetDrawPictureValue(Icon,APageDataDir,AImageServerUrl,AValue)
  end
  else if AFieldName='ItemPic' then
  begin
    //uSkinImageType.
    SetDrawPictureValue(Pic,APageDataDir,AImageServerUrl,AValue)
  end
...
procedure TSkinItem.SetAutoSizeHeight(const Value: Boolean);
begin
...
procedure TSkinItem.SetAutoSizeWidth(const Value: Boolean);
begin
...
//  end
  ...
  //FDataJson
  else if (FDataJson<>nil) and FDataJson.Contains(AFieldName) then
  begin
    Result:=FDataJson.V[AFieldName];
  end
  else
  begin
    Result:=Inherited GetValueByBindItemField(AFieldName);
...
begin
  Result:=varVariant;
  if (AFieldName='ItemIcon')
    or (AFieldName='ItemPic')
    or (AFieldName='ItemColor')
    then
  begin
    Result:=varObject;
  end
...
//    Result:=FColor.Color;
//  end
//  else if (SubItems<>nil) and ({$IFDEF DELPHIXE8}AFieldName.Substring(0,12){$ELSE}Copy(AFieldName,1,12){$ENDIF}='ItemSubItems') then
//  begin
//      Result:=GetValueByBindItemSubItems(SubItems,AFieldName);
//
////    ASubItemsIndex:=-1;
...
function TRealSkinTreeViewItems.FindItemByCaption(const ACaption: String): TRealSkinTreeViewItem;
var
  I: Integer;
begin
  Result:=TRealSkinTreeViewItem(TSkinItems(Self).FindItemByCaption(ACaption));
  if Result=nil then
  begin
    for I := 0 to Self.Count-1 do
    begin
      Result:=TRealSkinTreeViewItem(TRealSkinTreeViewItems(Items[I].FChilds).FindItemByCaption(ACaption));
      if Result<>nil then Exit;
    end;
  end;
end;

function TRealSkinTreeViewItems.FindItemByName(const AName: String): TRealSkinTreeViewItem;
var
  I: Integer;
begin
  Result:=TRealSkinTreeViewItem(TSkinItems(Self).FindItemByName(AName));
  if Result=nil then
  begin
    for I := 0 to Self.Count-1 do
    begin
      Result:=TRealSkinTreeViewItem(TRealSkinTreeViewItems(Items[I].FChilds).FindItemByName(AName));
      if Result<>nil then Exit;
    end;
  end;
end;

function TRealSkinTreeViewItems.FindItemByDetail(const ADetail: String): TRealSkinTreeViewItem;
var
  I: Integer;
begin
  Result:=TRealSkinTreeViewItem(TSkinItems(Self).FindItemByDetail(ADetail));
  if Result=nil then
  begin
    for I := 0 to Self.Count-1 do
    begin
      Result:=TRealSkinTreeViewItem(TRealSkinTreeViewItems(Items[I].FChilds).FindItemByDetail(ADetail));
      if Result<>nil then Exit;
    end;
  end;
end;

function TRealSkinTreeViewItems.FindItemByDetail1(const ADetail1: String): TRealSkinTreeViewItem;
var
  I: Integer;
begin
  Result:=TRealSkinTreeViewItem(TSkinItems(Self).FindItemByDetail1(ADetail1));
  if Result=nil then
  begin
    for I := 0 to Self.Count-1 do
    begin
      Result:=TRealSkinTreeViewItem(TRealSkinTreeViewItems(Items[I].FChilds).FindItemByDetail1(ADetail1));
      if Result<>nil then Exit;
    end;
  end;
end;

function TRealSkinTreeViewItems.FindItemByData(const AData: Pointer): TRealSkinTreeViewItem;
var
  I: Integer;
begin
  Result:=TRealSkinTreeViewItem(TSkinItems(Self).FindItemByData(AData));
  if Result=nil then
  begin
    for I := 0 to Self.Count-1 do
    begin
      Result:=TRealSkinTreeViewItem(TRealSkinTreeViewItems(Items[I].FChilds).FindItemByData(AData));
      if Result<>nil then Exit;
    end;
  end;
end;
...
function TRealSkinTreeViewItems.GetDefaultSkinItemClass: TBaseSkinItemClass;
begin
...
constructor TBaseSkinTreeViewItem.Create(ACollection:TCollection=nil);
begin
  inherited;
...
procedure TBaseSkinTreeViewItem.GetSelectedChlids(ASelectedList:TList);
var
  I: Integer;
begin
  inherited;
  ...
  begin
    FChilds[I].GetSelectedChlids(ASelectedList);
  end;
end;

function TBaseSkinTreeViewItem.GetAllChildSelected: Boolean;
var
  I: Integer;
begin
  ...
  for I := 0 to Self.Childs.Count-1 do
  begin
    Result:=Result and Self.Childs[I].GetAllChildSelected;
  end;
end;
...
procedure TBaseSkinTreeViewItem.SetAllChildSelected(ASelected: Boolean);
var
  I: Integer;
begin
  ...
  for I := 0 to FChilds.Count-1 do
  begin
    FChilds[I].SetAllChildSelected(ASelected);
  end;
end;
...
begin
  ...
  begin
    ...
    if (Owner<>nil) and (Owner.GetListLayoutsManager<>nil) then
    begin
      Owner.GetListLayoutsManager.DoItemExpandedChange(Self);
    end;
...
procedure TBaseSkinTreeViewItem.SetSelectedByAllChildSelected;
var
  I: Integer;
begin
  if Self.Childs.Count>0 then
  begin
    for I := 0 to Self.Childs.Count-1 do
    begin
      Self.Childs[I].SetSelectedByAllChildSelected;
    end;
    Selected:=GetAllChildSelected;
  end;
end;
...
function TBaseSkinTreeViewItems.CreateBinaryObject(const AClassName:String=''): TObject;//TInterfacedPersistent;
begin
  Result:=Inherited;//SkinItemClass.Create;//(Self);
  if Result<>nil then
  begin
...
procedure TBaseSkinTreeViewItems.DoDelete(AObject: TObject);
var
...
begin
  ...
  if (ARootOwner<>nil) and (ARootOwner<>Self) then
  begin
    ARootOwner.DoDelete(AObject);//调用VirtualList.DoItemDelete,因为VirtualList.DoItemDelete只赋给顶层的List
  end;
end;
...
begin
  ...
  begin
      ARootOwner:=Self.GetRootOwner;
      if (ARootOwner<>nil) then
      begin
        Result:=ARootOwner.GetListLayoutsManager;
...
begin
  ...
  if (Self.FParent<>nil) then
  begin
    Result:=TBaseSkinTreeViewItems(Self.FParent.Owner);
    while (Result<>nil) and (Result.FParent<>nil) and (Result.FParent.Owner<>nil) do
    begin
      Result:=TBaseSkinTreeViewItems(Result.FParent.Owner);
    end;
  end;
end;

function TBaseSkinTreeViewItems.GetDefaultSkinItemClass: TBaseSkinItemClass;
begin
...
function TSkinListViewItems.GetDefaultSkinItemClass: TBaseSkinItemClass;
begin
  //wn
//  Result:=TSkinListViewItem;
  Result:=TRealSkinItem;
end;
...
function TRealSkinItems.Add: TRealSkinItem;
begin
  Result:=TRealSkinItem(Inherited Add);
end;

//procedure TRealSkinItems.InitSkinItemClass;
//begin
//  SkinItemClass:=TRealSkinItem;
//end;

function TRealSkinItems.Insert(Index:Integer): TRealSkinItem;
begin
  Result:=TRealSkinItem(Inherited Insert(Index));
end;

procedure TRealSkinItems.SetItem(Index: Integer;const Value: TRealSkinItem);
begin
  Inherited Items[Index]:=Value;
end;

//function TRealSkinItems.CreateBinaryObject(const AClassName:String=''): TInterfacedPersistent;
//begin
//  Result:=SkinItemClass.Create;//(Self);
//end;

function TRealSkinItems.GetItem(Index: Integer): TRealSkinItem;
begin
  Result:=TRealSkinItem(Inherited Items[Index]);
end;

function TRealSkinItems.GetDefaultSkinItemClass: TBaseSkinItemClass;
begin
  Result:=TRealSkinItem;
end;

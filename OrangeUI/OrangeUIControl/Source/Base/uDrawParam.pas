...
//    function SaveToJson(ASuperObject:ISuperObject):Boolean;
  public
    function SetVariable(AVariableNames:TStringList;AValueStr:String):Boolean;virtual;
    /// <summary>
    ///   <para>
    ///     初始化参数
    ///   </para>
...
//  FSetting:=GetDrawParamSettingClass.Create(Self);
  ...
  FShadowColor:=CreateDrawColor('ShadowColor','阴影颜色');
end;
...
function TDrawParam.SetVariable(AVariableNames:TStringList;AValueStr: String):Boolean;
begin
  Result:=False;

  if AVariableNames[0]='Alpha' then
  begin
    Self.Alpha:=StrToInt(AValueStr);
    Result:=True;
  end
  else if AVariableNames[0]='DrawEffectSetting' then
  begin
    AVariableNames.Delete(0);
    Self.FDrawEffectSetting.SetVariable(AVariableNames,AValueStr);
    Result:=True;
  end
  else if AVariableNames[0]='DrawRectSetting' then
  begin
    AVariableNames.Delete(0);
    Self.FDrawRectSetting.SetVariable(AVariableNames,AValueStr);
    Result:=True;
  end
  ;
...
begin
  ...
  case Self.FSizeType of
    ...
    begin
        ...
        else if FWidth=-1 then
        begin
          //FWidth为-1时,FWidth=FHeight,保持正方形
          AWidth:=AControlRect.Bottom-AControlRect.Top;
        end
        else
        begin
        ...
        if FHeight=-1 then
        begin
          //FHeight为-1时,FHeight=FWidth,保持正方形
          AHeight:=AControlRect.Right-AControlRect.Left;
        end
        else
        begin
    ...
    begin
        //像素
        ALeft:=ScreenScaleSize(FLeft);
        ATop:=ScreenScaleSize(FTop);
        ARight:=ScreenScaleSize(FRight);
        ABottom:=ScreenScaleSize(FBottom);
        if FWidth=0 then
        begin
          AWidth:=AControlRect.Right-AControlRect.Left;
        end
        ...
        begin
          AHeight:=ScreenScaleSize(FHeight);
        end;
    end;
  end;
...
function TDrawRectSetting.SetVariable(AVariableNames: TStringList;
  AValueStr: String): Boolean;
begin
  Result:=False;
...
//    FPositionHorzType:TDPPositionHorzType;
//    FPositionVertType:TDPPositionVertType;
  if AVariableNames[0]='Left' then
  begin
    Self.FLeft:=StrToFloat(AValueStr);
    Result:=True;
  end
  else if AVariableNames[0]='Top' then
  begin
    Self.FTop:=StrToFloat(AValueStr);
    Result:=True;
  end
  else if AVariableNames[0]='Right' then
  begin
    Self.FRight:=StrToFloat(AValueStr);
    Result:=True;
  end
  else if AVariableNames[0]='Bottom' then
  begin
    Self.FBottom:=StrToFloat(AValueStr);
    Result:=True;
  end
  else if AVariableNames[0]='Width' then
  begin
    Self.FWidth:=StrToFloat(AValueStr);
    Result:=True;
  end
  else if AVariableNames[0]='Height' then
  begin
    Self.FHeight:=StrToFloat(AValueStr);
    Result:=True;
  end
  else if AVariableNames[0]='Enabled' then
  begin
    Self.FEnabled:=StrToBool(AValueStr);
    Result:=True;
  end
//    else if ABTNode.NodeName='SizeType' then
//    begin
...
//function GetCommonEffectTypes(ACommonEffectTypesStr:String):TDPCommonEffectTypes;
//
  else if AVariableNames[0]='SizeType' then
  begin
    Self.FSizeType:=GetSizeType(AValueStr);
    Result:=True;
  end
  else if AVariableNames[0]='PositionHorzType' then
  begin
    Self.FPositionHorzType:=GetPositionHorzType(AValueStr);
    Result:=True;
  end
  else if AVariableNames[0]='PositionVertType' then
  begin
    Self.FPositionVertType:=GetPositionVertType(AValueStr);
    Result:=True;
  end
  ;
...
function TDrawParamCommonEffect.SetVariable(AVariableNames: TStringList;
  AValueStr: String):Boolean;
begin
  Result:=False;
  ...
  if AVariableNames[0]='Alpha' then
  begin
    Self.FAlpha:=StrToInt(AValueStr);
    FCommonEffectTypes:=FCommonEffectTypes+[dpcetAlphaChange];
    Result:=True;
  end
  else if AVariableNames[0]='Offset' then
  begin
    Self.FOffset:=StrToInt(AValueStr);
    FCommonEffectTypes:=FCommonEffectTypes+[dpcetOffsetChange];
    Result:=True;
  end
  else if AVariableNames[0]='CommonEffectTypes' then
  begin
    Self.FCommonEffectTypes:=GetSetFromStr_TDPCommonEffectTypes(AValueStr);
    Result:=True;
  end
  ;
...
function TDrawEffectSetting.SetVariable(AVariableNames: TStringList;
  AValueStr: String):Boolean;
var
  ADrawParamCommonEffect:TDrawParamCommonEffect;
begin
  Result:=False;
//    //按下状态的效果
//    FPushedEffect: TDrawParamCommonEffect;
...
//    FFocusedEffect: TDrawParamCommonEffect;
//
  ADrawParamCommonEffect:=nil;
  if AVariableNames[0]='PushedEffect' then
  begin
    ADrawParamCommonEffect:=FPushedEffect;
  end
  else if AVariableNames[0]='MouseOverEffect' then
  begin
    ADrawParamCommonEffect:=FMouseOverEffect;
  end
  else if AVariableNames[0]='MouseDownEffect' then
  begin
    ADrawParamCommonEffect:=FMouseDownEffect;
  end
  else if AVariableNames[0]='DisabledEffect' then
  begin
    ADrawParamCommonEffect:=FDisabledEffect;
  end
  else if AVariableNames[0]='FocusedEffect' then
  begin
    ADrawParamCommonEffect:=FFocusedEffect;
  end
  ;
  if ADrawParamCommonEffect<>nil then
  begin
    AVariableNames.Delete(0);
    ADrawParamCommonEffect.SetVariable(AVariableNames,AValueStr);
    Result:=True;
  end;

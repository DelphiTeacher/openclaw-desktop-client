...
//    {$ENDIF}
  ...
  public
    ...
    procedure DoDelete(AObject:TObject);virtual;
    procedure DoInsert(AObject:TObject);virtual;abstract;
    procedure BeginUpdate;override;
    procedure EndUpdate;override;//因为Delphi加载Collection的时候需要调用原始的EndUpdate (AIsForce:Boolean=False);overload;virtual;
...
//    function Add(AObject:TInterfacedPersistent):Integer;overload;
    ...
    function Delete(AIndex:Integer;AIsFree:Boolean=True):Integer;virtual;
    /// <summary>
    ///   <para>
    ///     创建并插入子对象
    ///   </para>
...
function TBinaryObject.QueryInterface({$IFDEF FPC}constref{$ELSE}const{$ENDIF} IID: TGUID;
...
function TBinaryCollection.QueryInterface({$IFDEF FPC}constref{$ELSE}const{$ENDIF} IID: TGUID;
...
function TBinaryCollection.Add: TCollectionItem;
begin
  Result:=TCollectionItem(CreateBinaryObject);
//  Result.Collection:=Self;
end;

//{$IFDEF DELPHI}
//
//procedure TBinaryCollection.Added(var Item: TCollectionItem);
//begin
//  inherited;
//  Self.DoAdd(Item);
//end;
...
procedure TBinaryCollection.BeginUpdate;
begin
  inherited;
...
//{$IFDEF DELPHI}
//
//procedure TBinaryCollection.Deleting(Item: TCollectionItem);
//begin
//  inherited;
//
//  DoDelete(Item);
...
procedure TBinaryCollection.Update(Item: TCollectionItem);
begin
  inherited;
...
procedure TBinaryCollection.Notify(Item: TCollectionItem;
...
begin
  ...
  //if (UpdateCount = 0) then
    Case Action of
      ...
      begin
        //只能做个标记先
        DoDelete(Item);//预删除，列表里面还存在着    最终是要到Changed才是已经删除了的
//        FLastItemChangeTypes:=FLastItemChangeTypes+[ictDel];
//        FHasItemDeleted:=True;
//        if Assigned(Self.FOnItemDelete) then
//        begin
//          Self.FOnItemDelete(Self,Item);
//        end;
...
//        begin
          ABinaryObject:=TInterfacedPersistent(CreateBinaryObject(AClassName));
          Add(TCollectionItem(ABinaryObject));
//        end;
...
procedure TBinaryCollection.Move(ASrcIndex: Integer; ADestIndex: Integer);
//var
//  ASrcPointer:TObject;
begin
  Self.Items[ASrcIndex].Index:=ADestIndex;
...
procedure TBinaryCollection.DoChange;
begin
...
procedure TBinaryCollection.DoAdd(AObject: TObject);
begin
...
procedure TBinaryCollection.DoDelete(AObject: TObject);
begin
  ...
  if Assigned(Self.FOnItemDelete) then
  begin
    Self.FOnItemDelete(Self,AObject);
  end;
...
begin
  ...
  begin
    //已经通过Collection的Items加载过了,不需要再通过Data来再加载一遍
    uBaseLog.HandleException(nil,'TBinaryCollection.DoReadData 已经通过Collection的Items加载过了,不需要再通过Data来再加载一遍');
    Exit;
  end;
//  Exit;
...
procedure TBinaryCollection.EndUpdate;//(AIsForce: Boolean);
begin
  Inherited EndUpdate;
end;
...
function TBinaryCollection.Delete(AIndex:Integer;AIsFree:Boolean=True):Integer;
var
  AItem:TObject;
begin
  AItem:=Items[AIndex];
  TCollectionItem(Items[AIndex]).Collection:=nil;
  if AIsFree then FreeAndNil(AItem);
end;

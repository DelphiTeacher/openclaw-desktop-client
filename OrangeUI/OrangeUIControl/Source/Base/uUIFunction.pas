...
procedure ShowFrame(
                    ...
                    {$IFDEF DELPHIXE8}overload;{$ENDIF}
...
begin
    ...
    begin
        ...
        GetGlobalVirtualKeyboardFixer.ProcessVKShowFocusedControl(AFocusedControl,
                        AKeyboardBounds_Height+AVirtualKeyboardHeightAdjustHeight);
...
//  AChild:TFmxObject;
  ...
  AParentIsScrollControl:Boolean;
  AParentScrollControl:TControl;
  AParent:TFmxObject;
  AVirtualKeyboardHeightAdjustHeight:Double;
begin
  if AIsFirst then
  ...
  AParentIsScrollControl:=False;
  AParentScrollControl:=nil;
  if AFocusedControl<>nil then
  begin
    if not GetParentIsScrollControl(AFocusedControl,AParentIsScrollControl,AParentScrollControl) then
    begin
      uBaseLog.OutputDebugString('CallSubFrameVirtualKeyboardShown Parent Is Not ScrollBox');
    end
    else
    begin
      uBaseLog.OutputDebugString('CallSubFrameVirtualKeyboardShown Parent ScrollBox is '+AParentScrollControl.Name);
    end;
  end;
...
//            if AParentScrollControl<>nil then
//            begin
//                //有ScrollBox,那么取ScrollBox.Parent,一般为Frame
//                AVirtualKeyboardPanelParent:=AParentScrollControl.Parent;
//
//                  if AParentScrollControl.Parent is TFrame then
//                  begin
//                      //有时候需要同时支持IFrameVirtualKeyboardEvent和IFrameVirtualKeyboardAutoProcessEvent
//                      if AParentScrollControl.Parent.GetInterface(IID_IFrameVirtualKeyboardAutoProcessEvent,AFrameVirtualKeyboardAutoProcessEvent) then
//                      begin
//
//                        uBaseLog.OutputDebugString('IFrameVirtualKeyboardAutoProcessEvent '+AParentScrollControl.Parent.Name+'');
//
...
//                end;
                ...
                //设置ScrollBox的位置
                if (AFocusedControl<>nil) and (AParentScrollControl<>nil) then
                begin

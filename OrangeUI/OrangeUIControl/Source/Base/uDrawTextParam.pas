...
function GetSetFromStr_TFontStyles(ASetStr:String):TFontStyles;
function GetStrFromSet_TFontStyles(ASet:TFontStyles):String;
function GetSetFromStr_TDTPEffectTypes(ASetStr:String):TDTPEffectTypes;
...
function TBaseDrawTextParam.SetVariable(AVariableNames:TStringList;AValueStr: String):Boolean;
begin
  Result:=inherited;
  ...
  begin
      if AVariableNames[0]='FontColor' then
      begin

//        {$IFDEF FMX}
//        AColorValue:=StrToInt(AValueStr);
//        Self.FontColor:=AColorValue;
//        {$ENDIF}
//        {$IFDEF VCL}
...
//        Self.FontColor:=AColorValue;
//        {$ENDIF}
        Self.FDrawFont.FFontColor.FColor:=ColorNameToColor(AValueStr);
        Result:=True;

      end
      else if AVariableNames[0]='FontSize' then
      begin
        {$IFDEF FMX}
        Self.FDrawFont.Size:=Ceil(StrToFloat(AValueStr));
        {$ENDIF}
        {$IFDEF VCL}
        //系统在高分屏上的时候，加载font的时候自动会将字体缩小比例
        //而用代码加载的话，没有自动缩小，要缩小
        Self.FDrawFont.Size:=Ceil(StrToFloat(AValueStr)/GetScreenScaleRate);
//        Self.FDrawFont.Size:=Ceil(StrToFloat(AValueStr));
        {$ENDIF}
        Result:=True;
      end
      else if AVariableNames[0]='FontHorzAlign' then
      begin
        Self.FFontHorzAlign:=GetFontHorzAlign(AValueStr);
        Result:=True;
      end
      else if AVariableNames[0]='FontVertAlign' then
      begin
        Self.FFontVertAlign:=GetFontVertAlign(AValueStr);
        Result:=True;
      end
      else if AVariableNames[0]='IsWordWrap' then
      begin
        Self.FIsWordWrap:=StrToBool(AValueStr);
        Result:=True;
      end
      ;
  end;
...
//  GlobalDrawTextParamList.Add(Self);
end;
...
//  GlobalDrawTextParamList.Remove(Self);
...
function TDrawFont.QueryInterface({$IFDEF FPC}constref{$ELSE}const{$ENDIF} IID: TGUID;
...
function TDrawTextParamEffect.SetVariable(AVariableNames: TStringList;
...
begin
  ...
  //    FEffectTypes: TDTPEffectTypes;
    ...
    if AVariableNames[0]='FontColor' then
    begin
      FFontColor.Color:=ColorNameToColor(AValueStr);
      FEffectTypes:=FEffectTypes+[dtpetFontColorChange];
      Result:=True;
    end
    else if (AVariableNames[0]='FontSize') then
    begin
      Self.FFontSize:=ControlSize(StrToFloat(AValueStr));
      FEffectTypes:=FEffectTypes+[dtpetFontSizeChange];
      Result:=True;
    end
    else if (AVariableNames[0]='FontStyle') then
    begin
      Self.FFontStyle:=GetSetFromStr_TFontStyles(AValueStr);
      FEffectTypes:=FEffectTypes+[dtpetFontStyleChange];
      Result:=True;
    end
  ...
  //  end
    ...
    begin
      Self.FEffectTypes:=GetSetFromStr_TDTPEffectTypes(AValueStr);
      Result:=True;
    end;
  end;
...
//  GlobalDrawTextParamList:=TList.Create;


finalization
  FreeAndNil(GlobalDrawTextParam);
//  FreeAndNil(GlobalDrawTextParamList);

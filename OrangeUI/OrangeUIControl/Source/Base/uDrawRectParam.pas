...
function GetSetFromStr_TDRPEffectTypes(ASetStr:String):TDRPEffectTypes;
var
  I,AElem:Integer;
  J:TDRPEffectType;
...
function GetSetFromStr_TDRPBorderEadges(ASetStr:String):TDRPBorderEadges;
var
  I,AElem:Integer;
  J:TDRPBorderEadge;
  AStrList:TStringList;
...
function GetSetFromStr_TDRPRectCorners(ASetStr:String):TDRPRectCorners;
var
  I,AElem:Integer;
  J:TDRPRectCorner;
  AStrList:TStringList;
...
procedure TBaseDrawRectParam.SetFillGradientColor1(const Value: TDrawColor);
begin
...
begin
  ...
  {$ENDIF FMX}
  FFillGradientColor1.Color:=Value;
end;
...
procedure TBaseDrawRectParam.SetStyle(const Value: String);
begin
  FStyle := Value;
end;

function TBaseDrawRectParam.SetVariable(AVariableNames:TStringList;AValueStr: String):Boolean;
begin
  Result:=inherited;
  ...
  if AVariableNames.Count>0 then
  begin
      if AVariableNames[0]='FillColor' then
      begin
//        {$IFDEF FMX}
...
////        end;
//        Self.FillColor.Color:=AColorValue;
//        {$ENDIF}
        Self.FFillDrawColor.Color:=ColorNameToColor(AValueStr);
        Result:=True;
      end
      else if AVariableNames[0]='IsFill' then
      begin
        Self.IsFill:=StrToBool(AValueStr);
        Result:=True;
      end
      else if AVariableNames[0]='IsRound' then
      begin
        Self.IsRound:=StrToBool(AValueStr);
        Result:=True;
      end
      else if AVariableNames[0]='RoundWidth' then
      begin
        Self.RoundWidth:=StrToInt(AValueStr);
        Result:=True;
      end
      else if AVariableNames[0]='RoundHeight' then
      begin
        Self.RoundHeight:=StrToInt(AValueStr);
        Result:=True;
      end
      //
      //背景透明,并且不需要选中的白底效果
      //+'ItemDesignerPanel.SelfOwnMaterial.BackColor.DrawEffectSetting.PushedEffect.IsFill:=False;'
//      else if AVariableNames[0]='DrawEffectSetting.PushedEffect.IsFill' then
//      begin
//        Self.DrawEffectSetting.PushedEffect.IsFill:=StrToBool(AValueStr);
//      end
      ;
  end;
end;
...
function TDrawRectParamEffect.IsFillColorChangeTypeStored: Boolean;
begin
...
function TDrawRectParamEffect.SetVariable(AVariableNames: TStringList;
...
//    FFillColorChangeType: TColorChangeType;
  ...
  begin
    ...
    if AVariableNames[0]='IsFill' then
    begin
      FIsFill:=StrToBool(AValueStr);
      FEffectTypes:=FEffectTypes+[drpetIsFillChange];
      Result:=True;
    end
    else if (AVariableNames[0]='FillDrawColor') or (AVariableNames[0]='FillColor') then
    begin
      Self.FFillDrawColor.Color:=ColorNameToColor(AValueStr);
      FEffectTypes:=FEffectTypes+[drpetFillColorChange];
      Result:=True;
    end
    else if (AVariableNames[0]='BorderDrawColor') or (AVariableNames[0]='BorderColor') then
    begin
      Self.FBorderDrawColor.Color:=ColorNameToColor(AValueStr);
      FEffectTypes:=FEffectTypes+[drpetBorderColorChange];
      Result:=True;
    end
    else if AVariableNames[0]='BorderWidth' then
    begin
      Self.FBorderWidth:=ControlSize(StrToFloat(AValueStr));
      FEffectTypes:=FEffectTypes+[drpetBorderWidthChange];
      Result:=True;
    end
  //  else if AVariableNames[0]='ImageName' then
  //  begin
  //    Self.FImageName:=AValueStr;
  //  end
    else if AVariableNames[0]='EffectTypes' then
    begin
      Self.FEffectTypes:=GetSetFromStr_TDRPEffectTypes(AValueStr);
      Result:=True;
    end;
  end;

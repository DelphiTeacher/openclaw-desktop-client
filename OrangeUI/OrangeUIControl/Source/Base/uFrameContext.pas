...
begin
  //记录索引
  DoRecordSubControlsLangIndex(AParent,
                            GlobalLang,
                            ACurLang,
                            ''
                            );
...
procedure TranslateSubControlsLang(
                            ...
                            AParent:TChildControl);
begin
  DoTranslateSubControlsLang(AParent,
                            GlobalLang,
                            GlobalCurLang);
end;
...
begin
  ...
  if (ToFrame.ClassName<>'TFrameMessageBox')
    and (ToFrame.ClassName<>'TFramePopupTreeMenu') then
  begin
    {$IFDEF FMX}
...
//  end;
  ...
  GlobalIsAllControlEnableBuffer:=False;
end;
...
//        AFrameOperItem.IsUseCacheImage:=Not AIsNew;
        ...
        GlobalIsAllControlEnableBuffer:=True;
    end;
...
//                begin
                    ...
                    begin
                      GlobalFrameHistoryLogList[0].FrameHistory.OnReturnFrame(AFromFrame);
                    end;
...
//  FrameSwitchEffectAnimator.EndTimesCount:=20;
  FrameSwitchEffectAnimator.EndTimesCount:=10;
  //页面切换效果的类型

...
//    FPriorUrlPicture:TUrlPicture;
    ...
    FFixedColor:TDrawColor;
    FFixedSVGWidth:Integer;
    FFixedSVGHeight:Integer;
...
//    function IsFixedColorStroed: Boolean;

    function IsFixedSVGWidthStroed: Boolean;
    function IsFixedSVGHeightStroed: Boolean;
    ...
    procedure SetFixedColor(const Value: TDrawColor);
    procedure DoFixedColorChange(Sender:TObject);
  protected
//    function GetSelfPicture:TSkinPicture;
//    procedure SetSelfPicture(const Value: TSkinPicture);
...
//    procedure DoUrlPictureDownloaded(AUrlPicture:TUrlPicture);
    ...
    function IsNeedStored:Boolean;override;
  ...
  published
    ...
    //因为DrawPicture可以展开的，双击在Lazarus下面不能弹出窗体来选择图片  但会造成LAZ保存窗体卡死慢
    //property SelfPicture:TSkinPicture read GetSelfPicture write SetSelfPicture stored False;

    property FixedSVGWidth:Integer read FFixedSVGWidth write FFixedSVGWidth stored IsFixedSVGWidthStroed;
    property FixedSVGHeight:Integer read FFixedSVGHeight write FFixedSVGHeight stored IsFixedSVGWidthStroed;
    property FixedColor:TDrawColor read FFixedColor write SetFixedColor;// stored IsFixedColorStroed;
...
//    property IsClipRound;
  ...
  TSkinPictureList=TDrawPictureList;
    ...
    TDrawPictureCollection=class;


    { TDrawPictureItem }

    TDrawPictureItem=class(TBinaryObject)
    private
      procedure SetDrawPicture(AValue: TDrawPicture);
    protected
    ...
    ///   </para>
    /// </summary>
    TDrawPictureCollection=class(TBinaryCollection)
    private
      function GetItem(Index: Integer): TDrawPicture;
      function GetItemByName(const AName: String): TDrawPicture;
//      procedure SetItem(Index: Integer; const Value: TDrawPictureItem);
    protected
      //创建图片(保存在FMX或DFM文件中时,加载的时候创建它来读取图片)
      function CreateBinaryObject(const AClassName:String=''):TObject;override;//TInterfacedPersistent;override;
    public
    ...
    public
      ...
      /// </summary>
      function Add:TDrawPicture;overload;
      procedure Add(ADrawPicture:TDrawPicture);overload;
  ...
  private
    ...
    procedure SetPictureList(const Value: TDrawPictureCollection);
  ...
  published
    ...
    ///   </para>
    /// </summary>
    property PictureList:TDrawPictureCollection read FPictureList write SetPictureList;
  end;
...
//  {$IFNDEF MSWINDOWS}
  ...
  begin
    ...
    begin
      ALastIconsMD5:=GetStringFromFile(AIconsZipFilePath+'.md5'{$IFDEF DELPHIXE},TEncoding.UTF8{$ENDIF});
    end;
    ...
    begin
      ...
      SaveStringToFile(AIconsMD5,AIconsZipFilePath+'.md5'{$IFDEF DELPHIXE},TEncoding.UTF8{$ENDIF});
    end
    else
    begin
...
begin
  ...
  FFixedColor:=TDrawColor.Create('FixedColor','SVG调整颜色');
...
//  FFixedColor.FColor:=TAlphaColorRec.Null;
//  {$ENDIF}
  FFixedColor.StoredDefaultColor:=DefaultColor;
  FFixedColor.FColor:=DefaultColor;

  FFixedColor.OnChange:=DoFixedColorChange;//DoChange;
...
begin
  ...
  case Self.FPictureDrawType of
    ...
    begin
      //自已的图片
      Result:=Self;//Inherited;// CurrentPicture;
...
procedure TBaseDrawPicture.SetFixedColor(const Value: TDrawColor);
begin
  FFixedColor.Assign(Value);
  DoChange;
end;
...
//            AUrlPicture.Picture:=uDrawEngine.CreateCurrentEngineSkinPicture;
    ...
    //          uBaseLog.OutputDebugString('TDownloadPictureManager.DownloadPicture 成功加载图片 end ');//+'耗时'+FloatToStr(DateUtils.MilliSecondsBetween(ABefore,Now)/1000));
        ...
        begin
          {$IFDEF FMX}
            TUrlPicture(AUrlPicture).ClipRoundPicture:=GlobalDrawCanvasClass.RoundSkinPicture(TUrlPicture(AUrlPicture).Picture,
                                                                        Self.FClipRoundXRadis,
                                                                        Self.FClipRoundYRadis);//,
//                                                                        Self.FClipRoundCorners);
          {$ENDIF}
...
procedure TBaseDrawPicture.DoFixedColorChange(Sender: TObject);
begin
  DoChange;
end;
...
function TBaseDrawPicture.IsFixedSVGHeightStroed: Boolean;
begin
  Result:=(FFixedSVGHeight<>0);
end;

function TBaseDrawPicture.IsFixedSVGWidthStroed: Boolean;
begin
  Result:=(FFixedSVGWidth<>0);
end;
...
//function TBaseDrawPicture.IsFixedColorStroed: Boolean;
//begin
//  Result:=FFixedColor.;
//end;
...
function TBaseDrawPicture.IsNeedStored: Boolean;
begin
//    function IsFixedSVGWidthStroed: Boolean;
//    function IsFixedSVGHeightStroed: Boolean;
//    function IsFileNameStroed: Boolean;
...
//    function IsUrlStroed: Boolean;
  Result:=Inherited
          or IsFileNameStroed
          or IsImageIndexStroed
          or IsImageNameStroed
          or IsResourceNameStroed
          or IsUrlStroed
          ;
end;
      ...
      begin
            ...
            begin
                ...
                begin
                    ...
                    begin
                      {$IFDEF FMX}
                      FUrlPicture.ClipRoundPicture:=GlobalDrawCanvasClass.RoundSkinPicture(FUrlPicture.Picture,
                                                                      Self.FClipRoundXRadis,
                                                                      Self.FClipRoundYRadis);//,
//                                                                      Self.FClipRoundCorners);
                      {$ENDIF}
...
begin
  ...
  begin
    ...
    else if ABTNode.NodeName='FixedColor' then
    begin
      FFixedColor.Assign(ABTNode.ConvertNode_Color.Data);
    end
...
procedure TBaseDrawPicture.LoadFromJson(AJson: ISuperObject);
begin
  if AJson.Contains('ImageIndex') then FImageIndex:=AJson.I['ImageIndex'];
  if AJson.Contains('ImageName') then FImageName:=AJson.S['ImageName'];
  if AJson.Contains('FileName') then FFileName:=AJson.S['FileName'];
  if AJson.Contains('ResourceName') then FResourceName:=AJson.S['ResourceName'];
  if AJson.Contains('Url') then FUrl:=AJson.S['Url'];
  if AJson.Contains('IsClipRound') then FIsClipRound:=AJson.B['IsClipRound'];
//  if AJson.Contains('FixedColor') then FFixedColor.Color:=StrToColor(AJson.S['FixedColor']);
...
begin
  ...
  ABTNode:=ADocNode.AddChildNode_Color('FixedColor','调整颜色');
  ABTNode.ConvertNode_Color.Data.Assign(FFixedColor);

  Result:=True;
end;

procedure TBaseDrawPicture.SaveToJson(AJson: ISuperObject);
begin
//    function IsFixedSVGWidthStroed: Boolean;
//    function IsFixedSVGHeightStroed: Boolean;
//    function IsFileNameStroed: Boolean;
...
//    function IsResourceNameStroed: Boolean;
//    function IsUrlStroed: Boolean;
  if IsImageIndexStroed then AJson.I['ImageIndex']:=FImageIndex;
  if IsImageNameStroed then AJson.S['ImageName']:=FImageName;
  if IsFileNameStroed then AJson.S['FileName']:=FFileName;
  if IsResourceNameStroed then AJson.S['ResourceName']:=FResourceName;
  if IsUrlStroed then AJson.S['Url']:=FUrl;
  if IsClipRound then AJson.B['IsClipRound']:=FIsClipRound;
//  if FImageIndex<>-1 then AJson.S['FixedColor']:=ColorToStr(FFixedColor.Color);
end;
...
function TDrawPictureList.CreateBinaryObject(const AClassName:String=''):TObject;//TInterfacedPersistent;
begin
  //创建临时图片
//  Result:=CreateCurrentEngineDrawPicture('','','');
  Result:=TDrawPicture.Create('','','');
...
procedure TSkinBaseImageList.SetPictureList(const Value: TDrawPictureCollection);
begin
...
procedure TDrawPictureItem.SetDrawPicture(AValue: TDrawPicture);
begin
  if FDrawPicture=AValue then Exit;
  FDrawPicture.Assign(AValue);
end;

function TDrawPictureItem.LoadFromDocNode(ADocNode: TBTNode20_Class): Boolean;
begin
  Result:=FDrawPicture.LoadFromDocNode(ADocNode);
...
var
  DestObject:TDrawPictureItem;
begin

  if Dest is TDrawPictureItem then
  begin
    DestObject:=TDrawPictureItem(Dest);

    DestObject.FDrawPicture.Assign(Self.FDrawPicture);
...
begin
  ...
  FDrawPictureIsSelfOwn:=True;
end;

destructor TDrawPictureItem.Destroy;
begin
  if FDrawPictureIsSelfOwn then
  begin
...
{ TDrawPictureCollection }

function TDrawPictureCollection.GetItem(Index: Integer): TDrawPicture;
begin
  Result:=TDrawPictureItem(Inherited Items[Index]).FDrawPicture;
end;


function TDrawPictureCollection.GetItemByName(const AName: String): TDrawPicture;
var
  I: Integer;
  AScale:Integer;
  AScaleName:String;
...
var
  ASkinPictureItem:TDrawPictureItem;
begin
  ASkinPictureItem:=TDrawPictureItem.Create(Self);//'','');
  ASkinPictureItem.FDrawPicture.Assign(AGraphic);
  //Result:=Self.Add(ASkinPictureItem);
end;

function TDrawPictureCollection.AddPicture(APicture: TPicture): Integer;
begin
  Result:=Self.AddGraphic(APicture.Graphic);
end;
{$ENDIF}

function TDrawPictureCollection.AddPictureFile(const APictureFile: String): Integer;
var
  ADrawPictureItem:TDrawPictureItem;
begin
//  ADrawPicture:=CreateCurrentEngineDrawPicture('','');
  ADrawPictureItem:=TDrawPictureItem.Create(Self);//'','');
  ADrawPictureItem.FDrawPicture.LoadFromFile(APictureFile);
  //Result:=Inherited Add(ADrawPicture);
end;

function TDrawPictureCollection.Add: TDrawPicture;
begin
//  Result:=CreateCurrentEngineDrawPicture('','');
  Result:=TDrawPictureItem.Create(Self).FDrawPicture;//('','');
  //Inherited Add(Result);
end;

procedure TDrawPictureCollection.Add(ADrawPicture: TDrawPicture);
var
  ADrawPictureItem:TDrawPictureItem;
begin
  ADrawPictureItem:=TDrawPictureItem.Create(Self);
  FreeAndNil(ADrawPictureItem.FDrawPicture);
  ADrawPictureItem.FDrawPicture:=ADrawPicture;
  ADrawPictureItem.FDrawPictureIsSelfOwn:=False;
end;

function TDrawPictureCollection.AddFileNameOnly(const AFileName: String): Integer;
var
  ADrawPictureItem:TDrawPictureItem;
begin
//  ADrawPicture:=CreateCurrentEngineDrawPicture('','');
  ADrawPictureItem:=TDrawPictureItem.Create(Self);
  ADrawPictureItem.FDrawPicture.FileName:=ExtractFileName(AFileName);
  //Result:=Inherited Add(ADrawPicture);
end;
...
//  ADestDrawPicture:=TDrawPictureItem.Create();
  Self.Items[AIndex].LoadFromFile(APictureFile);;
//  ASrcSkinPicture:=Self.Items[AIndex];
//  Self.Items[AIndex]:=ADestDrawPicture;
//  FreeAndNil(ASrcSkinPicture);
end;
...
//procedure TDrawPictureCollection.SetItem(Index: Integer; const Value: TDrawPictureItem);
//begin
//  Inherited Items[Index]:=Value;
//end;

function TDrawPictureCollection.CreateBinaryObject(const AClassName:String=''):TObject;//TInterfacedPersistent;
begin
  //创建临时图片
//  Result:=CreateCurrentEngineDrawPicture('','','');

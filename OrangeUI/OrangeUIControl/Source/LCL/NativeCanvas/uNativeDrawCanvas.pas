...
//    Stroke:TStrokeBrush;
  ...
  public
    ...
    //闭合路径
    procedure StartPath;override;
    //闭合路径
    procedure StopPath;override;
//    //获取区域,用来判断鼠标是否在区域内
...
//    function CreateRectF(const X,Y,Width,Height:Double):TRectF;overload;
  ...
  public
    ...
    procedure PrepareBitmap(const ABitmap:TObject);override;
...
function TNativeDrawCanvas.DrawBezierLine(const ADrawLineParam: TDrawLineParam;APoints:Array of TPointF): Boolean;
//var
//  APt1, APt2: TPointF;
//  ARectF:TRectF;
var
...
//  Y2:=ARectF.Bottom;
  ...
  Self.FCanvas.PolyBezier(tempPoints);
...
//  AIsEnd:=False;
  ...
  FCanvas.Pen.Width:=Ceil(ADrawPathParam.CurrentEffectPenWidth);
  FCanvas.Pen.Style := TPenStyle.psSolid;
...
function TNativeDrawCanvas.DrawPathData(ADrawPathData: TBaseDrawPathData;const ADrawRect:TRectF;APathActions:TPathActionCollection): Boolean;
begin
...
//  AHTextAlign:=TTextAlign.{$IF CompilerVersion >= 35.0}Leading{$ELSE}taLeading{$IFEND};
  case ADrawTextParam.FontHorzAlign of
    fhaLeft: ATextFormat:=ATextFormat+[tfLeft];//AHTextAlign:=TTextAlign.{$IF CompilerVersion >= 35.0}Leading{$ELSE}taLeading{$IFEND};
    fhaCenter: ATextFormat:=ATextFormat+[tfCenter];//AHTextAlign:=TTextAlign.{$IF CompilerVersion >= 35.0}Center{$ELSE}taCenter{$IFEND};
    fhaRight: ATextFormat:=ATextFormat+[tfRight];//AHTextAlign:=TTextAlign.{$IF CompilerVersion >= 35.0}Trailing{$ELSE}taTrailing{$IFEND};
  end;

//  //垂直对齐
//  AVTextAlign:=TTextAlign.{$IF CompilerVersion >= 35.0}Leading{$ELSE}taLeading{$IFEND};
  case ADrawTextParam.FontVertAlign of
    fvaTop: ATextFormat:=ATextFormat+[tfTop];//AVTextAlign:=TTextAlign.{$IF CompilerVersion >= 35.0}Leading{$ELSE}taLeading{$IFEND};
    fvaCenter: ATextFormat:=ATextFormat+[tfVerticalCenter,tfSingleLine];//AVTextAlign:=TTextAlign.{$IF CompilerVersion >= 35.0}Center{$ELSE}taCenter{$IFEND};
    fvaBottom: ATextFormat:=ATextFormat+[tfBottom];//AVTextAlign:=TTextAlign.{$IF CompilerVersion >= 35.0}Trailing{$ELSE}taTrailing{$IFEND};
  end;
//  ATextFormat:=ATextFormat+[tfVerticalCenter];
  {$ENDIF}
  ...
  //  AHTextAlign:=TTextAlign.{$IF CompilerVersion >= 35.0}Leading{$ELSE}taLeading{$IFEND};
    case ADrawTextParam.FontHorzAlign of
      fhaLeft: ATextStyle.Alignment := taLeftJustify;//AHTextAlign:=TTextAlign.{$IF CompilerVersion >= 35.0}Leading{$ELSE}taLeading{$IFEND};
      fhaCenter: ATextStyle.Alignment := taCenter;//AHTextAlign:=TTextAlign.{$IF CompilerVersion >= 35.0}Center{$ELSE}taCenter{$IFEND};
      fhaRight: ATextStyle.Alignment := taRightJustify;//AHTextAlign:=TTextAlign.{$IF CompilerVersion >= 35.0}Trailing{$ELSE}taTrailing{$IFEND};
    end;

  //  //垂直对齐
  //  AVTextAlign:=TTextAlign.{$IF CompilerVersion >= 35.0}Leading{$ELSE}taLeading{$IFEND};
    case ADrawTextParam.FontVertAlign of
      fvaTop: ATextStyle.Layout := tlTop;//AVTextAlign:=TTextAlign.{$IF CompilerVersion >= 35.0}Leading{$ELSE}taLeading{$IFEND};
      fvaCenter: ATextStyle.Layout := tlCenter;//AVTextAlign:=TTextAlign.{$IF CompilerVersion >= 35.0}Center{$ELSE}taCenter{$IFEND};
      fvaBottom: ATextStyle.Layout := tlBottom;//AVTextAlign:=TTextAlign.{$IF CompilerVersion >= 35.0}Trailing{$ELSE}taTrailing{$IFEND};
    end;
  {$ENDIF}
...
function TNativeDrawCanvas.FillPathData(ADrawPathParam:TDrawPathParam;ADrawPathData: TBaseDrawPathData;const ADrawRect:TRectF;APathActions:TPathActionCollection): Boolean;
begin
//  //创建画刷
//  Self.FCanvas.Fill.Kind:=FMX.Graphics.TBrushKind.{$IF CompilerVersion >= 35.0}Solid{$ELSE}bkSolid{$IFEND};
//  Self.FCanvas.Fill.Color:=ADrawPathParam.CurrentEffectFillDrawColor.Color;

...
type
  ...
  public
    ...
    function IsVisible(ACanvas:TCanvas;const APoint: TPointF):Boolean;override;
//    //闭合路径
...
var
  //是否高速画图
  GlobalIsDrawBitmapHignSpeed:Boolean;
...
class function TFireMonkeyDrawCanvas.RoundSkinPicture(ABitmap: TSkinPicture;
  AXRadius, AYRadius: Double): TSkinPicture;
var
  ABrush:TBrush;
...
function TFireMonkeyDrawCanvas.DrawBezierLine(
  const ADrawLineParam: TDrawLineParam; APoints: array of TPointF): Boolean;
//var
//  APt1, APt2: TPointF;
//  ARectF:TRectF;
var
...
//  end;
  ...
  //一共四个点,起点，控制点1,控制点2,终点
  APath:=TPathData.Create;
  APath.MoveTo(APoints[0]);
  APath.CurveTo(APoints[1],APoints[2],APoints[3]);
  FCanvas.DrawPath(APath,1);
  FreeAndNil(APath);
...
function TFireMonkeyDrawCanvas.DrawPathData(ADrawPathData: TBaseDrawPathData;const ADrawRect:TRectF;APathActions:TPathActionCollection): Boolean;
begin
  Self.FCanvas.Stroke.Thickness:=ADrawPathData.PenWidth;
  Self.FCanvas.Stroke.Kind := TBrushKind.Solid;
  Self.FCanvas.Stroke.Color := ADrawPathData.PenColor.Color;
  Self.FCanvas.Stroke.Dash:=TStrokeDash.Solid;
...
//              begin
//                  ADrawRectParam.FShadowEffect.ShadowColor:=ADrawRectParam.CurrentEffectFillDrawColor.Color;
                  ADrawRectParam.FShadowEffect.ShadowColor:=ADrawRectParam.FShadowColor.Color;
...
//              Self.FCanvas.Fill.Kind:=FMX.Graphics.TBrushKind.None;
            ...
            begin
              ...
              Self.FCanvas.Fill.Gradient.Assign(ADrawRectParam.Gradient);//因为还有怎么Points什么的
              Self.FCanvas.Fill.Gradient.Color:=ADrawRectParam.FillColor.Color;
//              Self.FCanvas.Fill.Gradient.Color1:=ADrawRectParam.FillGradientColor1.Color;
            end;
          end;
...
function TFireMonkeyDrawCanvas.FillPathData(ADrawPathParam:TDrawPathParam;ADrawPathData: TBaseDrawPathData;const ADrawRect:TRectF;APathActions:TPathActionCollection): Boolean;
begin
  //创建画刷
  Self.FCanvas.Fill.Kind:=FMX.Graphics.TBrushKind.{$IF CompilerVersion >= 35.0}Solid{$ELSE}bkSolid{$IFEND};
  Self.FCanvas.Fill.Color:=ADrawPathParam.CurrentEffectFillDrawColor.Color;
...
//      Self.FCanvas.Fill.Gradient.Assign(ADrawRectParam.Gradient);
      Self.FCanvas.Fill.Gradient.Color:=ADrawPathParam.FillColor.Color;
      Self.FCanvas.Fill.Gradient.Color1:=ADrawPathParam.FillGradientColor1.Color;
    end;
  end;
...
procedure TFireMonkeyDrawPathData.AddBezierLine(const X, Y, X1, Y1, X2, Y2, X3,
  Y3: Double);
begin
//  Self.Path.MoveTo(PointF(X, Y));
  Self.Path.CurveTo(PointF(X1, Y1),PointF(X2, Y2),PointF(X3, Y3));
//  Self.Path.MoveTo(PointF(X3, Y3));
end;
...
procedure TFireMonkeyDrawPathData.AddPloygon(const APoints: array of TPointF);
var
  I: Integer;
begin
  if Length(APoints)>0 then
  begin
    Self.Path.MoveTo(APoints[0]);
    for I := 1 to Length(APoints)-1 do
    begin
      Self.Path.LineTo(APoints[I]);
    end;
    Self.Path.LineTo(APoints[0]);
  end;
end;
...
function TFireMonkeyDrawPathData.IsVisible(ACanvas:TCanvas;const APoint: TPointF): Boolean;
begin
  Result:=ACanvas.PtInPath(APoint,Path);
end;
...
initialization
  GlobalIsDrawBitmapHignSpeed:=False;

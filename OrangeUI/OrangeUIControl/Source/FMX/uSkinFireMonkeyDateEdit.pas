...
//    FIsMouseDownCanFocus:Boolean;
  ...
  public
    ...
    FStyleObj_text: TControl;
    FStyleObj_foreground: TControl;
...
procedure TSkinFMXCustomDateEdit.UpdateText;
begin
end;

procedure TSkinFMXCustomDateEdit.SetTextInternal(const Value: string);
begin
end;
...
//  Exit;
  ...
  begin
    ...
    if not (Self.Children[I] is TControl) then
    begin
      Continue;
    end;
    ...
    begin
      if not (Self.Children[I].Children[J] is TControl) then
      begin
        Continue;
      end;
      ...
      begin
        ...
        FStyleObj_background.OnPaint:=nil;
      end;
      if Self.Children[I].Children[J].StyleName='Text' then
      begin

        //文本
        FStyleObj_text:=TControl(Self.Children[I].Children[J]);
        ClearStyleObjectVisiualElement(FStyleObj_text,'DateEdit');
        FStyleObj_text.OnPainting:=nil;
        FStyleObj_text.OnPaint:=nil;
      end;
      if Self.Children[I].Children[J].StyleName='selection' then
      begin
        //文本
        TControl(Self.Children[I].Children[J]).OnPainting:=nil;
        TControl(Self.Children[I].Children[J]).OnPaint:=nil;
      end;

    end;


    if Self.Children[I].StyleName='foreground' then
    begin

      //前景
      FStyleObj_foreground:=TControl(Self.Children[I]);
      ClearStyleObjectVisiualElement(FStyleObj_foreground,'DateEdit');
      FStyleObj_foreground.OnPainting:=nil;
      FStyleObj_foreground.OnPaint:=nil;
    end;
...
function TSkinFMXCustomDateEdit.GetIsEmpty:Boolean;
begin
  Result:=IsEmpty;
end;
...
begin
//  if (Self.DateTime=0) or FormatDate then//'1899-12-31'
//  if (Self.DateTime=0) or (Copy(Text,1,10)='1899-12-31') then
  ...
  begin
    Result:=FormatDateTime(GetFormat, DateTime);
  end;
end;
...
begin
    ...
    begin
      ...
      Self.GetSkinControlType.Paint(FDrawCanvas,ASkinMaterial,RectF(0,0,Self.Width,Self.Height),FPaintData);
    end;
    ...
    begin
      FDrawCanvas.DrawDesigningRect(RectF(0,0,Width,Height),GlobalNormalDesignRectBorderColor);
      FDrawCanvas.DrawDesigningText(RectF(0,0,Width,Height),Self.Name);

      //绘制控件绑定的字段
//      FDrawCanvas.DrawDesigningText(RectF(0,0,Width,Height),Self.BindItemFieldName);
      Canvas.Font.Size:=10;
...
//  FProperties:=Self.GetPropertiesClassType.Create(Self);
  ...
  Self.TextSettings.FontColor:=TAlphaColorRec.Null;
...
function TSkinFMXCustomDateEdit.EditGetValue:Variant;
begin

end;

procedure TSkinFMXCustomDateEdit.EditSetValue(const AValue:Variant);
begin
...
begin
  ...
  if FStyleObj_foreground<>nil then
  begin
    FStyleObj_foreground.RePaint;
  end;
  if FStyleObj_text<>nil then
  begin
    FStyleObj_text.RePaint;
  end;
end;

...
//    procedure SetProp(APropName:String;APropValue:Variant);
  ...
  published
    ...
    ///   是否在滚动框中
    /// </summary>
    property IsInScrollBox:Boolean read FIsInScrollBox write FIsInScrollBox Stored False;
    //在列表项中自动启动编辑
...
//    begin
//      FDrawCanvas.DrawRect(TSkinControlMaterial(Self.CurrentUseMaterial).BackColor,
...
//  end;
  ...
  begin
    ...
    Self.SkinControlType.Paint(FDrawCanvas,ASkinMaterial,RectF(0,0,Self.Width,Self.Height),FPaintData);
  end;
...
function TSkinFMXCustomEdit.EditGetValue:Variant;
begin
  Result:=Self.Text;
end;

procedure TSkinFMXCustomEdit.EditSetValue(const AValue:Variant);
begin
  Self.Text:=AValue;
end;

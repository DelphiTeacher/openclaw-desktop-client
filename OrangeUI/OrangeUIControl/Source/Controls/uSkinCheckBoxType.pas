...
type
  ...
  private
    FPartiallyChecked: Boolean;
    procedure SetPartiallyChecked(const Value: Boolean);
  protected
    procedure DoChange;
  ...
  published
    ...
    //第三种状态,未确定，部分勾选，未全选
    property PartiallyChecked:Boolean read FPartiallyChecked write SetPartiallyChecked;
  end;
  ...
  public
    procedure DrawTo(ACanvas:TDrawCanvas;
                      ...
                      ADrawRect:TRectF;
                    AChecked:Boolean;
                      APartiallyChecked:Boolean);virtual;
  published
  ...
  private
    ...
    FNormalPartiallyCheckedPicture: TDrawPicture;
    ...
    procedure SetNormalPartiallyCheckedDrawPicture(const Value: TDrawPicture);
  ...
  private
    ...
    FDrawPartiallyCheckStateParam: TDrawRectParam;
    ...
    procedure SetDrawPartiallyCheckStateParam(const Value: TDrawRectParam);
    procedure SetIsSimpleDrawCheckState(const Value: Boolean);
    procedure SetCheckChar(const Value: String);
  ...
  public
    procedure DrawTo(ACanvas:TDrawCanvas;
                    ...
                    ADrawRect:TRectF;
                    AChecked:Boolean;
  APartiallyChecked:Boolean);override;
  public
    constructor Create(AOwner:TComponent);override;
    destructor Destroy;override;
  published
    ...
    property NormalPartiallyCheckedPicture:TDrawPicture read FNormalPartiallyCheckedPicture write SetNormalPartiallyCheckedDrawPicture;
  ...
  published
    //√,✓,✔   √,✓,✔
    property CheckChar:String read FCheckChar write SetCheckChar;
    /// <summary>
    ///   <para>
    ///     勾选状态路径
    ///   </para>
    ...
    property DrawPartiallyCheckStateParam:TDrawRectParam read FDrawPartiallyCheckStateParam write SetDrawPartiallyCheckStateParam;
    /// <summary>
    ///   <para>
    ///     勾选框
    ///   </para>
...
//    property DrawCheckCharParam:TDrawTextParam read FDrawCheckCharParam write SetDrawCheckCharParam;
//  end;
  TSkinCheckBoxColorMaterial=class(TSkinCheckBoxDefaultMaterial)
...
//  end;
  ...
  protected
    ...
    procedure SetControlValueByBindItemField(const AFieldName:String;//ItemCaption
  ...
  published
    ...
    //标题绑定字段
//    property CaptionBindItemFieldName:String read FCaptionBindItemFieldName write FCaptionBindItemFieldName;
...
//  {$I ComponentPlatformsAttribute.inc}
//  TSkinFMXCheckBox=class(TSkinCheckBox)
//  end;
  ...
  {$IFDEF VCL}
  TSkinWinCheckBox=class(TSkinCheckBox)
  end;
  {$ENDIF}
...
begin
  ...
  if Self.FSkinCheckBoxIntf.Prop.FChecked or Self.FSkinCheckBoxIntf.Prop.FPartiallyChecked then
  begin
...
begin
  ...
  begin
    ...
    //而不是根据自身的Checked
    if Not (Self.FSkinCheckBoxIntf.Prop.FChecked or Self.FSkinCheckBoxIntf.Prop.FPartiallyChecked) then
    begin
      Result:=Result-[dpstPushed];
    end
    else
...
//    begin
      ...
      APartiallyChecked:=Self.FSkinCheckBoxIntf.Properties.FPartiallyChecked;
//    end;
  ...
  //begin
    ...
    begin
      ...
      else if APartiallyChecked then
        APicture:=AMaterial.FNormalPartiallyCheckedPicture
      else
    ...
    if not APicture.CurrentPictureIsEmpty then
    begin
      if AMaterial.FIsAutoPosition then
      begin
          ...
          ACanvas.DrawPicture(AMaterial.FDrawPictureParam,APicture,APictureDrawRect);

          ACaptionDrawRect:=ADrawRect;
          ACaptionDrawRect.Left:=APictureDrawRect.Right+5;
          ACanvas.DrawText(AMaterial.FDrawCaptionParam,
                            Self.FSkinControlIntf.Caption,
                            ACaptionDrawRect);
      end
      else
      begin
          ACanvas.DrawPicture(AMaterial.FDrawPictureParam,APicture,ADrawRect);
          ACanvas.DrawText(AMaterial.FDrawCaptionParam,Self.FSkinControlIntf.Caption,ADrawRect);
      end;
    end
    else
    begin
...
begin
  ...
  FNormalPartiallyCheckedPicture:=CreateDrawPicture('NormalPartiallyCheckedPicture','正常状态时已勾选图片','已勾选所有状态图片');
  ...
  FDrawPartiallyCheckStateParam:=CreateDrawRectParam('DrawPartiallyCheckStateParam','未全部勾选状态的路径绘制参数');
...
begin
  ...
  FreeAndNil(FNormalPartiallyCheckedPicture);
  ...
  FreeAndNil(FDrawPartiallyCheckStateParam);
...
procedure TSkinCheckBoxDefaultMaterial.SetNormalPartiallyCheckedDrawPicture(const Value: TDrawPicture);
begin
  FNormalPartiallyCheckedPicture.Assign(Value);
end;
...
procedure TSkinCheckBoxDefaultMaterial.AssignTo(Dest: TPersistent);
var
  DestObject:TSkinCheckBoxDefaultMaterial;
begin

  if Dest is TSkinCheckBoxDefaultMaterial then
  begin
    DestObject:=TSkinCheckBoxDefaultMaterial(Dest);
...
procedure TSkinCheckBoxDefaultMaterial.DrawTo(ACanvas: TDrawCanvas;
  ACaption:String;
  ADrawRect: TRectF;
  AChecked:Boolean;
  APartiallyChecked:Boolean);
...
begin
  ...
  begin
      ...
      begin
        if AChecked then
        begin
             ACanvas.DrawText(Self.FDrawCheckCharParam,FCheckChar,ACheckDrawRect);
        end
        else
        if APartiallyChecked then
        begin
          ACanvas.DrawText(Self.FDrawCheckCharParam,'■',ACheckDrawRect);
        end
      end
      else
      begin
        ...
        if APartiallyChecked then
        begin
          ACanvas.DrawRect(Self.FDrawPartiallyCheckStateParam,ACheckDrawRect);
        end;
      end;
...
//      ACanvas.DrawPath(Self.FDrawCheckStateParam,ADrawRect);
      ...
      begin
        if AChecked then
        begin
          ACanvas.DrawText(Self.FDrawCheckCharParam,FCheckChar,ADrawRect);
        end
        else
        if APartiallyChecked then
        begin
          ACanvas.DrawText(Self.FDrawCheckCharParam,'■',ADrawRect);
        end;
      end
      else
      begin
        ...
        begin
          ACanvas.DrawRect(Self.FDrawPartiallyCheckStateParam,ADrawRect);
        end;
      end;
...
procedure TSkinCheckBoxDefaultMaterial.ProcessSimpleDrawCheckState;
var
...
procedure TSkinCheckBoxDefaultMaterial.SetDrawCheckStateParam(const Value: TDrawPathParam);
begin
  FDrawCheckStateParam.Assign(Value);
end;

procedure TSkinCheckBoxDefaultMaterial.SetDrawPartiallyCheckStateParam(const Value: TDrawRectParam);
begin
  FDrawPartiallyCheckStateParam.Assign(Value);
end;

procedure TSkinCheckBoxDefaultMaterial.SetIsSimpleDrawCheckChar(
  const Value: Boolean);
begin
...
procedure TSkinCheckBoxDefaultMaterial.SetIsSimpleDrawCheckState(const Value: Boolean);
begin
...
procedure TSkinCheckBoxDefaultMaterial.SetCheckChar(const Value: String);
begin
...
procedure TSkinCheckBoxDefaultMaterial.SetDrawCheckCharParam(
  const Value: TDrawTextParam);
begin
  FDrawCheckCharParam.Assign(Value);
end;

procedure TSkinCheckBoxDefaultMaterial.SetDrawCheckRectParam(const Value: TDrawRectParam);
begin
...
procedure TCheckBoxProperties.SetPartiallyChecked(const Value: Boolean);
begin
  if FPartiallyChecked<>Value then
  begin
    Self.FPartiallyChecked:=Value;
    Invalidate;
  end;
end;
...
//                           Self.FSkinCheckBoxIntf.Properties.PartiallyChecked
//                           );
//  end;
//end;
...
function TSkinCheckBox.Material:TSkinCheckBoxDefaultMaterial;
begin
  Result:=TSkinCheckBoxDefaultMaterial(SelfOwnMaterial);
end;

function TSkinCheckBox.SelfOwnMaterialToDefault:TSkinCheckBoxDefaultMaterial;
begin
  Result:=TSkinCheckBoxDefaultMaterial(SelfOwnMaterial);
end;

function TSkinCheckBox.CurrentUseMaterialToDefault:TSkinCheckBoxDefaultMaterial;
begin
  Result:=TSkinCheckBoxDefaultMaterial(CurrentUseMaterial);
end;

procedure TSkinCheckBox.StayClick;
begin
...
function TSkinCheckBox.GetPostValue(ASetting:TFieldControlSetting;APageDataDir:String;ASetRecordFieldValueIntf:ISetRecordFieldValue;
...
procedure TSkinCheckBox.SetControlValue(ASetting:TFieldControlSetting;APageDataDir:String;AImageServerUrl:String;AValue:Variant;AValueCaption:String;
                            //要设置多个值,整个字段的记录
...
function TSkinCheckBox.GetPropertiesClassType: TPropertiesClassType;
begin
  Result:=TCheckBoxProperties;
end;

function TSkinCheckBox.GetCheckBoxProperties: TCheckBoxProperties;
begin
  Result:=TCheckBoxProperties(Self.FProperties);
end;

function TSkinCheckBox.GetOnChange: TNotifyEvent;
begin
  Result:=Self.FOnChange;
end;

procedure TSkinCheckBox.SetCheckBoxProperties(Value: TCheckBoxProperties);
begin
  Self.FProperties.Assign(Value);
end;

procedure TSkinCheckBox.SetControlValueByBindItemField(const AFieldName: String;
  const AFieldValue: Variant;
                                              APropertyName:String;
                                              ABindItemFieldSetting:TBindItemFieldSetting;
...
procedure TSkinCheckBox.DoCustomSkinMaterialChange(Sender: TObject);
begin

...
type
  ...
  ISkinVirtualList=interface//(ISkinScrollControl)
    ...
    function GetOnNewListItemStyleFrameCacheChangeItem:TNewListItemStyleFrameCacheInitEvent;
    ...
    //列表项样式Frame切换Item事件
    property OnNewListItemStyleFrameCacheChangeItem:TNewListItemStyleFrameCacheInitEvent read GetOnNewListItemStyleFrameCacheChangeItem;
  ...
  protected
    ...
    FItemColorFieldName:String;
    FItemColorFieldSetType:TSkinItemColorSetType;
  ...
  private
    ...
    procedure SetItemColorType(const Value: TSkinItemColorType);
    procedure CalcItemsAutoSize;
...
//                                          AEditItemSubItemsIndex:Integer;
  ...
  public
    ...
    //计算列表项的尺寸
    function CalcItemAutoSize(AItem:TSkinItem;const ABottomSpace:TControlSize=10;const ARightSpace:TControlSize=20):TSizeF;

    procedure SetItemAutoSize(AItem:TSkinItem);
    procedure SetItemsAutoSize;
...
//                                  AListItemStyle:String):Boolean;override;
  published
    ...
    ///   </para>
    /// </summary>
    property ItemColorType:TSkinItemColorType read FItemColorType write SetItemColorType;
    property ItemColorFieldName:String read FItemColorFieldName write FItemColorFieldName;
    property ItemColorFieldSetType:TSkinItemColorSetType read FItemColorFieldSetType write FItemColorFieldSetType;
...
//    FOriginStaticRectCorners:TDRPRectCorners;
  ...
  private
    ...
    FOnNewListItemStyleFrameCacheChangeItem:TNewListItemStyleFrameCacheInitEvent;
...
//    procedure Loaded;override;
  ...
  published
    ...
    property OnNewListItemStyleFrameCacheChangeItem:TNewListItemStyleFrameCacheInitEvent read GetOnNewListItemStyleFrameCacheChangeItem write FOnNewListItemStyleFrameCacheChangeItem;
...
begin
  ...
  begin
    ...
    FItem1ItemStyleSetting.FOnChangeItem:=Self.DoNewListItemStyleFrameCacheChangeItem;
    ...
    FItem2ItemStyleSetting.FOnChangeItem:=Self.DoNewListItemStyleFrameCacheChangeItem;
    ...
    FItem3ItemStyleSetting.FOnChangeItem:=Self.DoNewListItemStyleFrameCacheChangeItem;
    ...
    FItem4ItemStyleSetting.FOnChangeItem:=Self.DoNewListItemStyleFrameCacheChangeItem;
    ...
    FHeaderItemStyleSetting.FOnChangeItem:=Self.DoNewListItemStyleFrameCacheChangeItem;
    ...
    FFooterItemStyleSetting.FOnChangeItem:=Self.DoNewListItemStyleFrameCacheChangeItem;
    ...
    FSearchBarItemStyleSetting.FOnChangeItem:=Self.DoNewListItemStyleFrameCacheChangeItem;



    FListItemTypeStyleSettingList:=TListItemTypeStyleSettingList.Create();
...
//  AItem.FDrawListItemTypeStyleSetting:=nil;
    ...
    begin
      AListItemTypeStyleSetting:=FListItemTypeStyleSettingList.FindByStyle(AItem.ItemStyle);
      if AListItemTypeStyleSetting=nil then
      begin
        AListItemTypeStyleSetting:=TListItemTypeStyleSetting.Create(Self,sitUseDrawItemDesignerPanel);
        AListItemTypeStyleSetting.Style:=AItem.ItemStyle;
        AListItemTypeStyleSetting.FOnInit:=Self.DoNewListItemStyleFrameCacheInit;
        AListItemTypeStyleSetting.FOnChangeItem:=Self.DoNewListItemStyleFrameCacheChangeItem;
        FListItemTypeStyleSettingList.Add(AListItemTypeStyleSetting);
      end;
      Result:=AListItemTypeStyleSetting.GetInnerItemDesignerPanel(AItem);
      Exit;
    end;
...
procedure TVirtualListProperties.DoItemPropChange(Sender: TObject);
begin
  inherited;

  if Sender<>nil then
  begin
    SetItemAutoSize(TSkinItem(Sender));
  end;
...
procedure TVirtualListProperties.DoNewListItemStyleFrameCacheChangeItem(
  Sender: TObject; AListItemTypeStyleSetting: TListItemTypeStyleSetting;
  ANewListItemStyleFrameCache: TListItemStyleFrameCache);
begin
  if Assigned(Self.FSkinVirtualListIntf.OnNewListItemStyleFrameCacheChangeItem) then
  begin
    Self.FSkinVirtualListIntf.OnNewListItemStyleFrameCacheChangeItem(Sender,AListItemTypeStyleSetting,ANewListItemStyleFrameCache);
  end;
end;
...
function TVirtualListProperties.CalcItemAutoSize(AItem: TSkinItem;const ABottomSpace:TControlSize=10;const ARightSpace:TControlSize=20): TSizeF;
var
  AItemDrawRectF:TRectF;
  AVirtualListMaterial:TSkinVirtualListDefaultMaterial;
  AListItemTypeStyleSetting:TListItemTypeStyleSetting;
begin
  //默认值
  //不自动尺寸
  ...
  AItemDrawRectF:=RectF(0,
                      0,
                      Self.CalcItemWidth(AItem),
                      Self.CalcItemHeight(AItem)
                      );

//  if (not AItem.AutoSizeWidth) and (not AItem.AutoSizeHeight) then
//  begin
//  end;

  if AItem.AutoSizeWidth then
  begin
    AItemDrawRectF.Width:=1000;
  end;

  if AItem.AutoSizeHeight then
  begin
    AItemDrawRectF.Height:=1000;
  end;

  AListItemTypeStyleSetting:=Self.FListItemTypeStyleSettingList.FindByItemType(AItem.ItemType);
  if (AListItemTypeStyleSetting<>nil)
    and ((AListItemTypeStyleSetting.ItemDesignerPanel<>nil) or (AListItemTypeStyleSetting.Style<>'')) then
  begin
      //使用设计面板进行计算，一般只计算高度
      Result:=AListItemTypeStyleSetting.CalcItemAutoSize(AItem,AItemDrawRectF,ABottomSpace);
  end
  else
  begin
      //使用自带素材进行计算
      AVirtualListMaterial:=TSkinVirtualListDefaultMaterial(Self.FSkinControlIntf.GetCurrentUseMaterial);
      if AVirtualListMaterial<>nil then
      begin
        Result:=uSkinBufferBitmap.GetStringSize(AItem.Caption,AItemDrawRectF,AVirtualListMaterial.DrawItemCaptionParam);
      end;
  ...
  if AItem.AutoSizeWidth then
  begin
    Result.cx:=Result.cx+ARightSpace;
  end;

  if AItem.AutoSizeHeight then
  begin
    Result.cy:=Result.cy+ABottomSpace;
  end;


end;

procedure TVirtualListProperties.CalcItemsAutoSize;
begin
...
begin
  ...
  begin
    Self.FSkinVirtualListIntf.OnAdvancedDrawItem(Self,
                  ACanvas,
                  {$IFDEF FMX}TSkinFMXItemDesignerPanel{$ELSE}TSkinItemDesignerPanel{$ENDIF}(TSkinItem(AItem).FDrawItemDesignerPanel),
                  TSkinItem(AItem),
...
function TVirtualListProperties.GetItemStyleByItemType(AItemType: TSkinItemType): String;
begin
  Result:='';
...
procedure TVirtualListProperties.SetItemAutoSize(AItem: TSkinItem);
var
  AItemSize:TSizeF;
begin
    if AItem.AutoSizeWidth or AItem.AutoSizeHeight then
    begin
      AItemSize:=Self.CalcItemAutoSize(AItem);

      if AItem.AutoSizeWidth then
      begin
        //AItem.FWidth:=AItemSize.Width;
        AItem.FWidth:=AItemSize.cx;
      end;

      if AItem.AutoSizeHeight then
      begin
        //AItem.FHeight:=AItemSize.Height;
        AItem.FHeight:=AItemSize.cy;
      end;
...
procedure TVirtualListProperties.SetItemColorType(
  const Value: TSkinItemColorType);
begin
  if FItemColorType<>Value then
  begin
    FItemColorType := Value;

    Self.FItemColorFieldName:=GetBindItemFieldNameByItemColorType(Value);
    Self.FItemColorFieldSetType:=GetBindItemColorSetTypeByItemColorType(Value);
...
procedure TVirtualListProperties.SetItemsAutoSize;
var
  I: Integer;
begin
  ...
  try
    ...
    begin
      Self.SetItemAutoSize(Self.Items[I]);
    end;
...
procedure TVirtualListProperties.LoadFromJson(ASuperObject: ISuperObject);
var
  I: Integer;
  AConfig:TStringList;
begin
...
procedure TVirtualListProperties.SaveToJson(ASuperObject: ISuperObject);
begin
  inherited;
...
begin
  //结束编辑 只读情况下,ComboEdit不能这么干,ClosePopup之后，都还没有把值赋给Text
...
begin
  ...
  if    PtInRectF(AItemDrawRect,PointF(X,Y))
...
begin
  ...
  //先将所有缓存标记为不使用
  Self.FSkinVirtualListIntf.Prop.FListItemTypeStyleSettingList.MarkAllCacheNoUsed;
...
//  Self.FSkinVirtualListIntf.Prop.FSearchBarItemStyleSetting.MarkAllCacheNoUsed;
  ...
  begin
    ...
    AListItemTypeStyleSetting:=FSkinVirtualListIntf.Prop.FListItemTypeStyleSettingList.FindByItem(AItem);
    if AListItemTypeStyleSetting<>nil then
    begin
      AListItemTypeStyleSetting.MarkCacheUsed(AItem);
    end;

//    if AItem.ItemStyle<>'' then
//    begin
//      AListItemTypeStyleSetting:=FSkinVirtualListIntf.Prop.FListItemTypeStyleSettingList.FindByStyle(AItem.ItemStyle);
//      if AListItemTypeStyleSetting<>nil then
//      begin
//        AListItemTypeStyleSetting.MarkCacheUsed(AItem);
//      end;
...
begin
  ...
  ASkinVirtualListMaterial.FDrawGroupBackColorParam.StaticEffectStates:=AItemEffectStates;
  ProcessDrawParamDrawAlpha(ASkinVirtualListMaterial.FDrawGroupBackColorParam);

  ASkinVirtualListMaterial.FDrawGroupBackPictureParam.StaticEffectStates:=AItemEffectStates;
  ProcessDrawParamDrawAlpha(ASkinVirtualListMaterial.FDrawGroupBackPictureParam);
...
var
  ...
  ASkinVirtualListMaterial:TSkinVirtualListDefaultMaterial;
  I: Integer;
  ATempItem:TBaseSkinItem;
  AGroupEndItem:TBaseSkinItem;
  AGroupDrawRect:TRectF;
begin
  ...
  begin
      ...
      FIsGroupBegin:=False;
      FIsGroupEnd:=False;
      if (ASkinItem.ItemType<>sitSpace)
        and (ASkinItem.ItemType<>sitRowDevideLine)
        then
      begin
          ...
          if
                //第一个
                //万一第一个Item我不想让它成为分组头怎么办？
                //那就不要让它变成第一个Item，让第一个Item变成sitRowDevideLine
                (AItemIndex=0)
                //后面几个
            or (AItemIndex>0)
                //并且上一个是分隔项
          ...
          if (AItemIndex=Self.FSkinVirtualListIntf.Prop.FListLayoutsManager.GetVisibleItemsCount - 1)
            or (AItemIndex>0) and (
                                  (Self.FSkinVirtualListIntf.Prop.ListLayoutsManager.GetVisibleItems(AItemIndex+1).ItemType=sitSpace)
    //                              or (Self.FSkinVirtualListIntf.Prop.ListLayoutsManager.GetVisibleItems(AItemIndex+1).ItemType=sitSpace)
                                  ) then
          begin
            FIsGroupEnd:=True;
          end;
...
//              end;
              ...
              //找到分组结束
              AGroupEndItem:=nil;
              for I := AItemIndex to Self.FSkinVirtualListIntf.Prop.FListLayoutsManager.GetVisibleItemsCount - 1 do
              begin
                ATempItem:=Self.FSkinCustomListIntf.Prop.FListLayoutsManager.GetVisibleItems(I);
                if (I=Self.FSkinVirtualListIntf.Prop.FListLayoutsManager.GetVisibleItemsCount - 1)
                  or //(I>0) and
                                (
                                (Self.FSkinVirtualListIntf.Prop.ListLayoutsManager.GetVisibleItems(I+1).ItemType=sitSpace)
//                                or (Self.FSkinVirtualListIntf.Prop.ListLayoutsManager.GetVisibleItems(I+1).ItemType=sitSpace)
                                )
//                  or (I>0) and (
//                                (Self.FSkinVirtualListIntf.Prop.ListLayoutsManager.GetVisibleItems(I+1).IsParent=True)
////                                or (Self.FSkinVirtualListIntf.Prop.ListLayoutsManager.GetVisibleItems(I+1).ItemType=sitSpace)
//                                )
                  then
                begin
                  AGroupEndItem:=ATempItem;
                  Break;
                end;
              end;

              //开始画分组背景
              if AGroupEndItem<>nil then
              begin
                AGroupDrawRect:=AItemDrawRect;

                //宽度截止
                AGroupDrawRect.Right:=ADrawRect.Right;

                //高度截止
                AGroupDrawRect.Bottom:=AGroupEndItem.FItemRect.Bottom
...
//                AGroupDrawRect.Bottom:=AGroupEndItem.FItemRect.Bottom-;
                ...
                //绘制分组背景色
                ACanvas.DrawRect(ASkinVirtualListMaterial.FDrawGroupBackColorParam,AGroupDrawRect);

                //绘制分组背景图片
                ACanvas.DrawPicture(ASkinVirtualListMaterial.FDrawGroupBackPictureParam,ASkinVirtualListMaterial.FGroupBackPicture,AGroupDrawRect);
              end;
...
//            AIsDrawItemInteractiveState);
      ...
      begin
              ...
              begin
                AItemDesignerPanel.Prop.ProcessItemBindingControlColor(Self.FSkinVirtualListIntf.Prop.ItemColorType,
                                                                        Self.FSkinVirtualListIntf.Prop.FItemColorFieldName,
                                                                        Self.FSkinVirtualListIntf.Prop.FItemColorFieldSetType,
                                                                        ASkinItem.Color,
                                                                        ATempColor);
              end;
...
//              ACustomListItemPaintOtherData.EditingItemDataType:=Self.FSkinVirtualListIntf.Prop.FEditingItem_DataType;
              ...
              if (Self.FSkinVirtualListIntf.Prop.ItemColorType<>sictNone)
                  and (ASkinItem.Color<>NullColor) then
              begin
                AItemDesignerPanel.Prop.RestoreItemBindingControlColor(Self.FSkinVirtualListIntf.Prop.ItemColorType,
                                                                        Self.FSkinVirtualListIntf.Prop.FItemColorFieldName,
                                                                        Self.FSkinVirtualListIntf.Prop.FItemColorFieldSetType,ASkinItem.Color,ATempColor);
              end;
...
function TSkinVirtualList.GetOnNewListItemStyleFrameCacheChangeItem:TNewListItemStyleFrameCacheInitEvent;
begin
  Result:=FOnNewListItemStyleFrameCacheChangeItem;
end;

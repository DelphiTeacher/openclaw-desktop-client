...
type
  ...
  private
    ...
    procedure SetDrawPositionParam(const Value: TDrawTextParam);
    procedure SetIsClipPercentRect(const Value: Boolean);
    procedure SetIsDrawPosition(const Value: Boolean);
  protected
  ...
  published
    ...
    property IsDrawPosition:Boolean read FIsDrawPosition write SetIsDrawPosition;
    ...
    property DrawPositionParam:TDrawTextParam read FDrawPositionParam write SetDrawPositionParam;
  published
  ...
  protected
    ...
    function DrawPosition(ACanvas:TDrawCanvas;ASkinMaterial:TSkinControlMaterial;const ADrawRect:TRectF):Boolean;
...
//    procedure SetIsReverse(const Value: Boolean);
    ...
    procedure SetMinDrawPercent(const Value: Double);
  public
    constructor Create(AOwner:TComponent);override;
    destructor Destroy;override;
  published
  ...
  published
    property MinDrawPercent:Double read FMinDrawPercent write SetMinDrawPercent;
...
function TSkinProgressBarType.DrawPosition(ACanvas: TDrawCanvas;ASkinMaterial:TSkinControlMaterial; const ADrawRect: TRectF): Boolean;
begin
  if (Self.GetSkinMaterial<>nil) and (Self.GetSkinMaterial.FIsDrawPosition) then
  begin
    ACanvas.DrawText(GetSkinMaterial.FDrawPositionParam,FloatToStr(Self.FSkinProgressBarIntf.Prop.FPosition),ADrawRect)
  end;
end;
...
constructor TSkinProgressBarDefaultType.Create(ASkinControl: TControl);
begin
  inherited;
...
begin
  ...
  if (Self.FSkinControl as ISkinControlMaterial).GetComponentTypeName='HalfCircle' then
  begin
    ...
    //  ATempDrawRect:=ADrawRect;
      ...
      //画前景色
      ADrawPercent:=Self.FSkinProgressBarIntf.Prop.GetPercent;

      if ADrawPercent<=TSkinProgressBarHalfCircleMaterial(ASkinMaterial).FMinDrawPercent then
      begin
        ADrawPercent:=TSkinProgressBarHalfCircleMaterial(ASkinMaterial).FMinDrawPercent;
      end;

      if ADrawPercent>0 then
      begin
        ...
        //半圆
        AOutPathActionItem.StartAngle:=TSkinProgressBarHalfCircleMaterial(ASkinMaterial).FStartAngle;
        AOutPathActionItem.SweepAngle:=TSkinProgressBarHalfCircleMaterial(ASkinMaterial).FMaxAngle*ADrawPercent;
    ...
    DrawPercent(ACanvas,ASkinMaterial,ADrawRect);
    DrawCaption(ACanvas,ASkinMaterial,ADrawRect);
    DrawPosition(ACanvas,ASkinMaterial,ADrawRect);
    ...
    ////    AInnerPathActionItem.Y1:=AOutPathActionItem.Y1-AOffset;
    ////    //要反方向画,不然填充不了扇形
    ////    AInnerPathActionItem.StartAngle:=TSkinProgressBarHalfCircleMaterial(ASkinMaterial).FStartAngle+TSkinProgressBarHalfCircleMaterial(ASkinMaterial).FMaxAngle*ADrawPercent;
    ////    AInnerPathActionItem.SweepAngle:=-TSkinProgressBarHalfCircleMaterial(ASkinMaterial).FMaxAngle*ADrawPercent;
    //
    //
    //
    //
...
destructor TSkinProgressBarDefaultType.Destroy;
begin
...
begin
  ...
  FMaxAngle:=180;
  FStartAngle:=-180;
  FDrawForePathParam:=CreateDrawPathParam('ForeColor','前景颜色路径绘制参数');
  FDrawBackPathParam:=CreateDrawPathParam('BackColor','底部颜色路径绘制参数');
end;
...
procedure TSkinProgressBarDefaultMaterial.SetMinDrawPercent(
  const Value: Double);
begin
  FMinDrawPercent := Value;
end;
...
begin
  Result:=Self.FSkinProgressBarIntf.Prop.Position/(Self.FSkinProgressBarIntf.Prop.Max-Self.FSkinProgressBarIntf.Prop.Min);
  if Result>1 then
  begin
    Result:=1;
  end;
...
begin
  ...
  begin
    ...
    DestObject.FIsDrawPosition:=Self.FIsDrawPosition;
...
begin
  ...
  FDrawPositionParam:=CreateDrawTextParam('DrawPotisionParam','值绘制参数');
...
procedure TSkinProgressBarMaterial.SetDrawPositionParam(const Value: TDrawTextParam);
begin
  FDrawPositionParam.Assign(Value);
end;
...
procedure TSkinProgressBarMaterial.SetIsDrawPosition(const Value: Boolean);
begin
  if FIsDrawPosition<>Value then
  begin
    FIsDrawPosition := Value;
    Self.DoChange;
  end;
end;
...
//  RegisterSkinControlStyle('SkinProgressBar',TSkinProgressBarHalfCircleType,TSkinProgressBarHalfCircleMaterial,TProgressBarProperties,'HalfCircle',False);
  RegisterSkinControlStyle('SkinProgressBar',TSkinProgressBarDefaultType,TSkinProgressBarHalfCircleMaterial,TProgressBarProperties,'HalfCircleTest',False);
  RegisterSkinControlStyle('SkinProgressBar',TSkinProgressBarDefaultType,TSkinProgressBarDefaultMaterial,TProgressBarProperties,'HalfCircle',False);
  RegisterSkinControlStyle('SkinProgressBar',TSkinProgressBarDefaultType,TSkinProgressBarDefaultMaterial,TProgressBarProperties,'Round',False);

...
//  TTreeViewDrawItemEvent=procedure(Sender:TObject;ACanvas:TObject;AItemDesignerPanel:TObject;AItem:TSkinTreeViewItem;AItemDrawRect:TRect) of object;
  ...
  ///   </para>
  /// </summary>
  TTreeViewProperties=class(TListViewProperties)
  protected
    //自动展开
...
//  TSkinTreeViewLayoutsManager=class(TSkinVirtualListLayoutsManager)
  TSkinTreeViewLayoutsManager=class(TSkinListViewLayoutsManager)
  private
...
//    function GetVisibleItems:TSkinBaseItems;override;
  ...
  TSkinTreeViewDefaultMaterial=class(TSkinListViewDefaultMaterial)
  private
    //父节点节点绘制素材
  ...
  TSkinTreeViewDefaultType=class(TSkinListViewDefaultType)
  protected
    FSkinTreeViewIntf:ISkinTreeView;
  protected
  ...
  {$I ComponentPlatformsAttribute.inc}
  TSkinTreeView=class(TSkinListView,ISkinTreeView,ISkinItems)
  private
...
begin
  ...
  begin
    ...
    ListLayoutsManager.FViewType:=lvtList;
    ListLayoutsManager.FItemWidth:=-1;
    ...
    FParentItemStyleSetting.FOnChangeItem:=Self.DoNewListItemStyleFrameCacheChangeItem;
...
procedure TTreeViewProperties.SetSelectedByAllChildSelected;
var
  I: Integer;
begin
  ...
  try
    ...
    begin
      Self.Items[I].SetSelectedByAllChildSelected;
    end;
  finally
...
//  Self.FSkinTreeViewIntf.Prop.FParentItemStyleSetting.MarkAllCacheNoUsed;
  ...
  begin
    ...
    if (AItem.ItemType=sitDefault) and AItem.IsParent then
    begin
...
begin
  ...
  begin
    ...
    AllItems.Add(AItem);
    //如果包含子节点,那么加入子节点列表
    if (AItem.Childs.Count > 0) then
    begin
      DoCalcAllItems(AllItems,AItem.Childs);
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      AVisible:=AItem.Visible;
    end
    else
    begin
      AVisible:=AItems.Parent.Expanded and AItem.Visible;
    end;
    ...
    //如果包含子节点,那么加入子节点列表
    if AVisible and AItem.Expanded and (AItem.Childs.Count > 0) then
    begin
      DoCalcVisibleItems(AItem.Childs);
    end;

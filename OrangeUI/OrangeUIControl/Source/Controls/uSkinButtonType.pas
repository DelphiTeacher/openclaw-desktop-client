...
var
  ...
  AArrowDrawRect:TRectF;
  AOldPictureHorzAlign:TPictureHorzAlign;
  AOldFontHorzAlign:TFontHorzAlign;
  ACurrentPictureWidth,ACurrentPictureHeight:Integer;
begin
...
//        begin
            ...
            begin
                ...
                AIconDrawRect:=GetSkinMaterial.FDrawIconParam.CalcDrawRect(ADrawRect);
                  ...
                  if (GetCurrentIcon.CurrentPicture<>nil) and (GetCurrentIcon.CurrentPicture.FSVGObject<>nil) then
                  begin
                    if AIconDrawRect.Width>AIconDrawRect.Height then
                    begin
                      ACurrentPictureWidth:=Ceil(AIconDrawRect.Height);
                      ACurrentPictureHeight:=Ceil(AIconDrawRect.Height);
                    end
                    else
                    begin
                      ACurrentPictureWidth:=Ceil(AIconDrawRect.Width);
                      ACurrentPictureHeight:=Ceil(AIconDrawRect.Width);
                    end;
                  end
                  else
                  begin
                    ACurrentPictureWidth:=GetCurrentIcon.CurrentPictureWidth;
                    ACurrentPictureHeight:=GetCurrentIcon.CurrentPictureHeight;
                  end;

                CalcImageDrawRect(GetSkinMaterial.FDrawIconParam,
                                  ACurrentPictureWidth,
                                  ACurrentPictureHeight,
                                  AIconDrawRect,
                                  AIconDrawRect
                                  );
            ...
            //有下拉箭头
            if not GetSkinMaterial.ArrowPicture.CurrentPictureIsEmpty then
            begin
                //图标不为空
                AArrowDrawRect:=GetSkinMaterial.FDrawArrowPictureParam.CalcDrawRect(ADrawRect);

                AContentWidth:=AContentWidth+AArrowDrawRect.Width;
            ...
            begin
                ...
                ATempDrawRect.Left:=ATempDrawRect.Right+AIconDrawRect.Width;
                ATempDrawRect.Width:=AArrowDrawRect.Width;
...
procedure TSkinButtonMaterial.SetArrowPicture(const Value: TDrawPicture);
begin
  FArrowPicture.Assign(Value);
end;

procedure TSkinButtonMaterial.SetDrawArrowPictureParam(const Value: TDrawPictureParam);
begin
...
procedure TBaseButtonProperties.SaveToJson(ASuperObject: ISuperObject);
begin
  inherited;
...
procedure TBaseButtonProperties.LoadFromJson(ASuperObject: ISuperObject);
begin
  inherited;
...
//function TSkinButtonList.GetOwnerProperties: TObject;
//begin
//  Result:=nil;
//end;
...
{$IFDEF FPC}
function TBaseSkinButton.GetVisible:Boolean;
begin
  Result:=Visible;
end;
{$ENDIF}
...
function TBaseSkinButton.GetIsMouseOver: Boolean;
begin
  Result:=False;
...
procedure TBaseSkinButton.SetControlValueByBindItemField(
  ...
  AIsDrawItemInteractiveState: Boolean);
begin
  if AFieldName='ItemExpanded' then
  begin
    Self.Prop.FIsPushed:=AFieldValue;
  end
  else
  begin
    Caption:=AFieldValue;
...
initialization
  RegisterClasses([TSkinButton]);
  RegisterClasses([TSkinButtonGroup]);

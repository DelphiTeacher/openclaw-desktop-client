...
//    Result:=TItemGridProperties(TSkinItemGridRowLayoutsManager(GetListLayoutsManager).FProperties).CalcContentWidth;
    Result:=TScrollControlProperties(TSkinItemGridRowLayoutsManager(Owner.GetListLayoutsManager).FVirtualGridProperties).CalcContentWidth;
...
function TSkinItemGridRows.GetDefaultSkinItemClass: TBaseSkinItemClass;
begin
...
begin
  ...
  //绘制单元格内容
  Result:=VarToStr(AItemRow.GetValueByBindItemField(AItemColumn.FBindItemFieldName));
...
//        try
    ...
    //                    begin
                          AValue:=FItems[I].GetValueByBindItemField(TSkinItemGridColumn(ASumFooterList[J]).FBindItemFieldName);
                          if not VarIsNull(AValue) then
                          begin
                            TSkinVirtualGridColumn(ASumFooterList[J]).Footer.SumValue:=
                                  TSkinVirtualGridColumn(ASumFooterList[J]).Footer.SumValue+AValue;
                          end;
    //                    end;
                      except
...
//创建列表项
function TSkinItemGridColumns.GetDefaultSkinItemClass:TBaseSkinItemClass;
begin
  Result:=TSkinItemGridColumn;
end;

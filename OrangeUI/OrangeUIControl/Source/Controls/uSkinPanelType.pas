...
//    function SaveToDocNode(ADocNode:TBTNode20_Class):Boolean;override;
  ...
  TSkinWinPanel=class(TSkinPanel)
  end;
  {$ENDIF VCL}
...
//  TSkinFMXPanel=class(TSkinPanel)
//  end;
  {$ENDIF FMX}
...
procedure TBaseSkinPanel.SetControlObjectByBindItemField(
  const AFieldName: String; const AFieldValue: TObject;
                                              APropertyName:String;
                                              ABindItemFieldSetting:TBindItemFieldSetting;
  ...
  AIsDrawItemInteractiveState: Boolean);
begin
  if (AFieldValue<>nil) and (AFieldValue is TDrawColor) then
  begin
    Self.Material.BackColor.FillColor.FColor:=TDrawColor(AFieldValue).FColor;
    {$IFDEF VCL}
    Self.Material.BackColor.FillColor.FAlpha:=TDrawColor(AFieldValue).FAlpha;
    {$ENDIF}
  end;
end;

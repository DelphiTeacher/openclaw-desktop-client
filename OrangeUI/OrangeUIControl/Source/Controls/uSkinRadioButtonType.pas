...
//    property DrawCheckRectParam:TDrawRectParam read FDrawCheckRectParam write SetDrawCheckRectParam;
//  end;
  TSkinRadioButtonColorMaterial=class(TSkinRadioButtonDefaultMaterial)
  end;
...
//  ACaptionDrawRect:TRectF;
  ...
  AMaterial:TSkinRadioButtonDefaultMaterial;
begin
  AMaterial:=TSkinRadioButtonDefaultMaterial(ASkinMaterial);
  ...
  //begin
    ...
    begin
      if Self.FSkinRadioButtonIntf.Prop.Checked then
        ADrawPicture:=AMaterial.FDisabledCheckedPicture
      else
        ADrawPicture:=AMaterial.FDisabledUnCheckedPicture;
    end
    else if Self.FSkinControlIntf.IsMouseOver and APaintData.IsDrawInteractiveState then
    begin
      if Self.FSkinRadioButtonIntf.Prop.Checked then
        ADrawPicture:=AMaterial.FHoverCheckedPicture
      else
        ADrawPicture:=AMaterial.FHoverUnCheckedPicture;
    end
    else
    begin
      if Self.FSkinRadioButtonIntf.Prop.Checked then
        ADrawPicture:=AMaterial.FNormalCheckedPicture
      else
        ADrawPicture:=AMaterial.FNormalUnCheckedPicture;
    end;
    ...
    if not ADrawPicture.CurrentPictureIsEmpty then
    begin
      //图片模式
      ...
      begin
        ...
        ACanvas.DrawPicture(AMaterial.FDrawPictureParam,ADrawPicture,APictureDrawRect);
        ACaptionDrawRect:=ADrawRect;
        ACaptionDrawRect.Left:=APictureDrawRect.Right+5;
        ACanvas.DrawText(AMaterial.FDrawCaptionParam,Self.FSkinControlIntf.Caption,ACaptionDrawRect);

      end
      else
      begin

        ACanvas.DrawPicture(AMaterial.FDrawPictureParam,ADrawPicture,ADrawRect);
    ...
    begin
      ...
      //begin
        ...
        begin
            ...
            ACanvas.DrawRect(AMaterial.FDrawCheckRectParam,ACheckDrawRect);
            ACanvas.DrawRect(AMaterial.FDrawCheckStateParam,ACheckDrawRect);
        ...
        begin
            ACanvas.DrawRect(AMaterial.FDrawCheckRectParam,ADrawRect);
            ACanvas.DrawRect(AMaterial.FDrawCheckStateParam,ADrawRect);
            ACanvas.DrawText(AMaterial.FDrawCaptionParam,Self.FSkinControlIntf.Caption,ADrawRect);
        end;
...
procedure TSkinRadioButtonDefaultMaterial.SetDrawCheckStateParam(const Value: TDrawRectParam);
begin
  FDrawCheckStateParam.Assign(Value);
end;

procedure TSkinRadioButtonDefaultMaterial.SetDrawCheckRectParam(const Value: TDrawRectParam);
begin

...
//    FContentMargins:TBorderMargins;
    ...
    //用于自定义下拉加载面板的上边距
    function GetContentMarginsTop:Double;virtual;
    function GetContentMarginsBottom:Double;virtual;
  ...
  private
    FEnabledAutoScroll: Boolean;
    procedure SetEnabledAutoScroll(const Value: Boolean);
    {$IFDEF FMX}
...
//    procedure DoCustomStartDrag(var DragObject: TObject);virtual;
    ...
    procedure DoCustomDragDrop(ADragObject:Pointer; const Point: TPointF);virtual;
...
//    FEnableAutoScroll:Boolean;
    //自动滚动动画
    FAutoScrollAnimator:TSkinAnimator;
    FAutoScrollTimer:TTimer;
    procedure DoAutoScrollAnimatorAnimateBegin(Sender:TObject);virtual;
    procedure DoAutoScrollAnimatorAnimate(Sender:TObject);virtual;
    procedure DoAutoScrollAnimatorAnimateEnd(Sender:TObject);virtual;
  ...
  published
    ...
    //是否自动滚动
    property EnabledAutoScroll:Boolean read FEnabledAutoScroll write SetEnabledAutoScroll;
  end;
...
//    procedure Paint;override;
  ...
  protected
    ...
    //当不允许开始拖动的时候,拖动结束，在BeginInternalDrag中调用
...
//    //垂直滚动条
//    property VertScrollBar: TSkinChildScrollBar read FVertScrollBar write SetVertScrollBar;
//    //水平滚动条
//    property HorzScrollBar: TSkinChildScrollBar read FHorzScrollBar write SetHorzScrollBar;
//    //滚动框角
//    property ScrollControlCorner: TSkinChildScrollControlCorner read FScrollControlCorner write SetScrollControlCorner;
...
//      FContentMargins:=TBorderMargins.Create;
//      FContentMargins.SetBounds(0,0,0,0);
      ...
      //自动滚动动画
      FAutoScrollAnimator:=TSkinAnimator.Create(nil);
      FAutoScrollAnimator.OnAnimateBegin:=DoAutoScrollAnimatorAnimateBegin;
      FAutoScrollAnimator.OnAnimate:=DoAutoScrollAnimatorAnimate;
      FAutoScrollAnimator.OnAnimateEnd:=DoAutoScrollAnimatorAnimateEnd;
      FAutoScrollAnimator.Max:=60;

      FAutoScrollTimer:=TTimer.Create(nil);
      FAutoScrollTimer.OnTimer:=DoAutoScrollTimer;
      FAutoScrollTimer.Interval:=5000;
...
begin
  ...
  FreeAndNil(FAutoScrollAnimator);
...
//  FreeAndNil(FContentMargins);

  inherited;
end;

procedure TScrollControlProperties.DoAutoScrollAnimatorAnimate(Sender: TObject);
begin
  //
  Self.FVertControlGestureManager.StaticPosition:=Self.FVertControlGestureManager.StaticPosition
          +(Self.FAutoScrollAnimator.Position-Self.FAutoScrollAnimator.LastPosition);
  Self.Invalidate;
end;

procedure TScrollControlProperties.DoAutoScrollAnimatorAnimateBegin(
  Sender: TObject);
begin
  Self.FVertControlGestureManager.StaticPosition:=0;
end;

procedure TScrollControlProperties.DoAutoScrollAnimatorAnimateEnd(
  Sender: TObject);
begin
  //将顶部Item放在最后面
  ...
  if Self.FEnabledAutoScroll then
  begin
    Self.FAutoScrollTimer.Enabled:=True;
  end;
  Self.FVertControlGestureManager.StaticPosition:=0;
end;

procedure TScrollControlProperties.DoAutoScrollTimer(Sender: TObject);
begin
  Self.FAutoScrollTimer.Enabled:=False;

  if Self.FEnabledAutoScroll then
  begin
    Self.FAutoScrollAnimator.Start;
  end;
end;
  ...
  begin
    ...
    FPullDownRefreshPanelIntf.Prop.
                DoOverRangePosValueChange(Self,
                                          ...
                                          LastValue,
                                          Step,
                                          GetContentMarginsTop,GetContentMarginsBottom,
                                          NewValue,
                                          CanChange);
  end;
...
procedure TScrollControlProperties.DoAutoDragScroll(ADragOverPoint:TPointF);
begin
  case FAutoDragScrollVertDirection of
    isdNone: ;
    isdScrollToMin:
    begin
      Self.FVertControlGestureManager.Position:=Self.FVertControlGestureManager.Position-60;
    end;
    isdScrollToMax:
    begin
      Self.FVertControlGestureManager.Position:=Self.FVertControlGestureManager.Position+60;
  ...
  case FAutoDragScrollHorzDirection of
    ...
    isdScrollToMin:
    begin
      Self.FHorzControlGestureManager.Position:=Self.FVertControlGestureManager.Position-60;
    end;
    isdScrollToMax:
    begin
      Self.FHorzControlGestureManager.Position:=Self.FVertControlGestureManager.Position+60;
...
procedure TScrollControlProperties.DoCustomDragDrop(ADragObject:Pointer; const Point: TPointF);
begin

end;

procedure TScrollControlProperties.DoCustomDragCanceled;
begin

end;

procedure TScrollControlProperties.DoCustomDragEnd;
begin

end;

procedure TScrollControlProperties.DoCustomDragOver(const Data: TObject; const Point: TPointF;var Accept: Boolean);
var
  ATopRect:TRectF;
  ABottomRect:TRectF;
  ALeftRect:TRectF;
...
begin
  ...
  ATopRect:=RectF(0,0,FSkinControl.Width,60);
  ABottomRect:=RectF(0,FSkinControl.Height-60,FSkinControl.Width,FSkinControl.Height);
  ALeftRect:=RectF(0,0,FSkinControl.Height,60);
  ARightRect:=RectF(FSkinControl.Width-60,0,FSkinControl.Width,FSkinControl.Height);
...
procedure TScrollControlProperties.DoAutoDragScrollTimer(Sender:TObject);
begin
...
function TScrollControlProperties.GetContentMarginsBottom: Double;
begin
  Result:=0;
end;

function TScrollControlProperties.GetContentMarginsTop: Double;
begin
  Result:=0;
end;
...
//  Result:=RectF(Self.FContentMargins.Left,
//                Self.FContentMargins.Top,
//                Self.FSkinControlIntf.Width-Self.FContentMargins.Right,
//                Self.FSkinControlIntf.Height-Self.FContentMargins.Bottom-ScreenScaleSize(FClientMarginBottom)
//                );
...
procedure TScrollControlProperties.SetEnabledAutoScroll(const Value: Boolean);
begin
  if FEnabledAutoScroll<>Value then
  begin
    FEnabledAutoScroll:=Value;

    Self.FAutoScrollTimer.Enabled:=FEnabledAutoScroll;
  end;
end;
...
//  end;
  ...
  begin
      ...
      if PtInRectF(AScrollBarRect,PointF(X,Y)) then
      begin
        //在垂直滚动条中
...
begin
  uBaseLog.HandleException(nil,'TSkinScrollControl.DragEnter');
  Inherited;
end;
...
var
  Accept:Boolean;
begin
  uBaseLog.HandleException(nil,'TSkinScrollControl.DragOver');
  Inherited;

  //移动的光标
  Operation:=TDragOperation.Move;
//  DoDragOver(Data,Point);
  Properties.DoCustomDragOver(Data.Source,Point,Accept);

end;

procedure TSkinScrollControl.DragDrop(const Data: TDragObject; const Point: TPointF);
begin
  uBaseLog.HandleException(nil,'TSkinScrollControl.DragDrop');
  Inherited;

  if Properties.FAutoDragScrollTimer<>nil then
  begin
    Properties.FAutoDragScrollTimer.Enabled:=False;
  end;

  Properties.DoCustomDragDrop(@Data,Point);
end;

procedure TSkinScrollControl.DragLeave;
begin
  uBaseLog.HandleException(nil,'TSkinScrollControl.DragLeave');
  Inherited;
end;

procedure TSkinScrollControl.DragEnd;
begin
  uBaseLog.HandleException(nil,'TSkinScrollControl.DragEnd');
  Inherited;

  Properties.DoCustomDragEnd;
...
begin
  Inherited;
  Properties.DoCustomDragCanceled;
  if Properties.FAutoDragScrollTimer<>nil then
  begin
    Properties.FAutoDragScrollTimer.Enabled:=False;
  end;
...
//  TDragState = (dsDragEnter, dsDragLeave, dsDragMove);
  Properties.DoCustomDragOver(Source,PointF(X,Y),Accept);
end;
...
begin
  ...
  Properties.DoCustomDragDrop(Source,PointF(X,Y));
end;
...
begin
  ...
  begin
      ...
      begin
        ...
        Self.FVertScrollBarControlIntf.GetSkinControlType.Paint(FDrawCanvas,ASkinMaterial,AScrollBarRect,FPaintData);
      end;
      ...
      begin
        ...
        Self.FHorzScrollBarControlIntf.GetSkinControlType.Paint(FDrawCanvas,ASkinMaterial,AScrollBarRect,FPaintData);
      end;
      ...
      begin
        ...
        Self.FScrollControlCornerControlIntf.GetSkinControlType.Paint(FDrawCanvas,ASkinMaterial,AScrollBarRect,FPaintData);
      end;
...
initialization
  RegisterClasses([TSkinScrollControl]);
  //滚动控件
  RegisterSkinControlStyle('SkinScrollControl',TSkinScrollControlDefaultType,TSkinScrollControlDefaultMaterial,TScrollControlProperties,Const_Default_ComponentType,True);

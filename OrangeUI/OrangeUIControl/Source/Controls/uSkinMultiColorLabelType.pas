...
//
  ...
  protected
    FOwnerProperties:TMultiColorLabelProperties;
...
//    function GetItemByBindItemFieldName(Name: String): TColorTextItem;
  ...
  public
    ...
    property OwnerProperties:TMultiColorLabelProperties read FOwnerProperties;
    property OwnerProp:TMultiColorLabelProperties read FOwnerProperties;
  public
...
begin
  ...
  begin
    if TColorTexts(Self.Collection).FOwnerProperties<>nil then
    begin
      //自定义大小
      TColorTexts(Self.Collection).FOwnerProperties.AdjustAutoSizeBounds;
      //重绘
      TColorTexts(Self.Collection).FOwnerProperties.Invalidate;
    end;
  end;
end;
...
function TColorTexts._AddRef: {$IFDEF LINUX}HResult{$ELSE}Integer{$ENDIF};
begin
...
function TColorTexts._Release: {$IFDEF LINUX}HResult{$ELSE}Integer{$ENDIF};
begin
  if FOwnerInterface <> nil then
...
function TColorTexts.QueryInterface({$IFDEF FPC}constref{$ELSE}const{$ENDIF} IID: TGUID;

...
//    property DrawSortStatePictureParam:TDrawPictureParam read FDrawSortStatePictureParam write SetDrawSortStatePictureParam;
  ...
  protected
    ...
    //绘制内容,要绘制排序的顺度状态，要用Grid的ColumnMaterial来绘制内容
...
function TSkinColumnHeaderDefaultType.CustomPaint(ACanvas: TDrawCanvas;
...
function TSkinColumnHeaderDefaultType.CustomPaintContent(ACanvas: TDrawCanvas;
  ASkinMaterial: TSkinControlMaterial; const ADrawRect: TRectF;
  APaintData: TPaintData): Boolean;
begin
  Result:=Inherited;
...
begin
  ...
  begin
    ...
    if AColumn.IsUseDefaultGridColumnCaptionParam then
    begin
...
function TSkinColumnHeaderDefaultType.GetPaintCurrentUseMaterial: TSkinControlMaterial;
begin
  if GetGrid<>nil then
  begin
    Result:=TSkinVirtualGrid(GetGrid).CurrentUseMaterialToDefault.ColumnHeaderMaterial;
  end
  else
  begin
    Result:=Inherited;
...
//  //重新计算下需要自动适应的列的宽度
//  if TSkinVirtualGrid(GetGrid).Prop<>nil then
//  begin
...
//end;
//
//function TSkinColumnHeaderItems.GetDefaultSkinItemClass: TBaseSkinItemClass;
//begin
...
begin
  ...
  if AItem is TSkinVirtualGridColumn then
  begin
    if FIsAddAutoFitWidth then//and TSkinVirtualGridColumn(AItem).FIsAutoFitControlWidth then
    begin

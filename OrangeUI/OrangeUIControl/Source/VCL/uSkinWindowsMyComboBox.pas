...
procedure TSkinWinMyComboBox.SetViewType(const Value: TSkinComboBoxViewType);
begin
...
function TSkinWinMyComboBox.GetValue:String;
begin
  Result:='';
...
procedure TSkinWinMyComboBox.SetValue(const Value:String);
begin
...
procedure TSkinWinMyComboBox.SetValues(const Value: TStringList);
begin
...
//是否下拉框弹出了
function TSkinWinMyComboBox.GetIsDropedDown:Boolean;
begin
  Result:=False;
end;

function TSkinWinMyComboBox.GetViewType: TSkinComboBoxViewType;
begin
...
//  edtValue.Top:=100;
  edtValue.Align:=alClient;
  edtValue.AlignWithMargins:=True;
  edtValue.Margins.Right:=20;
...
initialization
  RegisterClasses([TSkinWinMyComboBox]);

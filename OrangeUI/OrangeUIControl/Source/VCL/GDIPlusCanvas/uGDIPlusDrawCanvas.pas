...
//    procedure GetRegion;
  public
    ...
    //添加一个圆
    procedure AddPloygon(const APoints:array of TPointF);override;
    procedure AddBezierLine(const X:Double;const Y:Double;
                    const X1:Double;const Y1:Double;
                    const X2:Double;const Y2:Double;
                    const X3:Double;const Y3:Double
                    );override;
//    //闭合路径
...
//    function IsInRegion(const APoint: TPointF):Boolean;override;
  ...
  public
    ...
    //绘制路径//
    function DrawPathData(ADrawPathData:TBaseDrawPathData;const ADrawRect:TRectF;APathActions:TPathActionCollection):Boolean;override;
    function FillPathData(ADrawPathParam:TDrawPathParam;ADrawPathData:TBaseDrawPathData;const ADrawRect:TRectF;APathActions:TPathActionCollection):Boolean;override;
    //绘制路径
...
//  FGraphics.DrawBeziers(AGPPen,AGPPoints);
  FGraphics.DrawBezier(AGPPen,AGPPoints[0],AGPPoints[1],AGPPoints[2],AGPPoints[3]);
...
//
//          Self.DrawPathData(
//                ADrawPathData,ADrawRect,APathActions
...
//
//          //创建画刷
//          FillPathData(ADrawPathParam,ADrawPathData,ADrawRect,APathActions);
...
//      patAddPolygon:
//      begin
//          ADrawPathData.AddPloygon(APathActionItem.FPoints);
//      end;
//      patAddBezierLine:
//      begin
//          ADrawPathData.AddBezierLine(
//
//                                BDrawRect.Left+APathActionItem.GetX(BDrawRect),
//                                BDrawRect.Top+APathActionItem.GetY(BDrawRect),
//
...
//                                BDrawRect.Top+APathActionItem.GetY2(BDrawRect),
//
//                                BDrawRect.Left+APathActionItem.GetX3(BDrawRect),
//                                BDrawRect.Top+APathActionItem.GetY3(BDrawRect)
//
//              );
//      end;
////      patGetRegion:
...
function TGDIPlusDrawCanvas.DrawPathData(ADrawPathData: TBaseDrawPathData;const ADrawRect:TRectF;APathActions:TPathActionCollection): Boolean;
var
  AGPPen:IGPPen;
  AGPColor:TGPColor;
//  AGPPathData:IGPPathData;
...
//    end;
  ...
  begin
      ...
      case ADrawRectParam.FBrushKind of
        drpbkFill:
        begin
          AGPBrush:=TGPSolidBrush.Create(AGPFillColor);
        end;
        drpbkGradient:
        begin
          AGPFillColor1:=TGPColor_CreateFromColorRef(ADrawRectParam.GradientColor1);
          TGPColor_SetAlpha(AGPFillColor1.FArgb,Ceil(ADrawRectParam.FillGradientColor1.Alpha*ADrawRectParam.DrawAlpha/255));

          AGPBrush := TGPLinearGradientBrush.Create(
                                                    TGPRectF_Create(AFillRect),
                                                    AGPFillColor,
                                                    AGPFillColor1,
...
//        Self.FGraphics.SmoothingMode:=TGPSmoothingMode.SmoothingModeDefault;
  ...
  //画阴影
  //是否需要阴影的效果
  if (ADrawRectParam.ShadowSize>0) and (CDrawRect.Width>0) and (CDrawRect.Height>0) then
  begin
      ...
      //默认应该使用填充色或者边框色来做为阴影颜色
      //创建阴影渐变画刷
//      AShadowGPColor:=TGPColor_CreateFromColorRef(ADrawRectParam.FShadowColor.FColor);
      AShadowGPColor:=TGPColor_CreateFromColorRef(ADrawRectParam.FillDrawColor.FColor);
      TGPColor_SetAlpha(AShadowGPColor.FArgb,50);//ADrawRectParam.FShadowColor.Alpha);

      AShadowGPColor1:=TGPColor_CreateFromColorRef(ADrawRectParam.FShadowColor.FColor);
...
//      end;
      ...
      //中心区水平占比，垂直占比
      AShadowGPBrush.SetFocusScales(1-ADrawRectParam.ShadowSize/CDrawRect.Width*3,1-ADrawRectParam.ShadowSize/CDrawRect.Height*3);
...
function TGDIPlusDrawCanvas.FillPathData(ADrawPathParam:TDrawPathParam;ADrawPathData: TBaseDrawPathData;const ADrawRect:TRectF;APathActions:TPathActionCollection): Boolean;
var
  AGPBrush:IGPBrush;
  AGPColor:TGPColor;
  AGPColor1:TGPColor;
  AAreaRect:TRectF;
//  ARoundRect:TRectF;
  ARoundGradientPath: IGPGraphicsPath;
//  ARoundCenter:TPointF;
//  ARoundRadius:Double;
  ARoundGPBrush:IGPPathGradientBrush;
//  AGPPathData:IGPPathData;
//  AGPGraphicsPath:IGPGraphicsPath;
//begin
//  ACanvas.FCanvas.Stroke.Thickness:=ADrawPathData.PenWidth;
...
//  end;
  ...
  case ADrawPathParam.BrushKind of
    drpbkFill:
    begin
      AGPBrush:=TGPSolidBrush.Create(AGPColor);
    //  AGPPathData:=ADrawPathData.PathData;
    ...
    drpbkGradient:
    begin
      case ADrawPathParam.GradientStyle of
        Linear:
        begin
          ...
          AGPColor1:=TGPColor_CreateFromColorRef(ADrawPathParam.FillGradientColor1.FColor);
          TGPColor_SetAlpha(AGPColor1.FArgb,ADrawPathParam.FillGradientColor1.FAlpha);
          //计算出矩形
          AAreaRect:=APathActions.FAreaRect;
          OffsetRect(AAreaRect,ADrawRect.Left,ADrawRect.Top);
          AGPBrush := TGPLinearGradientBrush.Create(
//                                                    TGPRectF_Create(0,0,300,300),
                                                    TGPRectF_Create(AAreaRect),
                                                    AGPColor,
                                                    AGPColor1,
                                                    LinearGradientModeVertical
                                                  );
//          AGPBrush := TGPPathGradientBrush.Create(TGDIPlusDrawPathData(ADrawPathData).Path);
...
//          FGraphics.SmoothingMode:=SmoothingModeAntiAlias;
          ...
          //如果是path,那么需要计算出中心点,以及半径,画个圆,
          AGPColor1:=TGPColor_CreateFromColorRef(ADrawPathParam.FillGradientColor1.FColor);
          TGPColor_SetAlpha(AGPColor1.FArgb,ADrawPathParam.FillGradientColor1.FAlpha);

//          ARoundCenter.X:=APathActions.FAreaRect.Left+APathActions.FAreaRect.Width/2;
//          ARoundCenter.Y:=APathActions.FAreaRect.Top+APathActions.FAreaRect.Height/2;
...
//          //计算出圆型
//          ARoundRect:=RectF(ARoundCenter.X-ARoundRadius,ARoundCenter.Y-ARoundRadius,ARoundCenter.X+ARoundRadius,ARoundCenter.Y+ARoundRadius);

//          ARoundRect:=APathActions.FAreaRect;
//          if ARoundRect.Width>ARoundRect.Height then
//          begin
//            ARoundRect.Top:=ARoundRect.Top-(ARoundRect.Width-ARoundRect.Height)/2;
//            ARoundRect.Height:=ARoundRect.Width;
//          end
//          else
//          begin
//            ARoundRect.Left:=ARoundRect.Left-(ARoundRect.Height-ARoundRect.Width)/2;
//            ARoundRect.Width:=ARoundRect.Width;
//          end;



          // 创建矩形路径对象
          ARoundGradientPath := TGPGraphicsPath.Create;
        //  ARoundGradientPath.AddRectangle(TGPRectF.Create(10, 10, 100, 100));
          ARoundGradientPath.AddEllipse(TGPRectF_Create(APathActions.FRoundRect));


          ARoundGPBrush:=TGPPathGradientBrush.Create(ARoundGradientPath);
          ARoundGPBrush.CenterColor:=AGPColor1;
          ARoundGPBrush.SetSurroundColors([AGPColor]);
//          ARoundGPBrush.SetFocusScales(0.3,0.3);
          AGPBrush:=ARoundGPBrush;
...
//          FGraphics.SetClip(TGDIPlusDrawPathData(ADrawPathData).ARoundGradientPath);


//          FGraphics.FillEllipse(AGPBrush,TGPRectF_Create(APathActions.FRoundRect));
//          FGraphics.FillRectangle(AGPBrush,TGPRectF_Create(ARoundRect));
          FGraphics.FillPath(AGPBrush,TGDIPlusDrawPathData(ADrawPathData).Path);
//          FGraphics.ResetClip();
...
procedure TGDIPlusDrawPathData.AddBezierLine(const X:Double;const Y:Double;
                    const X1:Double;const Y1:Double;
                    const X2:Double;const Y2:Double;
                    const X3:Double;const Y3:Double);
begin
...
procedure TGDIPlusDrawPathData.AddPloygon(const APoints: array of TPointF);
var
  AGPPoints:array of TGPPointF;
  I: Integer;
begin
//  Path.AddPolygon()(TGPRectF_Create(ARect));
  SetLength(AGPPoints,Length(APoints));
  for I := 0 to Length(APoints)-1 do
  begin
    AGPPoints[I]:=TGPPointF_Create(APoints[I].X,APoints[I].Y);
  end;
  Path.AddPolygon(AGPPoints);
end;
...
procedure TGDIPlusDrawPathData.CurveTo(const X, Y, X1, Y1, X2, Y2: Double);
var
  AGPPoints:array of TGPPointF;
begin
  SetLength(AGPPoints,3);
  AGPPoints[0]:=TGPPointF_Create(X,Y);
  AGPPoints[1]:=TGPPointF_Create(X1, Y1);
  AGPPoints[2]:=TGPPointF_Create(X2, Y2);
  Path.AddCurve(AGPPoints);
end;

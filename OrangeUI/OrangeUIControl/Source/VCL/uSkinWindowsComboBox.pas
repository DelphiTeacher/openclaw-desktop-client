...
//    property OnStartDrag;
  ...
  TSkinComboBox=TSkinWinComboBox;
...
procedure TSkinWinComboBox.SetIsUseSystemStyle(Value:Boolean);
begin
...
function TSkinWinComboBox.GetIsDropedDown:Boolean;
begin
  Result:=Self.DroppedDown;
end;

function TSkinWinComboBox.GetViewType: TSkinComboBoxViewType;
begin
  Result:=TSkinComboBoxViewType.scbvtText;
end;
...
procedure TSkinWinComboBox.SetValues(const Value: TStringList);
begin
  FValues.Assign(Value);
end;

function TSkinWinComboBox.GetValue:String;
begin
  Result:='';
  if (Self.Values.Count>0) and (Self.ItemIndex<>-1) and (Self.Values.Count>Self.ItemIndex) then
  begin
    Result:=Self.Values[Self.ItemIndex];
  end;
end;

procedure TSkinWinComboBox.SetValue(const Value:String);
begin

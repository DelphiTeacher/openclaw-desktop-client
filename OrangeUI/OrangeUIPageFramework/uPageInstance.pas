...
//                                                    var AIsSetted:Boolean) of Object;
  ...
  public
    ...
    procedure DeleteJson(ADataJson:ISuperObject);
  private
    ///////////
    ///
    //获取数据列表相关
...
//                                          var AIsSetted:Boolean);
  ...
  public
    ...
    function DoSaveJson(ARecordDataJson:ISuperObject;var ADesc:String):Boolean;
  public
...
begin
  ...
  Self.FSaveDataSetting.Clear;
  Self.FSaveDataSetting.IsAddRecord:=True;
...
begin
  ...
  Self.FSaveDataSetting.IsAddRecord:=False;
...
begin
  Result:=Self.DataInterface.DelData(
...
//            TTimerTask(ATimerTask).TaskDesc:=AError;
          ...
          begin
            ...
            begin
              ...
              FSaveDataSetting.EditingRecordKeyValue:=Self.FCurrentRecordDataIntfResult.DataJson.V[Self.DataInterface.FKeyFieldName];
            end;
            ...
            begin
              //不一定都是从DataSet中获取的,不一定主键都是fid
              FSaveDataSetting.EditingRecordKeyValue:=Self.FLoadDataIntfResult.DataJson.V[Self.DataInterface.FKeyFieldName];
            end;
...
//          FSaveDataSetting.IsAddRecord:=Self.FLoadDataSetting.IsAddRecord;//AIsAdd;
          ...
          AIsNeedCallDefault:=True;
          Self.FPageStructure.CallSaveDataIntf(Self,FSaveDataIntfResult,FSaveDataSetting,AIsNeedCallDefault);
          if AIsNeedCallDefault and Assigned(Self.OnCustomCallSaveDataIntf) then
          begin
             Self.OnCustomCallSaveDataIntf(Self,
                                            Self,
                                            FSaveDataSetting,
...
//              end;
          ...
          begin
              ...
              //将接口保存到数据库
              if Self.DataInterface.SaveData(FSaveDataSetting,FSaveDataIntfResult) then
              begin
    //              //保存成功,要取出新增记录的fid
    //              if AIsAdd then
    //              begin
...
begin
  ...
  try
      ...
      begin
          ...
          if Self.PageStructure.page_type=Const_PageType_EditPage then
          begin
              //如果保存失败、更新的数据为空,那么不会返回DataJson
              Self.LoadDataJsonToControls(FSaveDataIntfResult.DataJson);
          end;
...
procedure TPageInstance.DeleteJson(ADataJson: ISuperObject);
begin
...
//        end;
    ...
    begin
        ...
        begin
            ...
            if not Self.DataInterface.IsEmpty then
            begin
              FLoadDataTimerTaskEvent.TaskName:=Self.PageStructure.name+'.LoadDataTask';
              FLoadDataTimerTaskEvent.Run();
              Result:=True;
...
begin
  ...
  try
      ...
      Self.CallLoadDataIntf(Self.DataInterface,FLoadDataIntfResult,FLoadDataSetting,Self.PageStructure.load_data_params);
...
function TPageInstance.GetDataInterface: TDataInterface;
var
  ASuperObject:ISuperObject;
begin
  if FDataInterface=nil then
  begin
    if GlobalDataInterfaceClassRegList.Find(Self.FPageStructure.data_intf_class)<>nil then
    begin
      FDataInterface:=GlobalDataInterfaceClassRegList.Find(Self.FPageStructure.data_intf_class).Create;
      ASuperObject:=TSuperObject.Create(Self.FPageStructure.data_intf_setting);
      FDataInterface.LoadFromJson(ASuperObject);
      FDataInterface.Name:=Self.FPageStructure.data_intf_name;
      FDataInterface.FKeyFieldName:=Self.FPageStructure.data_intf_key_field;
    end;
  end;
  Result:=Self.FDataInterface;
end;

function TPageInstance.GetDataInterface2: TDataInterface;
var
  ASuperObject:ISuperObject;
begin
  if FDataInterface2=nil then
  begin
    if GlobalDataInterfaceClassRegList.Find(Self.FPageStructure.data_intf_class2)<>nil then
    begin
      FDataInterface2:=GlobalDataInterfaceClassRegList.Find(Self.FPageStructure.data_intf_class2).Create;
      ASuperObject:=TSuperObject.Create(Self.FPageStructure.data_intf_setting2);
      FDataInterface2.LoadFromJson(ASuperObject);
      FDataInterface2.Name:=Self.FPageStructure.data_intf_name2;
    end;
  end;
  Result:=Self.FDataInterface2;
end;
...
function TPageInstance.DoSaveJson(ARecordDataJson: ISuperObject;var ADesc:String):Boolean;
begin
  Result:=False;
  ADesc:='';
  ...
  try
    ...
    FSaveDataSetting.IsAddRecord:=FLoadDataSetting.IsAddRecord;
    FSaveDataSetting.RecordDataJson:=ARecordDataJson;
    ...
    //将接口保存到数据库
    if not Self.DataInterface.SaveData(FSaveDataSetting,FSaveDataIntfResult) then
    begin
      ADesc:=Self.FSaveDataIntfResult.Desc;
      Exit;
    end;
    ...
    begin
      ADesc:=E.Message;
      uBaseLog.HandleException(E,'TPageInstance.DoSaveJson');
    end;
  end;
...
var
  ...
  AError1:String;
begin
          if AIsAddRecord then
          begin
            if (AddPostInitJson<>nil) then
            begin
              //添加记录使用初始的数据Json
...
function TPageInstance.CallLoadDataIntf(ADataInterface:TDataInterface;
                                        ALoadDataIntfResult:TDataIntfResult;
                                        ALoadDataSetting:TLoadDataSetting;
                                        ALoadDataParams:String
                                        ): Boolean;
...
begin
  ...
  try
      ...
      if (Self.FPageStructure.page_type=Const_PageType_ListPage)
        or (FPageStructure.page_type=Const_PageType_TableManagePage) then
      begin
...
//    ParamValues:TVariantDynArray;
      ...
      if (FPageStructure.page_type=Const_PageType_ListPage)
        or (FPageStructure.page_type=Const_PageType_TableManagePage) then
      begin
...
//              ALoadDataSetting.ParamRecordDataJsonStr:='';
      ...
      else if (FPageStructure.page_type=Const_PageType_EditPage)
            or (FPageStructure.page_type=Const_PageType_ViewPage) then
      begin

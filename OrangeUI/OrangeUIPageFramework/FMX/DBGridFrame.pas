...
//  cxCheckBox,cxLabel,
  ...
  uDrawCanvas, uSkinItems, FMX.Types, FMX.Controls,
...
type
  TFrameDBGrid = class(TFrame,IControlForPageFramework,IPageFrameworkListControl)
    DataSource1: TDataSource;
    RestMemTable1: TRestMemTable;
    dbgGrid: TSkinFMXDBGrid;
...
procedure TFrameDBGrid.btnRefreshClick(Sender: TObject);
begin
  inherited;
...
procedure TFrameDBGrid.btnSaveClick(Sender: TObject);
begin
  inherited;
//  //添加或者修改表格记录
...
procedure TFrameDBGrid.btnSaveGridClick(Sender: TObject);
//var
//  ADesc:String;
begin
  inherited;
...
constructor TFrameDBGrid.Create(AOwner: TComponent);
begin
  inherited;
...
procedure TFrameDBGrid.DeleteRecord(ADataIntfResult:TDataIntfResult);
begin
...
destructor TFrameDBGrid.Destroy;
begin

  inherited;
end;

procedure TFrameDBGrid.DoReturnFrame(AFromFrame: TFrame);
begin

end;

function TFrameDBGrid.GetPostValue(ASetting: TFieldControlSetting;
...
function TFrameDBGrid.GetPropJsonStr: String;
begin

end;

function TFrameDBGrid.GetSelectedRecordList: ISuperArray;
var
  I:Integer;
...
begin
  ...
  try
      ...
      for I := 0 to Self.dbgGrid.Prop.Items.Count-1 do
      begin
        if Self.dbgGrid.Prop.Items[I].Selected then
        begin
...
function TFrameDBGrid.LoadFromFieldControlSetting(
...
//        Exit;
//      end;
      if not TFieldControlSettingMap(AFieldControlSettingMap).GetPageInstance.DataInterface.GetFieldList(AppID,ADesc,ADataJson) then
      begin
...
//      Self.cxGrid1DBTableView1.DataController.CreateAllItems();
      ...
      Self.dbgGrid.Prop.Columns.BeginUpdate;
      try



          //将需要编辑的字段显示出来,不需要的字段隐藏掉
          for I := 0 to Self.dbgGrid.Prop.Columns.Count-1 do
          begin
            AColumn:=Self.dbgGrid.Prop.Columns[I];

            AFieldControlSetting:=APage.MainLayoutControlList.FindByFieldName(AColumn.FieldName);
    //        AColumnSettingJson:=LocateJsonArray(FColumnsSettingArray,'field_name',AColumn.DataBinding.FieldName);
    //        if AColumnSettingJson=nil then
    //        begin
    //          AColumn.Visible:=False;
    ...
    //        end;
          ...
          begin
            ...
            begin
              AColumn:=Self.dbgGrid.Prop.Columns.FindItemByFieldName(AFieldControlSetting.field_name);
              if AColumn=nil then
              begin
                //如果列不存在,则创建
                AColumn:=Self.dbgGrid.Prop.Columns.Add;
                AColumn.FieldName:=AFieldControlSetting.field_name;
    //            AColumn.Caption:=AFieldControlSetting.field_caption;
...
//                TcxCheckBoxProperties(AColumn.Properties).ValueChecked:=1;
    ...
    //      AColumn.Index:=AFieldControlSetting.Index;
        ...
        Self.dbgGrid.Prop.Columns.EndUpdate;
      end;
...
procedure TFrameDBGrid.RestMemTable1AfterInsert(DataSet: TDataSet);
begin
  inherited;
...
procedure TFrameDBGrid.RestMemTable1BeforeDelete(
  DataSet: TDataSet);
//var
//  AIsAdd:Boolean;
//  ADesc:String;
...
procedure TFrameDBGrid.RestMemTable1GetRestDatasetPage(
...
procedure TFrameDBGrid.SaveDataIntfResult(ALoadDataSetting:TLoadDataSetting;ADataIntfResult: TDataIntfResult);
begin
...
procedure TFrameDBGrid.SetControlValue(ASetting: TFieldControlSetting;
...
procedure TFrameDBGrid.SetListControlValue(ASetting: TFieldControlSetting;
  APageDataDir, AImageServerUrl: String; AValue: Variant; AValueCaption: String;
...
procedure TFrameDBGrid.SetOnFocusedRecordChange(
...
procedure TFrameDBGrid.SetPropJsonStr(AJsonStr: String);
var
...
procedure TFrameDBGrid.dbgGridDblClick(Sender: TObject);
begin
  //
...
initialization
  GetGlobalFrameworkComponentTypeClasses.Add('DBGrid',TFrameDBGrid,'DB表格');

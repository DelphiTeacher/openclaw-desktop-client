...
//    DataFunction:TDataFunction;
  ...
  public
    ...
    //页面类型,list_page,edit_page,view_page,custom_page
    page_type:String;
    //平台,web,pc,app,wxapp
    platform:String;
...
//                            var ADesc:String):Boolean;overload;
    ...
    function SavePageStructureToJson(ASuperObject:ISuperObject):Boolean;
  ...
  published
    ...
    property RefMainLayoutControlList:TFieldControlSettingList read FMainLayoutControlList write FMainLayoutControlList;
    //页面其他区域的控件(一些按钮)
...
//                                                    var AIsSetted:Boolean) of Object;
  //初始控件的事件,用于初始其他属性
  TPageFrameCustomInitControlEvent=procedure(AControl:TControl;
  ...
  private
    ...
    //获取数据列表相关
    //调用接口返回
    FDelDataTimerTask:TTimerTaskEvent;
  ...
  private
    ...
    //获取数据列表相关
    //调用接口返回
    FLoadDataTimerTask:TTimerTaskEvent;
...
//    function AlignControls(AParentMarginsLeft:Double=0;AParentMarginsTop:Double=0):Boolean;
  public
    ...
    //加载数据
    function LoadData(ALoadDataSetting:TLoadDataSetting):Boolean;
...
//    property LoadDataIntfResult2:TDataIntfResult read FLoadDataIntfResult2 write SetLoadDataIntfResult2;
  public
    ...
    //自定义初始控件,比如ListView
    procedure DoPageFrameCustomInitControl(AControl:TControl;AFieldControlSetting:TFieldControlSetting);virtual;
    //控件点击事件
  ...
  public
    ...
    procedure DoSaveRecordAction;virtual;
...
//    property OnReturnPage:TNotifyEvent read FOnReturnPage write FOnReturnPage;
  ...
  public
    ...
    //自定义初始控件,比如ListView
    procedure DoPageFrameCustomInitControl(AControl:TControl;AFieldControlSetting:TFieldControlSetting);override;
...
function DoCreateControl(
                        ...
                        APageDataDir:String;
                        AIsDesignTime:Boolean;
                        AOnPageFrameCustomInitControl:TPageFrameCustomInitControlEvent;
...
function SaveToFieldControlSettingList(
...
begin
  uBaseLog.HandleException(nil,'SaveToFieldControlSettingList');
  ...
  begin
      ...
      begin
          ...
          begin
            ...
            uBaseLog.HandleException(nil,'SaveToFieldControlSettingList '+AError);
    //        Exit;
            Continue;
          end;
...
//          AFieldControlSettingList.Add(AFieldControlSetting);
  ...
  begin
    ...
    //        Exit;
          ...
          //父控件
          AFieldControlSetting.ParentFieldControlSetting:=AParentFieldControlSetting;
          if not SaveComponentToFieldControlSetting(ARootParent,AControl,AFieldControlSetting,APageDataDir,AError) then
          begin
            //有时候Selection不保存进去的
            //Exit;
          end;
...
//          AFieldControlSettingList.Add(AFieldControlSetting);
          ...
          //保存子控件列表
          if not SaveToFieldControlSettingList(ARootParent,AControl,AFieldControlSettingList,AFieldControlSetting,APageDataDir,AError) then
          begin
            Exit;
          end;
...
//      end;
      ...
      if AControl.GetInterface(IID_ISkinControlMaterial,ASkinControlMaterialIntf) and (ASkinControlMaterialIntf.SelfOwnMaterial<>nil) then
      begin
...
//                end;
      ...
      if AComponent.GetInterface(IID_IControlForPageFramework,AControlMap.PageFrameworkControlIntf) then
      begin
//        AControlMap.PageFrameworkControlIntf:=AControlForPageFrameworkIntf;
        ...
        //设置自定义的值
        AControlMap.PageFrameworkControlIntf.SetPropJsonStr(AFieldControlSetting.prop);
    ...
    //      begin
              ...
              //赋设计时的值
              AControlMap.PageFrameworkControlIntf.SetControlValue(AFieldControlSetting,APageDataDir,
...
////                  then
////                begin
//                  AControlMap.InputPanel:=TSkinPanel.Create(nil);
//                  AControlMap.InputPanel.Parent:=AParent;
//                  AControlParent:=AControlMap.InputPanel;
//                  TSkinPanel(AControlMap.InputPanel).MaterialUseKind:=TMaterialUseKind.mukRefByStyleName;
//                  TSkinPanel(AControlMap.InputPanel).MaterialName:=Const_ControlStyle_EditPageInputPanelDefault;
////                end;
//
//                //创建提示控件
...
//            AControlMap.Control.OnClick:=AControlMapList.DoControlClick;
//            //设置控件的属性
...
//end;
//
//procedure TPageLayoutControlMapList.DoControlClick(Sender: TObject);
//var
//  I: Integer;
//begin
//  for I := 0 to Self.Count-1 do
...
//          else
//          begin
//              raise Exception.Create('TPageInstance.DoControlClick 不支持此动作'+APageLayoutControlMap.Setting.Action);
//          end;
//
//      end;
//  end;
...
//                  Continue;
    ...
    //                AControlMap.Control:=TControl(AControlMap.Component);
        ...
        //                begin
                          ...
                          TSkinPanel(AControlMap.InputPanel).Caption:=AFieldControlSetting.field_caption;
                          TSkinPanel(AControlMap.InputPanel).SkinControlType;
//                          TSkinPanel(AControlMap.InputPanel).SelfOwnMaterial;
                          TSkinPanel(AControlMap.InputPanel).MaterialName:=Const_ControlStyle_EditPageInputPanelDefault;
                          TSkinPanel(AControlMap.InputPanel).MaterialUseKind:=TMaterialUseKind.mukRefByStyleName;
        //                end;
...
//                      end;
                    ...
                    //点击事件,改在AOnPageFrameCustomInitControl中设置
    ...
    //                SetComponentTop(AControlMap.Component,);
                ...
                //自定义初始控件的属性和事件
                if Assigned(AOnPageFrameCustomInitControl) then
                begin
                  AOnPageFrameCustomInitControl(AControl,AFieldControlSetting);
                end;
    ...
    //    end;
        ...
        begin
            ...
            begin
                ...
                if not DoCreateControl(AOwner,AParent,AFieldControlSetting,AControlMapList,APageDataDir,AIsDesignTime,AOnPageFrameCustomInitCOntrol,
                                        AControlMap,AError) then
                begin
                  Exit;
                end;
...
begin
  ...
  Self.DataInterface.fid:=0;
  Self.DataInterface2.fid:=0;
...
begin
  Result:=GetApplicationPath+'programs\'+Self.program_template_name+'\'+Self.name+'\';
end;
...
begin
  ...
  try
      ...
      //所属的程序模块
      AJson.I['program_template_fid']:=program_template_fid;
      AJson.S['program_template_name']:=program_template_name;


      //所调用的接口
      AJson.I['data_intf_fid']:=data_intf_fid;
      AJson.S['data_intf_name']:=data_intf_name;



      //所调用的接口
      AJson.I['data_intf_fid2']:=data_intf_fid2;
      AJson.S['data_intf_name2']:=data_intf_name2;



      //所属的功能
      AJson.I['function_fid']:=function_fid;
      AJson.S['function_name']:=function_name;
    ...
    //    //保存接口1所需要的参数
    //    save_data_params:String;
      AJson.S['load_data_params']:=load_data_params;//Self.LoadDataParamsJsonArray.AsJSON;
      AJson.S['save_data_params']:=save_data_params;//Self.SaveDataParamsJsonArray.AsJSON;
    ...
    //    //保存接口2所需要的参数
    //    save_data_params2:String;
      AJson.S['load_data_params2']:=load_data_params2;//Self.LoadDataParamsJsonArray2.AsJSON;
      AJson.S['save_data_params2']:=save_data_params2;//Self.SaveDataParamsJsonArray2.AsJSON;



      //默认列表项的显示风格
      AJson.S['default_list_item_style']:=default_list_item_style;
      ...
      //默认列表项的绑定设置
      AJson.S['default_list_item_bindings']:=default_list_item_bindings;
...
function TPage.SavePageStructureToJson(ASuperObject: ISuperObject): Boolean;
var
  I: Integer;
  AControlRecordJson:ISuperObject;
...
begin
  for I := 0 to Self.Count-1 do
  begin
    if Self.Items[I].Setting.field_name<>'' then
    begin
...
//          AControlForPageFrameworkIntf
            Items[I].PageFrameworkControlIntf.SetControlValue(Self.Items[I].Setting,'','','','',nil);
//      end;
...
//  Inherited;
  ...
  FLoadDataTimerTask:=TTimerTaskEvent.Create(nil);
  FLoadDataTimerTask.TaskName:='PageInstance.LoadDataTask';
  FLoadDataTimerTask.OnBegin:=Self.DoLoadDataTaskExecuteBegin;
  FLoadDataTimerTask.OnExecute:=Self.DoLoadDataTaskExecute;
  FLoadDataTimerTask.OnExecuteEnd:=Self.DoLoadDataTaskExecuteEnd;



  FDelDataTimerTask:=TTimerTaskEvent.Create(nil);
  FDelDataTimerTask.TaskName:='PageInstance.DelDataTask';
  FDelDataTimerTask.OnBegin:=Self.DoDelDataTaskExecuteBegin;
  FDelDataTimerTask.OnExecute:=Self.DoDelDataTaskExecute;
  FDelDataTimerTask.OnExecuteEnd:=Self.DoDelDataTaskExecuteEnd;
  ...
  FGetDataIntfResultFieldValue:=TGetDataIntfResultFieldValue.Create(FLoadDataIntfResult,FLoadDataIntfResult2);
...
begin
  ...
  begin
      ...
      Result:=DoCreateControls(
                              ...
                              APageDataDir,
                              AIsDesignTime,
                              Self.DoPageFrameCustomInitControl,
                              Self.MainControlMapList,
                              AError);
//      MainControlMapList.AlignControls;
...
//  FreeAndNil(SaveRecordMessageBox);
  ...
  FreeAndNil(FLoadDataTimerTask);

  FreeAndNil(FDelDataTimerTask);
...
procedure TPageInstance.DoPageFrameCustomInitControl(AControl: TControl;
  AFieldControlSetting: TFieldControlSetting);
begin
  {$IFDEF FMX}
  AControl.OnClick:=Self.DoControlClick;
  {$ENDIF}
...
function TPageInstance.LoadData(ALoadDataSetting:TLoadDataSetting): Boolean;
begin
  Result:=False;
...
//        if not PageStructure.DataInterface.IsEmpty then
//        begin
          FLoadDataTimerTask.TaskName:=Self.PageStructure.name+'.LoadDataTask';
          FLoadDataTimerTask.Run();
          Result:=True;
//        end
//        else
//        begin
...
begin
  ...
  try
      ...
      begin
        //appid,user_fid,key,pageindex,pagesize
...
//      ALoadDataIntfResult.Succ:=(ACode=SUCC);
    ...
    on E:Exception do
    begin
      uBaseLog.HandleException(nil,'CallLoadDataIntf');
    end;
  end;
end;
...
//                                AControlForPageFrameworkIntf
                                  ...
                                  AFieldControlSettingMap.PageFrameworkControlIntf.SetControlValue(AFieldControlSettingMap.Setting,
                                                                        ...
                                                                        AValue,
                                                                        AValueCaption,
                                                                        Self.FGetDataIntfResultFieldValue
                                                                        );
                              end;
...
procedure TPageInstance.DoControlClick(Sender: TObject);
var
  I: Integer;
begin
  uBaseLog.HandleException(nil,'TPageInstance.DoControlClick');
...
begin
  FDelDataTimerTask.TaskName:=Self.PageStructure.name+'.DelDataTask';
  FDelDataTimerTask.Run();
...
begin
  ...
  //排列类型,auto,manual
  AJson.S[name+'_'+'align_type']:=align_type;


  //控件排几列
  AJson.I[name+'_'+'col_count']:=col_count;
  //控件的宽度
  AJson.F[name+'_'+'col_width']:=col_width;
  //控件的高度
  AJson.F[name+'_'+'row_height']:=row_height;
  //控件的间隔
  AJson.F[name+'_'+'row_space']:=row_space;



  //提示文本的宽度
  AJson.F[name+'_'+'hint_label_width']:=hint_label_width;


  //控件的左边距
  AJson.F[name+'_'+'margins_left']:=margins_left;
  AJson.F[name+'_'+'margins_top']:=margins_top;
  AJson.F[name+'_'+'margins_right']:=margins_right;
  AJson.F[name+'_'+'margins_bottom']:=margins_bottom;
...
//  if layout_fid<>0 then AJson.I['layout_fid']:=layout_fid;
  ...
  AJson.S['control_style']:=control_style;
...
//    //图片路径
//    pic_path:String;
  AJson.S['image_kind']:=image_kind;//	int	图片是否需要裁剪
  AJson.I['image_is_need_clip']:=image_is_need_clip;//	int	图片是否需要裁剪
  AJson.I['image_clip_width']:=image_clip_width;//	int	裁剪图片的宽度
  AJson.I['image_clip_height']:=image_clip_height;//	int	裁剪图片的高度
  AJson.I['image_max_count']:=image_max_count;//	int	最多支持添加几张图片,0表示默认1
  //image_upload_url	nvarchar(255)	上传图片的接口地址,
  //比如：http://www.orangeui.cn:10011/upload?appid=1003&filename=%s&filedir=repair_car_order_pic&fileext=.jpg
  AJson.S['other_field_names']:=other_field_names;//	nvarchar(45)	图片宽度的字段名,比如pic1_width,创建这个控件，在选择好图片之后赋宽度给它
  AJson.S['other_field_controlprops']:=other_field_controlprops;//	nvarchar(45)	图片宽度的字段名,比如pic1_width,创建这个控件，在选择好图片之后赋宽度给它
  ...
  AJson.S['input_format']:=input_format;//	nvarchar(45)	输入格式要求，保存的时候要使用,相关的控件是edit,comboedit
  //		number,只允许输入数字
  //		email,必须是email
  //		phone,必须是手机号
  AJson.S['input_prompt']:=input_prompt;//	nvarchar(255)	输入提示,比如请输入密码
  AJson.I['input_max_length']:=input_max_length;//	int	输入字符串的最大长度
  AJson.I['input_read_only']:=input_read_only;//	int	是否只读




  AJson.S['options_value']:=options_value;
  AJson.S['options_caption']:=options_caption;

  AJson.I['options_page_fid']:=options_page_fid;//	int	选择选项的列表页面fid,它里面包含数据接口
  AJson.S['options_page_name']:=options_page_name;//	int	选择选项的列表页面fid,它里面包含数据接口
  AJson.S['options_page_value_field_name']:=options_page_value_field_name;//	nvarchar(45)	选择选项列表页面的值字段
  AJson.S['options_page_caption_field_name']:=options_page_caption_field_name;//	nvarchar(45)	选择选项列表页面的标题字段
  AJson.I['options_has_empty']:=options_has_empty;//	int	是否拥有空的选项
  AJson.S['options_empty_value']:=options_empty_value;//	nvarchar(45)
  AJson.S['options_empty_caption']:=options_empty_caption;//	nvarchar(45)

  AJson.S['options_caption_field_name']:=options_caption_field_name;




  AJson.S['page_part']:=page_part;
  ...
  //子页面
  AJson.I['control_page_fid']:=control_page_fid;
  AJson.S['control_page_name']:=control_page_name;




  AJson.S['jump_to_page_program']:=jump_to_page_program;//	nvarchar(255)	跳转到指定的页面的程序模板name,比如ycliving
  AJson.S['jump_to_page_function']:=jump_to_page_function;//	nvarchar(255)	跳转到指定的页面的功能name,比如shop_goods_manage
  AJson.S['jump_to_page_name']:=jump_to_page_name;//	nvarchar(255)	跳转到指定的页面的页面name,比如goods_list_page
  AJson.S['jump_to_page_type']:=jump_to_page_type;//	nvarchar(255)	跳转到指定的页面的页面类型,list_page

  AJson.I['jump_to_page_fid']:=jump_to_page_fid;//	int	跳转到指定的页面的页面fid,比较直接
...
//  if appid<>0 then
  ...
  //所属页面的FID
//  if page_fid<>0 then
  if page_fid<>AOldSetting.page_fid then AJson.I['page_fid']:=page_fid;
  //布局的FID
//  if layout_fid<>0 then AJson.I['layout_fid']:=layout_fid;
...
//  if parent_control_fid<>0 then
  if parent_control_fid<>AOldSetting.parent_control_fid then AJson.I['parent_control_fid']:=parent_control_fid;


  if parent_control_name<>AOldSetting.parent_control_name then AJson.S['parent_control_name']:=parent_control_name;




  if name<>AOldSetting.name then AJson.S['name']:=name;
  if control_type<>AOldSetting.control_type then AJson.S['control_type']:=control_type;

  if field_name<>AOldSetting.field_name then AJson.S['field_name']:=field_name;


  if value<>AOldSetting.value then AJson.S['value']:=value;
  if visible<>AOldSetting.visible then AJson.I['visible']:=visible;
  if hittest<>AOldSetting.hittest then AJson.I['hittest']:=hittest;
  if enabled<>AOldSetting.enabled then AJson.I['enabled']:=enabled;


  if x<>AOldSetting.x then AJson.F['x']:=x;
  if y<>AOldSetting.y then AJson.F['y']:=y;
  if width<>AOldSetting.width then AJson.F['width']:=width;
  if height<>AOldSetting.height then AJson.F['height']:=height;
  if margins<>AOldSetting.margins then AJson.S['margins']:=margins;


  if anchors<>AOldSetting.anchors then AJson.S['anchors']:=anchors;
  if align<>AOldSetting.align then AJson.S['align']:=align;


  if back_color<>AOldSetting.back_color then AJson.S['back_color']:=back_color;
  if border_color<>AOldSetting.border_color then AJson.S['border_color']:=border_color;
  if border_width<>AOldSetting.border_width then AJson.F['border_width']:=border_width;
  if border_edges<>AOldSetting.border_edges then AJson.S['border_edges']:=border_edges;


  if back_round_width<>AOldSetting.back_round_width then AJson.F['back_round_width']:=back_round_width;
  if back_corners<>AOldSetting.back_corners then AJson.S['back_corners']:=back_corners;



  if text_font_name<>AOldSetting.text_font_name then AJson.S['text_font_name']:=text_font_name;
  if text_font_size<>AOldSetting.text_font_size then AJson.I['text_font_size']:=text_font_size;
  if text_font_color<>AOldSetting.text_font_color then AJson.S['text_font_color']:=text_font_color;
  if text_vert_align<>AOldSetting.text_vert_align then AJson.S['text_vert_align']:=text_vert_align;
  if text_horz_align<>AOldSetting.text_horz_align then AJson.S['text_horz_align']:=text_horz_align;
  if text_style<>AOldSetting.text_style then AJson.S['text_style']:=text_style;
  if text_wordwrap<>AOldSetting.text_wordwrap then AJson.I['text_wordwrap']:=text_wordwrap;



  if picture_is_stretch<>AOldSetting.picture_is_stretch then AJson.I['picture_is_stretch']:=picture_is_stretch;
  if picture_is_autofit<>AOldSetting.picture_is_autofit then AJson.I['picture_is_autofit']:=picture_is_autofit;
  if picture_vert_align<>AOldSetting.picture_vert_align then AJson.S['picture_vert_align']:=picture_vert_align;
  if picture_horz_align<>AOldSetting.picture_horz_align then AJson.S['picture_horz_align']:=picture_horz_align;


  if has_caption_label<>AOldSetting.has_caption_label then AJson.I['has_caption_label']:=has_caption_label;


  if bind_listitem_data_type<>AOldSetting.bind_listitem_data_type then AJson.S['bind_listitem_data_type']:=bind_listitem_data_type;

  if prop<>AOldSetting.prop then AJson.S['prop']:=prop;

  if orderno<>AOldSetting.orderno then AJson.F['orderno']:=orderno;
  if is_deleted<>AOldSetting.is_deleted then AJson.I['is_deleted']:=is_deleted;
...
procedure TListPageInstance.DoPageFrameCustomInitControl(AControl: TControl;
  AFieldControlSetting: TFieldControlSetting);
begin
  inherited;

  if (AControl is TSkinVirtualList) and SameText(AFieldControlSetting.field_name,'RecordList') then
  begin
  ...
  begin
      ...
      begin
          ...
          begin
            ...
            AFieldControlSettingMap.PageFrameworkControlIntf.SetControlValue(
...
begin
  Result:=False;
          ...
          //将接口保存到数据库
          if SaveRecordToServer(GlobalMainProgramSetting.DataIntfServerUrl,//Self.InterfaceUrl,//
...
//  GlobalAppID:=1010;
  ...
  GlobalFrameworkComponentTypeClasses.Add('button',TSkinButton,'按钮');
  GlobalFrameworkComponentTypeClasses.Add('label',TSkinLabel,'文本');
  GlobalFrameworkComponentTypeClasses.Add('panel',TSkinPanel,'面板');
  GlobalFrameworkComponentTypeClasses.Add('image',TSkinImage,'图片');
  GlobalFrameworkComponentTypeClasses.Add('item_designer_panel',TSkinItemDesignerPanel,'列表项设计面板');
  GlobalFrameworkComponentTypeClasses.Add('listview',TSkinListView,'列表视图');

  {$IFDEF FMX}
  GlobalFrameworkComponentTypeClasses.Add('edit',TSkinEdit,'编辑框');
  GlobalFrameworkComponentTypeClasses.Add('memo',TSkinMemo,'备注框');
  GlobalFrameworkComponentTypeClasses.Add('roll_label',TSkinRollLabel,'滑动文本');
  GlobalFrameworkComponentTypeClasses.Add('test_component',TComponent,'测试组件');
  GlobalFrameworkComponentTypeClasses.Add('image_list_edit',TFrameAddPictureListSub,'图片列表编辑框');
  GlobalFrameworkComponentTypeClasses.Add('dateedit',TSkinDateEdit,'日期框');
  GlobalFrameworkComponentTypeClasses.Add('timeedit',TSkinTimeEdit,'时间框');
  GlobalFrameworkComponentTypeClasses.Add('combobox',TSkinComboBox,'下拉框');
  GlobalFrameworkComponentTypeClasses.Add('comboedit',TSkinComboEdit,'下拉编辑框');
  {$ENDIF}



//  GlobalFrameworkComponentTypeClasses.Add('list_page',TSkinComboEdit,'下拉编辑框');
//  GlobalFrameworkComponentTypeClasses.Add('button_group',TSkinButtonGroup);
//  GlobalFrameworkComponentTypeClasses.Add('checkbox',TSkinCheckBox);
//  GlobalFrameworkComponentTypeClasses.Add('radio_button',TSkinRadioButton);
//  GlobalFrameworkComponentTypeClasses.Add('multi_color_label',TSkinMultiColorLabel);
//  GlobalFrameworkComponentTypeClasses.Add('Switch',TSkinSwitch);
//  GlobalFrameworkComponentTypeClasses.Add('DrawPanel',TSkinDrawPanel);
//  GlobalFrameworkComponentTypeClasses.Add('RoundRect',TSkinRoundRect);
//  GlobalFrameworkComponentTypeClasses.Add('TabSheet',TSkinTabSheet);
//  GlobalFrameworkComponentTypeClasses.Add('PageControl',TSkinPageControl);
//  GlobalFrameworkComponentTypeClasses.Add('TrackBar',TSkinTrackBar);
//  GlobalFrameworkComponentTypeClasses.Add('SwitchBar',TSkinSwitchBar);
//  GlobalFrameworkComponentTypeClasses.Add('ProgressBar',TSkinProgressBar);
//  GlobalFrameworkComponentTypeClasses.Add('ScrollBar',TSkinScrollBar);
//  GlobalFrameworkComponentTypeClasses.Add('ScrollBox',TSkinScrollBox);
//  GlobalFrameworkComponentTypeClasses.Add('SwitchPageListPanel',TSkinSwitchPageListPanel);
//  GlobalFrameworkComponentTypeClasses.Add('NotifyNumberIcon',TSkinNotifyNumberIcon);
//  GlobalFrameworkComponentTypeClasses.Add('FrameImage',TSkinFrameImage);
//  GlobalFrameworkComponentTypeClasses.Add('RoundImage',TSkinRoundImage);
//  GlobalFrameworkComponentTypeClasses.Add('ImageListPlayer',TSkinImageListPlayer);
//  GlobalFrameworkComponentTypeClasses.Add('ImageListViewer',TSkinImageListViewer);
//  GlobalFrameworkComponentTypeClasses.Add('listbox',TSkinListBox);
//  GlobalFrameworkComponentTypeClasses.Add('treeview',TSkinTreeView);
//  GlobalFrameworkComponentTypeClasses.Add('itemgrid',TSkinItemGrid);
//  GlobalFrameworkComponentTypeClasses.Add('dbgrid',TSkinDBGrid);
//  GlobalFrameworkComponentTypeClasses.Add('Popup',TSkinPopup);

...
//  end;
  ...
  //引用素材属性编辑器,用于选择公共素材
  TRefMaterialProperty = class(TComponentProperty, ICustomPropertyListDrawing)
  public const
    ItemHeight = 38;
  public
    { ICustomPropertyDrawing }
    procedure ListMeasureWidth(const Value: string; ACanvas: {$IFNDEF MY_PROGRAM_DESIGNER}VCL.Graphics.{$ELSE}FMX.Graphics.{$ENDIF}TCanvas; var AWidth: Integer);
    procedure ListMeasureHeight(const Value: string; ACanvas: {$IFNDEF MY_PROGRAM_DESIGNER}VCL.Graphics.{$ELSE}FMX.Graphics.{$ENDIF}TCanvas; var AHeight: Integer);
    procedure ListDrawValue(const Value: string; ACanvas:{$IFNDEF MY_PROGRAM_DESIGNER}VCL.Graphics.{$ELSE}FMX.Graphics.{$ENDIF}TCanvas; const ARect: TRect; ASelected: Boolean);
  private
...
begin
  ...
  begin
    ...
    begin
      ...
      {$ENDIF}
      {$IFDEF FPC}
      RegisterPropertyEditor(PropertyType, ComponentClasses[I], PropertyName,EditorClass);
      {$ENDIF}
    end;
  end;
end;
...
//    {$IFDEF DELPHIXE8}
    ComponentClasses:=[TSkinWindowsControl,
                        ...
                        TSkinWinMemo,
                        TSkinWinForm//,
//                        nil,TSkinWinComboBox,
//                        TSkinWinDateEdit,
//                        nil
                        ];
//    {$ELSE}
//    SetLength(ComponentClasses,6);
...
//    ComponentClasses[4]:=TSkinWinComboBox;
//    ComponentClasses[5]:=TSkinWinDateEdit;
//    {$ENDIF}
...
begin
  ...
  begin
    ...
    try
      ...
      begin
        //指定名称的组件
        LMaterial := Designer.GetComponent(S) as TComponent;
        //如果控件是素材组件
        if LMaterial is TSkinControlMaterial then
        begin
          ...
          begin
            ...
            //判断是否符合
//            if (AMaterialClass<>nil) and (LMaterial.ClassType = AMaterialClass) then
            if (AMaterialClass<>nil) and (LMaterial is AMaterialClass) then
            begin
              Proc(S);
            end;
...
procedure TRefMaterialProperty.ListDrawValue(const Value: string; ACanvas: {$IFNDEF MY_PROGRAM_DESIGNER}VCL.Graphics.{$ELSE}FMX.Graphics.{$ENDIF}TCanvas; const ARect: TRect; ASelected: Boolean);
...
//      end;
//
//      ACurrentControl:=TControl(List[0]);
//      ATempControl:=TControlClass(ACurrentControl.ClassType).Create(nil);
//      try
//        //找到指定的素材
//        LMaterial := Designer.GetComponent(Value) as TComponent;
//        if LMaterial is TSkinControlMaterial then
//        begin
//          (ATempControl as ISkinControlMaterial).RefMaterial:=TSkinControlMaterial(LMaterial);
//        end;
//        //复制属性
//        (ATempControl as ISkinControl).Properties.Assign((ACurrentControl as ISkinControl).Properties);
//        ATempControl.SetBounds(0,0,ACurrentControl.Width,ACurrentControl.Height);
//
//        (ATempControl as ISkinControl).Caption:=(ACurrentControl as ISkinControl).Caption;
//
//        //生成预览效果图
//        Preview:=MakeSkinControlSnapshot(ATempControl,TAlphaColorRec.White);
//
//
...
procedure TRefMaterialProperty.ListMeasureHeight(const Value: string; ACanvas: {$IFNDEF MY_PROGRAM_DESIGNER}VCL.Graphics.{$ELSE}FMX.Graphics.{$ENDIF}TCanvas; var AHeight: Integer);
//var
//  ACurrentControl:TControl;
//  {$IFNDEF FPC}
//  List: IDesignerSelections;
...
//  ACurrentControl:=TControl(List[0]);
//
//  AHeight:=Ceil(ACurrentControl.Height+20+5);//上面20画样式名称,底下留5显示边框

end;

procedure TRefMaterialProperty.ListMeasureWidth(const Value: string; ACanvas: {$IFNDEF MY_PROGRAM_DESIGNER}VCL.Graphics.{$ELSE}FMX.Graphics.{$ENDIF}TCanvas; var AWidth: Integer);
begin
...
//  AItemDesignerPanel:=nil;
  ...
  begin
      ...
      //看看设计面板是否在ListBox里面
      if AItemDesignerPanel.Parent is TSkinCustomList then
      begin
          ACustomList:=TSkinCustomList(AItemDesignerPanel.Parent);

          if not ACustomList.GetInterface(IID_ISkinItemBindingControl,ASkinItemBindingControlIntf) then
          begin
            inherited GetValues(Proc);
            Exit;
          end;
          ...
          AFieldDefs:=ASkinItemBindingControlIntf.GetBindDataSource.GetFieldDefList(ASkinItemBindingControlIntf.GetBindItemFieldName);
          for I := 0 to AFieldDefs.Count-1 do
          begin
            Proc(AFieldDefs[I].Name);
          end;
          Exit;
...
//      {$ENDIF}
  ...
  if not List[0].GetInterface(IID_ISkinItemBindingControl,ASkinItemBindingControlIntf) then
  begin
    inherited GetValues(Proc);
    Exit;
  end;
...
begin
  ...
  begin
    AItemDesignerPanel:=GetParentItemDesignerPanel(
        ...
        TColorTexts(TColorTextItem(List[0]).Collection).OwnerProp.SkinControl

...
procedure Swap(var a, b: TCoordinate);

procedure Permute(var coordinates: array of TCoordinate; start, n: Integer; var permutations: TArray<TArray<TCoordinate>>);
...
begin
  ...
  begin
    ...
    SetLength(permutations[High(permutations)], n);
    for i := 0 to n - 1 do
      permutations[High(permutations)][i] := coordinates[i];
  end
  else
  begin
    for i := start to n - 1 do
    begin
      Swap(coordinates[start], coordinates[i]);
      Permute(coordinates, start + 1, n, permutations);
      Swap(coordinates[start], coordinates[i]);
    end;
  end;
end;
...
//var
//  coordinates: array[0..2] of TCoordinate;
//  i: Integer;
//begin
//  // 输入经纬度数据
//  coordinates[0].Latitude := 1.23;
//  coordinates[0].Longitude := 4.56;
//  coordinates[1].Latitude := 7.89;
//  coordinates[1].Longitude := 0.12;
//  coordinates[2].Latitude := 3.45;
//  coordinates[2].Longitude := 6.78;
//
//  // 调用函数进行排列
//  Permute(coordinates, 0, Length(coordinates));
//
//  ReadLn;
//end.

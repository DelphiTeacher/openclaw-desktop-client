...
//  end;
  ...
  TSkinJsonTreeViewItem=class(TRealSkinTreeViewItem)
  private
    FJson: ISuperObject;
  public
    //根据绑定的FieldName获取Item的值,然后赋给绑定的控件
...
//
////  //表格数据行
////  TSkinJsonItemGridRow=class(TSkinVirtualGridRow)
////  private
////    FJson: ISuperObject;
////  public
////    //根据绑定的FieldName获取Item的值,然后赋给绑定的控件
...
begin
  inherited;
  //这个判断要注释掉，因为当Json里面为空的时候，如果给AFieldName赋值 ，就会赋值 不了
//  if (FJson<>nil) and Json.Contains(AFieldName) then
//  begin
    FJson.V[AFieldName]:=AValue;
//  end

...
//    DataStr:String;
  ...
  public
    ...
    //保存到日志表
    procedure SaveLogToDB(ALog:String;ALogLevel:TLogLevelType=lltInfo);virtual;abstract;
  public
    //执行任务
...
//          );
    ...
    Result.I['appid']:=StrToIntDef(FAppID,0);
    Result.S['user_fid']:=FUserFID;
    Result.S['user_name']:=FUserName;
    ...
    Result.S['createtime']:=StdDateTimeToStr(CreateTime);
...
//  begin
    Result.S['state']:=GetStateStr(State);//'finished';
    Result.S['end_time']:=StdDateTimeToStr(StopTime);
    //保持临时结果,比如当前搜索中断时的下标、是否有下一页
    Result.S['result_data_json']:=Self.ResultDataJson.AsJSON;
//  end;
...
//    end;
    ...
    ARecordDataJson.I['fail_count']:=FFailCount;
    ...
    ARecordDataJson.S['summary']:=Summary;

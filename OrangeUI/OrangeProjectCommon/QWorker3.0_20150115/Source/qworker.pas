...
{$ENDIF}
  ...
  public
    ...
    procedure Synchronize(AMethod: TThreadMethod); {$IFDEF UNICODE}inline;
{$ENDIF}
...
{$ENDIF}
          ...
          if (FActiveJobFlags and JOB_ANONPROC) <> 0 then
            IUnknown(FActiveJobProc.ProcA)._Release;
      ...
      begin
      if (FTimeout >= FOwner.FireTimeout) then // 加一个随机的2秒延迟，以避免同时释放
...
while not Terminated do
  ...
  case FEvent.WaitFor(ATimeout) of
    wrSignaled:
      begin
      if Assigned(Workers) and (Workers.IdleWorkers = 0) then
        Workers.LookupIdleWorker;
      end;
    wrTimeout:
      begin
      if Assigned(Workers.FSimpleJobs) and (Workers.FSimpleJobs.Count > 0) and

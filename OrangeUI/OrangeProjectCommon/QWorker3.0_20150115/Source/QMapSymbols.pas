...
resourcestring
  SUnsupportSymbolClass = '当前符号集不兼容类型 %s，期望 % 及其子类。';
...
{$ENDIF}
  ...
  var
    ...
    p: PChar;
  begin
  AHelper := TCurrentThreadStackHelper.Create(AThread);
  AHelper.WaitFor;
...
{$IFDEF MSWINDOWS}
              ...
              if AFile.Read(AThunk, SizeOf(IMAGE_THUNK_DATA))=SizeOf(IMAGE_THUNK_DATA) then
                ...
                if AThunk.Ordinal <> 0 then
                  ...
                  if PImageImportByName(@ABuffer[0]).Hint = 0 then
                    begin
                    ACat.Cat(AnsiDecode(@PImageImportByName(@ABuffer[0]).Name[0],
                      -1)).Cat(',');
  ...
  if AHdr.PE32.FileHeader.Machine = IMAGE_FILE_MACHINE_AMD64 then
    ...
    AMapFile.Tag := NativeInt(ReadImports(AFile,
    ...
    AMapFile.Tag := NativeInt(ReadImports(AFile,
      AHdr.PE32.OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_IMPORT]
  ...
  if EnumProcessModules(GetCurrentProcess, @AHandles[0], ACount, ACount) then
    ...
    try
      ...
      OutputDebugString(PChar('Error read symbols '+AFileName+' :'+E.Message));
    end;
    end;
  end;
{$ENDIF}
...
if Count > WCT_MAX_NODE_COUNT then
  begin
  ABuilder.Cat(SpaceChar, 1)
    .Cat(Format(SMoreChainNodes, [Count, WCT_MAX_NODE_COUNT])).Cat(SLineBreak);
  Count := WCT_MAX_NODE_COUNT;
  end;
I:=0;
while I<Count do

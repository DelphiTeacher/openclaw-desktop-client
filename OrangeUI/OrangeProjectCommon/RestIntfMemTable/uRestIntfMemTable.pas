...
type
  ...
  public
    ...
    procedure GetPage(APageIndex:Integer);
    //最后一页
    ...
    procedure LoadDataIntfResult(ADataJson:ISuperObject;ASumRecordCount:Integer;AIsNeedLoadRecordList:Boolean=True);
  public
    constructor Create(AOwner:TComponent);override;
    destructor Destroy;override;

  published
    property OnGetRestDatasetPageChange:TNotifyEvent read FOnChange write FOnChange;
    property OnGetRestDatasetPageBegin:TNotifyEvent read FOnExecuteBegin write FOnExecuteBegin;
    property OnGetRestDatasetPageEnd:TNotifyEvent read FOnExecuteEnd write FOnExecuteEnd;
    //获取表结构的接口链接
...
begin
  ...
  begin
    LoadDataIntfResult(ADataJson,ADataJson.I['SumCount']);
  end;
...
//                        Self
//                        ) and (ACode=SUCC) then
  if SimpleCallAPI('',//'get_patient_base_info_list',
                        nil,
                        FFieldDefsInterfaceUrl,//'http://127.0.0.1:10020/doctor_workstation/',
                        [],
                        [],
...
//    ShowMessage(ADesc);
    ...
    LoadMemTableFeildDefsByFieldDefArray(Self,ADataJson.A['FieldList']);
//    //显示到表格上
//    Self.DataSource1.DataSet:=tblData;
//    cxGrid1DBTableView1.DataController.DataSource
...
procedure TRestMemTable.GetPage(APageIndex: Integer);
begin
...
procedure TRestMemTable.LoadDataIntfResult(ADataJson:ISuperObject;ASumRecordCount:Integer;AIsNeedLoadRecordList:Boolean);
begin

    if (ADataJson<>nil) then
    begin
        SumRecordCount:=ASumRecordCount;//ADataJson.I['SumCount'];

...
//  TTimerThreadCanExecuteEvent=function(Sender:TObject):Boolean of object;
  ...
  private
    ...
    FEndTimerTask:TTimerTask;
  protected
    procedure DoExecuteTask(ATimerTask:TTimerTask);
    procedure DoExecuteTaskEnd(ATimerTask:TTimerTask);
    procedure Execute;override;
    procedure DoExecute;virtual;
    procedure DoExecuteEndEvent;
    procedure TerminatedSet;{$IFDEF DELPHIXE8}override;{$ENDIF}
  public
...
procedure TTimerThread.DoExecuteEndEvent;
var
  ATimerTask: TTimerTask;
begin
  //调用任务结束事件
  ATimerTask:=FEndTimerTask;
...
begin
      FEndTimerTask:=ATimerTask;
      if Assigned(ATimerTask.FOnExecuteEnd) then
      begin
          //同步
          TThread.Synchronize(nil,DoExecuteEndEvent);
      end;

...
function ConvertArrayToStringList(AStringDynArray:TStringDynArray):TStringList;overload;
function ConvertArrayToStringList(AValues:TVariantDynArray):TStringList;overload;
//获取字符串列表Name的数组
...
function SaveStringToFile(AString:String;AFilePath:String{$IFDEF DELPHIXE};AEncoding:TEncoding{$ENDIF}):Boolean;
function GetStringFromFile(AFilePath:String{$IFDEF DELPHIXE};AEncoding:TEncoding{$ENDIF}):String;
{$IFDEF DELPHIXE}
//比2007高的版本
...
begin
  ...
  AList.Delimiter:=ADelimiterChar;
...
function NewDelphiSubString(AStr:String;AIndex_StartWith0:Integer;ALength:Integer):String;
begin
  Result:={$IFDEF DELPHIXE8}AStr.Substring(AIndex_StartWith0,ALength){$ELSE}Copy(AStr,AIndex_StartWith0+1,ALength){$ENDIF};
end;

function NewDelphiStringIndexOfName(ASubStr,AStr:String):Integer;
begin
  Result:={$IFDEF DELPHIXE8}AStr.IndexOf(ASubStr){$ELSE}Pos(ASubStr,AStr){$ENDIF};
end;

function NewDelphiStringIndexOf(ASubStr,AStr:String):Boolean;
begin
  Result:={$IFDEF DELPHIXE8}AStr.IndexOf(ASubStr)<>-1{$ELSE}Pos(ASubStr,AStr)>0{$ENDIF};
end;
...
begin
  //+8时区
  Result:=(AMillionSecondsInterval/1000
                                      {$IFDEF FPC}
                                      {$ELSE}
                                       {$IFDEF DELPHIXE8}
                                       +TTimeZone.Local.UtcOffset.Hours*60*60
...
begin
  ...
  try
    ...
    if w = TextFormatFlag[tfUnicode] then
      ...
      Result := TEncoding.{$IFDEF DELPHIXE8}ANSI{$ELSE}ASCII{$ENDIF};
  finally
    Free;
  end;
...
begin
  Result:=False;
  AStringStream:=TStringStream.Create(AString{$IFDEF DELPHIXE},AEncoding{$ENDIF});
  try
...
begin
  ...
  begin
      ...
      AStringStream:=TStringStream.Create(''{$IFDEF DELPHIXE},AEncoding{$ENDIF});
      try
        {$IFDEF DELPHIXE}

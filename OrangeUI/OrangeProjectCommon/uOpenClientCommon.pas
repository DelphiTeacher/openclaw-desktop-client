...
function GetLatestAutoUpgradeVersion:String;
...
begin
  Self.SaveToINI(uFileCommon.GetApplicationPath+AConfigFileName);//'Config.ini');
end;
...
//  Avertype:string;
  AconfigStr: string;
  AversionObj: ISuperObject;
//  AconfigVersionID: Cardinal;
  AconfigVersion: string;
begin
...
//  {$ENDIF}
  ...
  //【Alpha版 和 Online版 CurrentVersionID必须区分，Online的版本号+1
//  CurrentVersionID:= 2306081330;
  Result:= '1.0.0';

  try

    AconfigStr := GetStringFromFile(GetApplicationPath + 'current_' + 'AutoUpgrade' + '.ver', TEncoding.UTF8);
    AversionObj := TSuperObject.Create(AconfigStr);
//    AconfigVersionID := AversionObj.I['id'];
    AconfigVersion := AversionObj.S['ver'];

    if AconfigVersion > Result then
    begin
//      CurrentVersionID:= AconfigVersionID;
      Result:= AconfigVersion;
    end;

  except
    on E: Exception do
    begin
      uBaseLog.HandleException(E,' 获取AutoUpgrade最新版本号');
    end;
  end;

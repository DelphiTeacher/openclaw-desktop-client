...
//  TMergeJsonKeyStatus=(mjksKeyNotExistsInFromJson,mjksValueNotExistsInFromJson,mjksValueExistsInToJson,mjksMerged);
...
function ConvertVarTypeToFieldType(AVarType:TVarType):TFieldType;
...
//连接两个Json数组
procedure ConcatJsonArray(AJsonArray:ISuperArray;
                        AJson2Array:ISuperArray);
...
function JsonHasValue(AFromJson:ISuperObject;AFieldName:String):Boolean;
...
//合并Json的字符串字段
function MergeJsonStringKey(AFromJson:ISuperObject;AToJson:ISuperObject;AFieldName:String;AIsOverride:Boolean):Boolean;
//合并Json的数组字段
...
function TSuperArrayParseStream(AStream:TStream):ISuperArray;
...
function GetJsonValueHasLevel(AJson:ISuperObject;AName:String):Variant;
function GetJsonArrayHasLevel(AJson:ISuperObject;AName:String):ISuperArray;
function GetJsonTypeHasLevel(AJson:ISuperObject;AName:String):TVarType;
function GetJsonContainsHasLevel(AJson:ISuperObject;AName:String):Boolean;
...
begin
  ...
  repeat
    ...
    if AIndex>0 then
    begin
      AObjName:=Copy(AName,1,AIndex-1);
      AName:=Copy(AName,AIndex+1,MaxInt);
      AJson:=AJson.O[AObjName];
    end
    else
    begin
...
//        Result.O['match'].V[AName]:=AValue;
  ...
  if SameText(AOperator,'NOT EXISTS') then
  begin
      //不存在字段
      Result.O['bool'].A['must_not'].O[Result.O['bool'].A['must_not'].Length].O['exists'].S['field']:=AName;
  end
...
//      end;
  ...
  if SameText(AOperator,'regexp') then
  begin
        Result.O['regexp'].S[AName]:=AValue;
  end
  else
  if SameText(AOperator,'query_string') then
  begin
        Result.O['query_string'].S['default_field']:=AName;
        Result.O['query_string'].S['query']:=AValue;
  end
  else
  if SameText(AOperator,'<>') then
  begin
...
begin
  ...
  try
    ...
    Result := SA(AStingStream.DataString);
  finally
    FreeAndNil(AStingStream);
  end;
end;
...
var
  ...
  AFindJson:ISuperObject;
begin
    Result:=False;
    //判断AFromJson中值是不是为空,为空不合并
    if not AFromJson.Contains(AFieldName) or (AFromJson.A[AFieldName].Length=0) then Exit;


    //直接覆盖
    if AIsOverride
      //AToJson中不存在AFieldName,或者值为空，才添加
      or not AIsOverride and (not AToJson.Contains(AFieldName) or (AToJson.A[AFieldName].Length=0)) then
    begin
      //
      AToJson.A[AFieldName]:=AFromJson.A[AFieldName];
      //覆盖了
      Result:=True;
    end
    else
    begin
      //需要判断是否存在,不存在,则添加
      for I := 0 to AFromJson.A[AFieldName].Length-1 do
      begin
        ARecordJson:=AFromJson.A[AFieldName].O[I];
        AFindJson:=LocateJsonArray(AToJson.A[AFieldName],ACheckDumpKeyName,ARecordJson.V[ACheckDumpKeyName]);
        if AFindJson=nil then
        begin
          //不存在,则添加
          AToJson.A[AFieldName].O[AToJson.A[AFieldName].Length]:=ARecordJson;
        end;
      end;
      Result:=True;
    end;
...
begin
    ...
    begin
      ...
      begin
        ARecordJson:=AFromJson.A[AFieldName].O[I];
        AFindJson:=LocateJsonArray(AToJson.A[AFieldName],ACheckDumpKeyNames,[ARecordJson.V[ACheckDumpKeyNames[0]],ARecordJson.V[ACheckDumpKeyNames[1]]]);
        if AFindJson=nil then
        begin
          //不存在,则添加
          AToJson.A[AFieldName].O[AToJson.A[AFieldName].Length]:=ARecordJson;
...
begin
    ...
    case AFromJson.GetType(AFieldName) of
      ...
      varString:
      begin
        if AFromJson.S[AFieldName]='' then Exit;
      end;
    end;
...
begin
    ...
    begin
      ...
      for I := 0 to AFromJson.A[AFieldName].Length-1 do
      begin
        AValue:=AFromJson.A[AFieldName].V[I];
        AFindIndex:=LocateJsonValueArray(AToJson.A[AFieldName],AValue);
        if AFindIndex=-1 then
        begin
          //不存在,则添加
          AToJson.A[AFieldName].V[AToJson.A[AFieldName].Length]:=AValue;
        end;
      end;
      Result:=True;
    end;
...
begin
    Result:=False;
    //判断AFromJson中值是不是为空,为空不合并
    if not AFromJson.Contains(AFieldName) or (AFromJson.S[AFieldName]='') then Exit;
    ...
    if
      ...
      AIsOverride
      //AToJson中不存在AFieldName,或者值为空，才添加
      or not AIsOverride and (not AToJson.Contains(AFieldName) or (AToJson.S[AFieldName]='')) then
    begin
      //
      AToJson.V[AFieldName]:=AFromJson.V[AFieldName];
      //覆盖了
      Result:=True;
    end;
...
var
  I: Integer;
begin
  for I := 0 to AJson2Array.Length-1 do
  begin
    AJsonArray.V[AJsonArray.Length]:=AJson2Array.V[I];
  end;
end;

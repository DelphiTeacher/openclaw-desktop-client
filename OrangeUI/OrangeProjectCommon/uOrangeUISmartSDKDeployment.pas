...
//    function GetAndroidPackageName(AProjectFilePath:String):String;
    ...
    function SaveAndroidJarListToProject(AAndroidJarList:TStringList;
                                          AProjectFilePath:String;AAndroidPlatform:String):Boolean;
    //把AndroidAar列表保存到工程
...
//                                  AAndroidPackageName:String;
                                  ...
                                  ABatStringList:TStringList;
                                  //高版本的Jdk不再支持1.6
                                  AJavaSourceVersion:String='1.6';
                                  AJavaTargetVersion:String='1.6'
                                  ):Boolean;
...
//                              AJarGenRootDir+'2.Gen_R_Jar_'+AGenJarFileNameNoExt+'.bat',
      ...
      FProjectConfig.SaveAndroidJarListToProject(AJarFileList,AProjectFilePath,AAndroidPlatform);
...
//                                  AAndroidPackageName:String;
                                  ...
                                  ABatStringList:TStringList;
                                  AJavaSourceVersion:String;
                                  AJavaTargetVersion:String
                                  ):Boolean;
var
  I: Integer;
  ANeedJars:String;
...
//  end;
  ...
  //当有多个java文件相互引用需要一起编译的
  //要编译哪些源码,输出到output目录
  ANeedJavas:='';
  for I := 0 to AJavaSourceFiles.Count-1 do
  begin
    ANeedJavas:=ANeedJavas+' "'+AJavaSourceFiles[I]+'"'
  end;
    //-encoding UTF-8加这个是因为，aapt生成的R.java是utf8格式的，但是默认javac编译的时候使用的是gbk格式来加载java代码的 ，会报错
    ABatStringList.Add(
//            '"%JDKDir%\bin\javac" %VERBOSE_FLAG% -g -source 1.6 -target 1.6 -encoding UTF-8 -Xlint:deprecation -cp '
            //高版本的Jdk不再支持1.6
            '"%JDKDir%\bin\javac" %VERBOSE_FLAG% -g -source '+AJavaSourceVersion+' -target '+AJavaTargetVersion+' -encoding UTF-8 -Xlint:deprecation -cp '
//            +' "%ANDROID_PLATFORM%\android.jar;%EMBO_LIB%\fmx.jar;" '
          +' "'+ANeedJars+'" '
...
//{$ENDIF MSWINDOWS}
var
  AFlatFileNames:TArray<String>;
  AFlatFileNamesStr:String;
  I: Integer;
  ACompiledResourcesDir:String;
begin
  Result:=False;
...
//  ABatStringList.Add('ECHO AAndroidManifestXmlFilePath '+AAndroidManifestXmlFilePath);
  ...
  ACompiledResourcesDir:=ExtractFilePath(AProjectResPath)+'CompiledResources';
  {$IFDEF MSWINDOWS}


  if not DirectoryExists(ACompiledResourcesDir) then
  begin
    AAndroidSDKAaptExeFilePath:=AAndroidSDKAaptExeFilePath+'aapt.exe';
      //表示是D10,D11,D12下
      //aapt1
      AGenR_Java_Command:=
    ...
    //"D:\PlatformSDKs_D10_4\android-sdk-windows\build-tools\29.0.3\aapt.exe"
    //  package -f -m
    //  -J "D:\TestDelphi12Package\OrangeSDK_JarGen\Project3\src"
    //  -S "D:\TestDelphi12Package\Android\Release\Project3\res"
    //  -I "D:\PlatformSDKs_D10_4\android-sdk-windows\platforms\android-29\android.jar"
    //  -M "D:\TestDelphi12Package\Android\Release\Project3\AndroidManifest.xml"
    //立即执行
    //ShellExecute不立即生成
  ...
  begin
    //表示是D12.1以上
    //"d:\program files (x86)\embarcadero\studio\23.0\bin\Android\aapt2-8.2.1.exe"
    // link
    //  -o "D:\TestDelphi12Package\Android\Release\Project3\test.apk"
    //  --manifest "D:\TestDelphi12Package\Android\Release\AndroidManifest.xml"
    //  -I "D:\PlatformSDKs_D12.1\android-sdk-windows\platforms\android-33\android.jar"
    //  -R @"D:\TestDelphi12Package\Android\Release\Project3\CompiledResourcesList.txt"
    //  --auto-add-overlay
    //  --java D:\TestDelphi12Package\Android\Release\

  //生成CompiledResourcesList.txt
    //生成CompiledResourcesList.txt
    AFlatFileNames:=TDirectory.GetFiles(ACompiledResourcesDir);
    AFlatFileNamesStr:='';
    for I := 0 to Length(AFlatFileNames)-1 do
    begin
//      AFlatFileNamesStr:=AFlatFileNamesStr+AFlatFileNames[I]+' ';
      AFlatFileNamesStr:=AFlatFileNamesStr+ExtractFileName(AFlatFileNames[I])+' ';
    end;
    SaveStringToFile(AFlatFileNamesStr,ExtractFilePath(AProjectResPath)+'CompiledResourcesList.txt',TEncoding.UTF8);

    //aapt2
//    AAndroidSDKAaptExeFilePath:='d:\program files (x86)\embarcadero\studio\23.0\bin\Android\aapt2-8.2.1.exe';
    AAndroidSDKAaptExeFilePath:=AAndroidSDKAaptExeFilePath+'aapt2-8.2.1.exe';
    AGenR_Java_Command:=//'CD '+ACompiledResourcesDir+#13#10

  //      '"'+AAndroidSDKAaptExeFilePath+'"'
        '"'+AAndroidSDKAaptExeFilePath+'"'
        +' link '
        +' '+'-o '+'"'+ExtractFilePath(AGenResJavaSrcDirPath)+'test.apk'+'"'
        //资源目录
        +' -R '+'@"'+ExtractFilePath(AProjectResPath)+'CompiledResourcesList.txt'+'"'
        //所使用的系统jar
        +' -I '+'"'+AAndroidSystemJarFilePath+'"'
  //      +' '+'-M '+'"'+'AndroidManifest.xml'+'"'
        //AndroidManifest.xml路径,主要使用了里面的包名
        +' --manifest '+'"'+AAndroidManifestXmlFilePath+'"'
        +' --auto-add-overlay '
        //生成的源码目录
        +' --java '+'"'+AGenResJavaSrcDirPath+'"'
  //      +#13#10
  //      +'PAUSE'
        ;
  //立即执行
...
//  AAnsiGenR_Java_Command:=AGenR_Java_Command;
  //WinExec(PAnsiChar(AAnsiGenR_Java_Command),SW_SHOWMAXIMIZED);
  uCommandLineHelper_Copy.ExecuteCommand(AGenR_Java_Command,ACompiledResourcesDir,'生成R.java',AGetCommandLineOutputEvent);//,PI);
...
function GenerateWeiXinJarBatToProject(AJarGenRootDir:String;
                                  ...
                                  //高版本的Jdk不再支持1.6
                                  AJavaSourceVersion:String;
                                  AJavaTargetVersion:String): Boolean;
var
//  AJarGenRootDir:String;
//  ATempJarDir:String;
...
//  ATempJarDir:=AJarGenRootDir+'wxapi'+'\';
  ...
  AJavaSources.WriteBOM:=False;
  try
        AJavaSources.Clear;
        AJavaSources.Assign(AWXEntryActivityLines);
        ReplaceStringList('com.ggggcexx.orangeui',AAndroidPackage,AJavaSources);
        AJavaSources.WriteBOM:=False;
        AJavaSources.SaveToFile(AJavaSourceDir+'WXEntryActivity.java',TEncoding.UTF8);

        AJavaSources.Clear;
        AJavaSources.Assign(AWXPayEntryActivityLines);
        ReplaceStringList('com.ggggcexx.orangeui',AAndroidPackage,AJavaSources);
        AJavaSources.WriteBOM:=False;
        AJavaSources.SaveToFile(AJavaSourceDir+'WXPayEntryActivity.java',TEncoding.UTF8);

        AJavaSources.Clear;
        AJavaSources.Assign(AOnWeixinListenerLines);
        ReplaceStringList('com.ggggcexx.orangeui',AAndroidPackage,AJavaSources);
        AJavaSources.WriteBOM:=False;
        AJavaSources.SaveToFile(AJavaSourceDir+'OnWeixinListener.java',TEncoding.UTF8);
...
//        AWxJavaSourceFiles.Add(AJavaSourceDir+'*.java');
        AWxJavaSourceFiles.Add(AJavaSourceDir+'WXEntryActivity.java');
        AWxJavaSourceFiles.Add(AJavaSourceDir+'WXPayEntryActivity.java');
        AWxJavaSourceFiles.Add(AJavaSourceDir+'OnWeixinListener.java');
...
begin
      ...
      GenerateResJavaBatString(
                          ...
                          //https://twodam.net/android-resource-linking
                          //aapt.exe路径
//                          'C:\Users\Public\Documents\Embarcadero\Studio\17.0\PlatformSDKs\android-sdk-windows\build-tools\22.0.1\aapt.exe',
//                          Self.edtAndroidSDKBuildTools.Text+'\'+'aapt.exe',
...
begin
  ...
  try
      ...
      begin
          DoDeployConfigLog(nil,'GetAndroidSDKSetting Open key "'+AKey+'" fail!');
          Exit;
      end;
      //D:\Program Files (x86)\Embarcadero\Studio\23.0\
      ADelphiRootDir:=AReg.ReadString('RootDir');
      ...
      begin
          ...
          begin
              ...
              //'D:\PlatformSDKs_D11\android-sdk-windows\build-tools\30.0.3\'
              //Delphi 12.1之后 ：
              //为空,要取 d:\program files (x86)\embarcadero\studio\23.0\bin\Android\aapt2-8.2.1.exe
              AAndroidSDKBuildTools:=ExtractFilePath(AReg.ReadString('SDKAaptPath'));
              if AAndroidSDKBuildTools='' then
              begin
                //AAndroidSDKBuildTools:=AReg.ReadString('SystemRoot')+'\build-tools\30.0.3\';
                AAndroidSDKBuildTools:=ADelphiRootDir+'bin\Android\';//aapt2-8.2.1.exe
              end;
...
//      FAndroidJarList.Add(AIniFile.ReadString('AndroidJar',IntToStr(I),''));
      FAndroidAarList.Add(AIniFile.ReadString('AndroidJar',IntToStr(I),''));
    end;
  end;
//  FIsDisableSysJars:=AIniFile.ReadBool('AndroidJar','IsDisableSysJars',False);
  ...
  FAndroidAarPlatform:=AIniFile.ReadString('','AndroidAarPlatform','Android32');
...
function TProjectConfig.SaveAndroidJarListToProject(AAndroidJarList: TStringList; AProjectFilePath: String;AAndroidPlatform:String): Boolean;
var
  I: Integer;
  AXMLNode: IXMLNode;
  AXMLDocument: TXMLDocument;
...
//        <JavaReference Include="OrangeSDK_JarGen\HuaweiScan_FMX_D11\R_JAVA_HuaweiScan_FMX_D11.jar">
//            <ContainerId>ClassesdexFile</ContainerId>
//            <Disabled/>
//        </JavaReference>
//        <JavaReference Include="HuaweiSDK 2.9.0.300\Android\tasks-1.5.2.206.jar">
//            <Disabled/>
      ...
      begin
          ...
          begin
            ...
            begin
              ...
              if AAndroidPlatform='Android64' then
              begin
                AJavaReferenceNode.AddChild('ContainerId').Text:='ClassesdexFile64';
              end;
...
function TProjectConfig.SaveAndroidAarListToProject(AAndroidAarList: TStringList; AProjectFilePath: String;AAndroidPlatform:String): Boolean;
var
  I: Integer;
  AAndroidJarList:TStringList;
...
//                CopyFile(PWideChar(AAndroidAarDirPath+'AndroidManifest.xml'),PWideChar(AAndroidAarDirPath+'AndroidManifest.xml.origin'),False);
          ...
          begin
              //是Jar
              AAndroidJarList.Add(AAndroidAarFilePath);
      ...
      Self.SaveAndroidJarListToProject(AAndroidJarList,AProjectFilePath,AAndroidPlatform);
...
//  AIniFile.WriteString('AndroidJar','---','');
  ...
  AIniFile.WriteString('','AndroidAarPlatform',FAndroidAarPlatform);

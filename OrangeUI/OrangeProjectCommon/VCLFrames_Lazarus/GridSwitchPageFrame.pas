...
//    FPageIndexItemCount:Integer;
  ...
  public
    ...
    FMaxPageIndexItemCount:Integer;
    ...
    procedure SetStaticPageSizeIndex(AValue:Integer);
    { Public declarations }
  end;
...
begin
  ...
  begin
    FOnLoadData(Self,StrToInt(Self.edtPageIndex.Text),GetPageSize);
  end;
...
begin
  if Self.cmbPageSize.Text='全部' then
  begin
    Result:=MaxInt;
  end
  else
...
procedure TFrameGridSwitchPage.SetStaticPageSizeIndex(AValue: Integer);
begin
  Clear_cmbPageSizePropertiesChange;
  try
    cmbPageSize.ItemIndex:=AValue;
  finally
    Restore_cmbPageSizePropertiesChange;
  end;
end;
...
//  begin
      ...
      try
        ...
        if (FPageCount>6) and (Self.FPageIndex-1>(FMaxPageIndexItemCount div 2+1)) then
        begin

          ASkinItem:=Self.lbPageIndexs.Prop.Items.Add;
          ASkinItem.Caption:='...';
          ASkinItem.Name:=IntToStr(FPageIndex-FMaxPageIndexItemCount);

          AStart:=FPageIndex-FMaxPageIndexItemCount div 2;
          AStop:=FPageIndex+FMaxPageIndexItemCount div 2;


          //当前PageIndex和末页超过3个Item,就要显示出...
          if FPageCount-Self.FPageIndex>(FMaxPageIndexItemCount div 2+1) then
          begin
          end
          else
          begin
            //右边能显示全,如共100页,95,96,97,98,99,100
            AStart:=FPageCount-FMaxPageIndexItemCount;
            AStop:=FPageCount-1;
          end;
        ...
        begin
          ...
          AStart:=2;
          AStop:=AStart+FMaxPageIndexItemCount-1;
        end;
        ...
        //当前PageIndex和末页超过3个Item,就要显示出...
        if (AStop<FPageCount-1) and (FPageCount-Self.FPageIndex>(FMaxPageIndexItemCount div 2+1)) then
        begin
          ASkinItem:=Self.lbPageIndexs.Prop.Items.Add;
          ASkinItem.Caption:='...';
          ASkinItem.Name:=IntToStr(FPageIndex+FMaxPageIndexItemCount);
...
begin
  ...
  FPageCount:=Ceil(Self.FSumCount/GetPageSize);
//  Self.lblPageCount.Caption:=IntToStr(Ceil(Self.FSumCount/GetPageSize));

...
//    {$ENDIF}
  ...
  public
    FSQLUpgradeList:TSQLUpgradeList;
    //使用的数据库连接
  ...
  public
    procedure InitSQLUpgradeList;virtual;
...
//  end;
  ...
  public
    ...
    procedure EmptyByUserFID(AUserFID:String);
  ...
  TServiceLogEvent=procedure(Sender:TObject;AError:String) of object;
  //服务工程
  TServiceProject = class
  public
    Name: String;
...
//    FServiceModuleInitProcList: TServiceModuleInitProcList;
  ...
  public
    ...
    EnableWebAPIAccessToken:Boolean;
...
//    LastNoceListCount:Integer;
    ...
    FEnableCheckIPWhiteList:Boolean;
  public
...
//  {$ELSE}
  ...
  //需要检测客户的授权文件是否有效
  {$IFDEF HAS_LICENSE_CHECK}
  if not IsLicenseCheckSucc then
  begin
    ADesc:='授权检测失败';
    Result:=False;
    Exit;
  ...
  if not IsEnableRestAPICheckSign and not IsEnableRestAPICheckAccessToken and not FEnableCheckIPWhiteList then
  begin
    Result:=True;
    Exit;
  end;

//  if Self.FEnableCheckIPWhiteList then
//  begin
//
//  end;

  //先判断白名单 //在白名单里面,则不检测签名
  if FEnableCheckIPWhiteList then//IsEnableRestAPICheckSign then
  begin
...
procedure TServiceProject.DoLog(AMessage: String);
begin
...
begin
  ...
  begin
    FIPWhiteList.LoadFromFile(GetApplicationPath+'IPWhiteList.txt',TEncoding.UTF8);
  end;
...
//      end;
      ...
      Self.WebAPIAccessToken:=AIniFile.ReadString('','WebAPIAccessToken','');
      Self.EnableWebAPIAccessToken:=AIniFile.ReadBool('','EnableWebAPIAccessToken',False);
      Self.FEnableCheckIPWhiteList:=AIniFile.ReadBool('','EnableCheckIPWhiteList',False);
...
//    AIniFile.WriteString('','ServiceDisplayName',Self.ServiceDisplayName);
    ...
    AIniFile.WriteString('','WebAPIAccessToken',Self.WebAPIAccessToken);
    AIniFile.WriteBool('','EnableWebAPIAccessToken',Self.EnableWebAPIAccessToken);
    AIniFile.WriteBool('','EnableCheckIPWhiteList',Self.FEnableCheckIPWhiteList);
...
//type
//  TProtectedkbmMWCustomTransport=class(TkbmMWCustomTransport)
//
//  end;
...
//      {$ELSE}
          ...
          begin
            ...
            //为了支持直接上传下载图片吧 企业微信登录需要能直接从服务端下载一个验证文件
            kbmMWHTTPSysServerTransport1.StreamFormat:='AJAX';
            {$ENDIF}
            ...
            GlobalFilePool:=kbmMWFilePool1;

            sd:=TkbmMWHTTPServiceDefinition(kbmMWServer1.RegisterService(TkbmMWEventHTTPService2,false));
            //必须加GetApplicationPath,不然在Service模式下面访问不到图片
            sd.RootPath[mwhfcHTML]:=GetApplicationPath+'webfiles/';
            sd.RootPath[mwhfcImage]:=GetApplicationPath+'webfiles/images';
            sd.RootPath[mwhfcJavascript]:=GetApplicationPath+'webfiles/js';
            sd.RootPath[mwhfcStyleSheet]:=GetApplicationPath+'webfiles/css';
            sd.RootPath[mwhfcOther]:=GetApplicationPath+'webfiles/files';
...
//      {$ENDIF}
      ...
      begin
          AError:='';
          //'ProgramFrameworkManageDBConfig.ini'
      ...
      begin
          ...
          //在一个数据库模块的情况下,在一个服务中连接上了，在另一个服务中又关闭重新连接不太好
...
//      {$ELSE}
        ...
        //          .Active:=True;
              ...
              //访问密钥
              if EnableWebAPIAccessToken and (WebAPIAccessToken<>'') then
              begin
                Self.FUserAccessTokenList.Add(
                  '1000',
                  'admin',
                  WebAPIAccessToken,
                  '',
              ...
              //定时输出服务端状态,避免服务端没有控制台输出shell连接被中断而关闭
...
begin
  FSQLUpgradeList:=TSQLUpgradeList.Create();
//  {$IFDEF USE_IDHTTPSERVERMODE}
//  //数据库模块
//  FDBModule := TFMXUnidacDatabaseModule.Create;
//  {$ELSE}
...
begin
  ...
  if not DBModule.DBConfig.IsEmpty then
  begin
    InitSQLUpgradeList;

    if Self.FSQLUpgradeList.Count>0 then
    begin
        //升级
        ANowDatabaseVer:='';
        ANewDatabaseVer:='';
    ...
    //                                      +'INSERT INTO tblversion (name,version) SELECT  WHERE NOT EXISTS (SELECT * FROM tblversion); ',
            ...
            if SameText(DBModule.DBConfig.FDBType,'MSSQL')
              or SameText(DBModule.DBConfig.FDBType,'MSSQL2000')
              or SameText(DBModule.DBConfig.FDBType,'SQLSERVER')
              or SameText(DBModule.DBConfig.FDBType,'SQLSERVER2000') then
            begin
              //判断是否存在版本信息表，不存在就创建
              ASQLDBHelper.SelfQuery('if not exists (select * from sysobjects where id = object_id(''tblversion'') and OBJECTPROPERTY(id, ''IsUserTable'') = 1) '
                                      +' CREATE TABLE tblversion(name nvarchar(150),version nvarchar(50)); ',
                                           [],[],asoExec);
            end
            else
            begin
              ASQLDBHelper.SelfQuery('CREATE TABLE IF NOT EXISTS tblversion(name nvarchar(150),version nvarchar(50)); ',
                                           [],[],asoExec);
            end;
            ...
            begin
                ...
                begin
                  ...
                  ANeedUpdateSQL:='insert into tblversion (name,version) values( '+QuotedStr(Self.Name)+','+QuotedStr(ANowDatabaseVer)+');';
                ...
                for I := 0 to Self.FSQLUpgradeList.Count-1 do
                begin
                  if TSQLUpgradeItem(FSQLUpgradeList[I]).ver > ANowDatabaseVer then
                  begin
                      ANeedUpdateSQL:=FSQLUpgradeList[I].SQL;
                      if SameText(DBModule.DBConfig.FDBType,'MSSQL')
                        or SameText(DBModule.DBConfig.FDBType,'MSSQL2000')
                        or SameText(DBModule.DBConfig.FDBType,'SQLSERVER')
                        or SameText(DBModule.DBConfig.FDBType,'SQLSERVER2000')
                        and (FSQLUpgradeList[I].SQLServerSQL<>'') then
                      begin
                        ANeedUpdateSQL:=FSQLUpgradeList[I].SQLServerSQL;
                      end
                      else if SameText(DBModule.DBConfig.FDBType,'MYSQL')
                        and (FSQLUpgradeList[I].MysqlSQL<>'') then
                      begin
                        ANeedUpdateSQL:=FSQLUpgradeList[I].MysqlSQL;
                      end
                      else if SameText(DBModule.DBConfig.FDBType,'SQLite')
                        and (FSQLUpgradeList[I].SQLiteSQL<>'') then
                      begin
                        ANeedUpdateSQL:=FSQLUpgradeList[I].SQLiteSQL;
                      end;

                      ANeedUpdateSQL:=ANeedUpdateSQL;//+#13#10+'update tblversion set version = '+QuotedStr(TSQLUpgradeItem(FSQLUpgradeList[I]).ver)+';';
                      if not ASQLDBHelper.SelfQuery(ANeedUpdateSQL, [], [], asoExec) then
                      begin
                        //数据库连接失败或异常
                      ...
                      ANeedUpdateSQL:='update tblversion set version = '+QuotedStr(TSQLUpgradeItem(FSQLUpgradeList[I]).ver)+';';
                      if not ASQLDBHelper.SelfQuery(ANeedUpdateSQL, [], [], asoExec) then
                      begin
                        //数据库连接失败或异常
...
procedure TKbmMWServiceModule.InitSQLUpgradeList;
begin
end;
...
begin
  ...
  begin
    ...
    {$IFDEF HAS_LICENSE_CHECK}
//    ALicenseInfo:=ISuperObject;
    if not CheckLicense(GetSystemKey('',OEM_ProductName_EN),OEM_ProductName_EN,ALicenseInfo,AMessage) then
    begin
      uBaseLog.HandleError(nil,'TSerivceStartThread.Execute 检测授权失败：'+AMessage);
      FServiceProject.IsLicenseCheckSucc:=False;
      FServiceProject.DoLog('检测授权失败：'+AMessage);
      SleepThread(15*1000);
      continue;
    end
    else
    begin
      FServiceProject.IsLicenseCheckSucc:=True;
    end;
    {$ENDIF}
...
//    ALicenseInfo:=ISuperObject;
    ...
    begin
      ...
      FServiceProject.DoLog('检测授权失败：'+AMessage);
      FServiceProject.IsLicenseCheckSucc:=False;
      FServiceProject.Stop;
    end
    else
    begin
      FServiceProject.IsLicenseCheckSucc:=True;

      //因为私有化服务有Web服务端，如果检测到开着Web服务端,则停止Web服务端
...
procedure TUserAccessTokenList.EmptyByUserFID(AUserFID: String);
var
  I: Integer;
begin
  for I := Self.Count-1 downto 0 do
  begin
    if (Items[I].UserFID=AUserFID) then
    begin
      Items[I].Clear;
    end;
  end;
...
procedure TUserAccessToken.Clear;
begin
  AppID:='';
  UserFID:='';
  IsSystem:=False;

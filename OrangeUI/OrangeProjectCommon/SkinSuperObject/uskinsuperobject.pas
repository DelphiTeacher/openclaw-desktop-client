...
{$ENDIF}
  ...
  ['{8B4D6082-DC4B-450F-94F6-84DE5174DD49}']
    ...
    function GetArrayItemD(const AIndex: Integer): Double;
    procedure PutArrayItemD(const AIndex: Integer; const Value: Double);
    //字符类型的元素
    ...
    property D[const AIndex: Integer]: Double read GetArrayItemD write PutArrayItemD;
    property F[const AIndex: Integer]: Double read GetArrayItemD write PutArrayItemD;
...
{.$ifend}
  ...
  protected
    {$IFDEF FPC}
    function QueryInterface({$IFDEF FPC}constref{$ELSE}const{$ENDIF} IID: TGUID; out Obj): {$IFDEF LINUX}longint{$ELSE}HResult{$ENDIF}; virtual; {$IFNDEF WINDOWS}cdecl{$ELSE}stdcall{$ENDIF};
...
function TSuperObject.GetArrayItemD(const AIndex: Integer): Double;
begin
  Result:=Self.AsArray.D[AIndex];
end;
...
procedure TSuperObject.PutArrayItemD(const AIndex: Integer; const Value: Double);
begin
//  if AIndex>=Self.AsArray.Length then
//  begin
//    Self.AsArray.Add
...
function TSuperObject.QueryInterface({$IFDEF FPC}constref{$ELSE}const{$ENDIF} IID: TGUID; out Obj): {$IFDEF LINUX}longint{$ELSE}HResult{$ENDIF}; {$IFNDEF WINDOWS}cdecl{$ELSE}stdcall{$ENDIF};
begin

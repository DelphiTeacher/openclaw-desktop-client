...
function DESEncryptHEX(StrHEX, KeyHEX, IVHEX: String): String;
function DESDecryptHEX(StrHEX, KeyHEX, IVHEX: String): String;
...
function StringHexToByte(Hex: String): Byte;
var
  I, Res: Byte;
  ch: AnsiChar;
begin
...
begin
  ...
  begin
    temp := String(ByteToStringHex(OutputBytes[I]));
    Result := Result + temp;
  end;
end;
...
begin
  ...
  Result := '';
  //(82, 212, 91, 248, 127, 110, 177, 102, 31, 62, 42, 136, 150, 66, 57, 105)
  //(89, 20, 6, 1, 32, 4, 16, 37)
  //(0, 128, 1, 2, 3, 4, 5, 6)

  //(49, 51, 56, 53, 55, 53, 119, 97, 110, 103, 110, 101, 110, 103)
...
var
  ABytes:TBytes;
begin
  ABytes:=TEncoding.UTF8.GetBytes(Buffer);
  BinToHex(ABytes, PWideChar(Result), Length(ABytes));
end;
...
var
  ABytes:TBytes;
begin
  SetLength(ABytes, Length(Buffer) div 2);
  HexToBin(PWideChar(Buffer), ABytes[0], Length(ABytes));

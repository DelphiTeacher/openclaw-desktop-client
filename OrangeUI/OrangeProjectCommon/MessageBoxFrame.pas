...
//    procedure tmrBringToFrontTimer(Sender: TObject);
    ...
    procedure FrameContext1BeginHide(Sender: TObject);
...
//                            AIsLogInHistory:Boolean=True
  ...
  TMessageBoxForm=class(TfrmBaseFrameParent)
  public
    FMessageBoxFrame:TFrameMessageBox;
    constructor Create(AOwner:TComponent);override;
    destructor Destroy;override;
  end;
...
procedure TFrameMessageBox.FrameContext1BeginHide(Sender: TObject);
begin
  //
end;

procedure TFrameMessageBox.FrameContext1Hide(Sender: TObject);
begin
  //
...
//  Self.BringToFront;
  ...
  //AMessageBoxFrame它的地址不会变
  ShowFrame(TFrame(AMessageBoxFrame),TFrameMessageBox,AParent,nil,nil,nil,nil,
            True,//AIsLogInHistory,
            True,ufsefAlpha,False,nil);
...
constructor TMessageBoxForm.Create(AOwner: TComponent);
begin
  inherited;

  FMessageBoxFrame:=TFrameMessageBox.Create(Self);
  FMessageBoxFrame.Parent:=Self;
  FMessageBoxFrame.Align:=TAlignLayout.Client;
end;

destructor TMessageBoxForm.Destroy;
begin
  FreeAndNil(FMessageBoxFrame);
  inherited;
end;

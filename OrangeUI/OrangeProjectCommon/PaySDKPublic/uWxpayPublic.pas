...
function GetWeiXinXMLNodeText(AXML:String;ANodeName:String):String;
...
begin
  ...
  {$IFDEF ANDROID64}
  //Android 64位 加载XML会报错：List index out of bounds (1).  TStringList object range is 0..-1
//  APosIndex:=Pos('<prepay_id><![CDATA[',AXML);
//  if APosIndex=0 then
//  begin
//    AError:='未找到prepay_id开始节点';
//    Exit;
//  end;
//  ATempStr:=Copy(AXML,APosIndex+Length('<prepay_id><![CDATA['),MaxInt);
//  APosIndex:=Pos(']]></prepay_id>',ATempStr);
//  if APosIndex=0 then
//  begin
//    AError:='未找到prepay_id结束节点';
//    Exit;
//  end;
//  ATempStr:=Copy(ATempStr,1,APosIndex-1);
//  APrepayId:=ATempStr;

  if (GetWeiXinXMLNodeText(AXML,'return_code')<>'SUCCESS')
    or (GetWeiXinXMLNodeText(AXML,'return_msg')<>'OK')
    then
  begin
    //接口调用有错误
    AError:=GetWeiXinXMLNodeText(AXML,'return_msg');
    Exit;
  end;

  //接口调用成功
  if (GetWeiXinXMLNodeText(AXML,'result_code')<>'SUCCESS')
    then
  begin
    AError:=GetWeiXinXMLNodeText(AXML,'err_code_des');
    if AError='' then
    begin
      AError:='result_code不为SUCCESS';
    end;
    Exit;
  end;

  Result:=GetWeiXinXMLNodeText(AXML,'prepay_id');
  if Result='' then
  begin
    AError:='未找到prepay_id节点';
    Exit;
  end;
  ...
  XMLDocument := TXMLDocument.Create({$IFDEF MSWINDOWS}Application{$ELSE}nil{$ENDIF});
  try
        //成功
        //<xml>
...
begin
  Result:='';
  //Android 64位 加载XML会报错：List index out of bounds (1).  TStringList object range is 0..-1
  APosIndex:=Pos('<'+ANodeName+'><![CDATA[',AXML);
  if APosIndex=0 then
  begin
    Exit;
  end;
  ATempStr:=Copy(AXML,APosIndex+Length('<'+ANodeName+'><![CDATA['),MaxInt);
  APosIndex:=Pos(']]></'+ANodeName+'>',ATempStr);
  if APosIndex=0 then
  begin
    Exit;
  end;
...
//  APrepayId:=ATempStr;
  ...
  APrepayId:=GetWeiXinXMLNodeText(AXML,'prepay_id');
  if APrepayId='' then
  begin
    AError:='未找到prepay_id节点';
    Exit;
  ...
  AQRCodePayUrl:=GetWeiXinXMLNodeText(AXML,'code_url');
  ...
  XMLDocument := TXMLDocument.Create({$IFDEF MSWINDOWS}Application{$ELSE}nil{$ENDIF});
//  AStringStream:=TStringStream.Create(AXML,TEncoding.UTF8);
  try
    ...
    //<return_msg><![CDATA[商户号该产品权限未开通，请前往商户平台>产品中心检查后重试]]></return_msg>'#$A'
    //</xml>'
    //<prepay_id><![CDATA[wx051222435826163641f35146eebe3a0001]]></prepay_id>
    ...
    //AStringStream
    XMLDocument.LoadFromXML(AXML);
//    XMLDocument.LoadFromStream(AStringStream);
    XMLNode := XMLDocument.DocumentElement;

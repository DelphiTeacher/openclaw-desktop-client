...
//  uRestIntfMemTable,
  ...
  uSkinListBoxType, FMX.ListBox, FMX.Edit, uSkinFireMonkeyControl,
  FMX.Controls.Presentation, uDrawCanvas;
...
type
  ...
  TFrameGridSwitchPage = class(TFrame)
    btnNextPage: TSkinButton;
    btnPriorPage: TSkinButton;
    edtPageIndex: TEdit;
    cmbPageSize: TComboBox;
    pnlClient: TPanel;
    lblSumCount: TSkinLabel;
    lblGoTo: TSkinLabel;
...
//    FPageIndexItemCount:Integer;
  ...
  TFrameBindDataSourceGridSwitchPage=class(TFrameGridSwitchPage,IBindSkinItemArrayControl)
  private
    FBindDataSource: TBindDataSource;
    procedure SetBindDataSource(const Value: TBindDataSource);
  public
...
{ TFrameBindDataSourceGridSwitchPage }

procedure TFrameBindDataSourceGridSwitchPage.SetBindDataSource(
  const Value: TBindDataSource);
begin
  if FBindDataSource<>Value then
  begin
    if Value<>nil then
    begin
      Value.UnBindControl(Value);
    end;

    FBindDataSource := Value;

    if FBindDataSource<>nil then
    begin
      FBindDataSource.BindControl(Value);
    end;

  end;
end;

procedure TFrameBindDataSourceGridSwitchPage.SetControlArrayByBindItemField(

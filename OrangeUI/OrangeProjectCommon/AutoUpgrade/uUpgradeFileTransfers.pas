...
type
  ...
  TLoadProgress = procedure(Sender: TObject; CurrentSize, ContentLength: DWORD; var ACancel: boolean) of object;
...
function UploadFile(const ASvr: TUpgradeSvr; const AKey, AFileName: string;
    AProgress:TLoadProgress; const AMethod: string = 'PUT'): Integer;
function DownloadFile(const ASvr: TUpgradeSvr;const AKey, AFileName: string;
    AUpdateProcess:TLoadProgress; const AMethod: string = 'GET'): Integer;
...
uses
  //uLogWriter,
  HTTPApp, untCommFuns, RTLConsts, uWinHttps, uLangRes;

// END resource string wizard section
const
  SAGENT_UPGRADE = 'AutoUpgrade/2.0 (%s %d.%d.%d)';
...
const
  ...
  BOUNDARYPART = '--MXSearcher-MULTIPARTS-WINHTTP-EUROPA20';
  CRLF = #13#10;
...
begin
  ...
  FParams.Init(ASvr.addr, ASvr.port, ASvr.IsSSL);
...
//      Method := APIMethod_Upload;
    ...
    FRequest := WinHttpAPI.OpenRequest(FConnection,PChar(AMethod), PChar(API),
    ...
    try
      ...
      if iFileSize > MAXUPDATEFILESIZE then
        raise Exception.Create(SMSG_UploadFILEOVERSize);
     ...
     if not (
        ...
        AddHeader('User-Agent: MXSearcher/2.0 (WinHTTP texst)')) then
        RaiseLastModuleError(WinHttpDll,EWinHTTP);
      ...
      if Assigned(AProgress) then
          AProgress(nil, 0, dTotalLen, bCancel);
      ...
      if Assigned(AProgress) then
        AProgress(nil, dWriteLen, dTotalLen, bCancel);
...
begin
  ...
  sUserAgent := format('AutoUpgrade/1.0 (%s %d.%d.%d)', [String(OSVer.szCSDVersion),
      OSVer.dwMajorVersion, OSVer.dwMinorVersion, OSVer.dwBuildNumber]);

...
//  end;
  ...
  public
    ...
    destructor Destroy;override;
    procedure Clear(AIsFree:Boolean=True;AIsNeedDelete:Boolean=True);override;
    procedure Lock;
    procedure UnLock;
  end;


  //JS调用orangeuiextension.notify传过来的通知
  TCustomNotifyFromJSEvent=procedure(Sender:TObject;AName:String;ADataJsonStr:String) of object;
...
var
  GlobalCustomNotifyFromJS:TCustomNotifyFromJSEvent;
...
procedure CreateGlobalCEFApp(CEFDIR:String);
...
//  CEFDIR:= '\cef93';
  CEFDIR:=PathDelim+CEFDIR;
  {$IFDEF CPUX64}
  CEFDIR:= CEFDIR+'_win64';
  {$ENDIF}
...
procedure TCEFMessageContentList.Clear(AIsFree, AIsNeedDelete: Boolean);
begin
  Lock;
  try
    inherited;

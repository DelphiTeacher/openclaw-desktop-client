...
type
  ...
  public
    ...
    public
      ...
      function save_record_post([kbmMW_Rest('value: "$appid", required: false')] AAppID:String;
      ...
      function GetRecordList([kbmMW_Rest('value: "$appid", required: false')] AAppID:String;
                [kbmMW_Rest('value: "$user_fid", required: false')] AUserFID:String;
                [kbmMW_Rest('value: "$key", required: false')] const AKey:String;
                [kbmMW_Rest('value: "$pageindex", required: false')] APageIndex:Integer;
                [kbmMW_Rest('value: "$pagesize", required: false')] APageSize:Integer;
                [kbmMW_Rest('value: "$rest_name", required: false')] const ARestName:String;
                ...
                [kbmMW_Rest('value: "$group_by", required: false')] const AGroupBy:String;
                [kbmMW_Rest('value: "$group_by_summary", required: false')] const AGroupBySummary:String;
...
//                [kbmMW_Rest('value: "$params_json", required: false')] const params_json:String;
                //为了支持积木报表
                [kbmMW_Rest('value: "$pageNo", required: false')] const AJimuPageNo:String;
                [kbmMW_Rest('value: "$pageSize", required: false')] const AJimuPageSize:String;
                //返回格式
                [kbmMW_Rest('value: "$format", required: false')] const AFormat:String;
...
//                [kbmMW_Rest('value: "$key", required: false')] const AKey:String;
      ...
      [kbmMW_Method]
      [kbmMW_Rest('method:get, path: "create_intf_item"')]
      function create_intf_item(//[kbmMW_Rest('value: "$appid", required: false')] const AAppID:String;
                //[kbmMW_Rest('value: "$user_fid", required: false')] const AUserFID:String;
//                [kbmMW_Rest('value: "$key", required: false')] const AKey:String;
                [kbmMW_Rest('value: "$name", required: false')] const ARestName:String;
                [kbmMW_Rest('value: "$caption", required: false')] const caption:String;
                [kbmMW_Rest('value: "$module", required: false')] const module:String;
                [kbmMW_Rest('value: "$table", required: false')] const table:String;
                [kbmMW_Rest('value: "$select", required: false')] const select:String;
                [kbmMW_Rest('value: "$delete_field", required: false')] const delete_field:String;
                [kbmMW_Rest('value: "$pkfield", required: false')] const pkfield:String;
                [kbmMW_Rest('value: "$orderby", required: false')] const orderby:String;
  ...
  TCommonRestServiceModule=class(TServiceModule)//TKbmMWServiceModule)
  public
    //表服务列表
...
//    First chance exception at $76D3F932. Exception class TJSONSyntaxError with message
//'Unexpected token ILLEGAL. (Line: 0 Col: 488)'.
//Process RPAServer_Console_D12.exe (14552)
      on E:Exception do
      begin
        SaveStringToFile(ARecordDataJson,GetApplicationPath+'log'+PathDelim+'AddRecord'+PathDelim+ARestName+'_'+FormatDateTime('YYYY-MM-DD HH-MM-SS-ZZZ.json',Now()),TEncoding.UTF8);
      end;
    end;
...
function TsrvTableCommonRestService.create_intf_item(const ARestName, caption, module, table, select, delete_field, pkfield, orderby, ARemoteLocation: String): String;
var
  ACode:Integer;
  ADesc:String;
  ADataJson:ISuperObject;
...
begin
  ...
  try
      ...
      begin
          if TServiceModule(GlobalServiceProject.ServiceModuleList[I]).Name=module then
          begin
            AServiceModule := TServiceModule(GlobalServiceProject.ServiceModuleList[I]);
            Break;
          end;
      end;
      if AServiceModule=nil then
      begin
        ADesc:='数据库模块不存在！';
        Exit;
      end;
      ...
      begin
          AIntfItem:=TCommonRestIntfItem.Create(
            ...
            caption,
            //数据库连接
            TKbmMWServiceModule(AServiceModule).DBModule,
            //表名
            table,
            select,
            '',
      ...
      begin
          AIntfItem.Caption:=caption;
          AIntfItem.DBModule:=TKbmMWServiceModule(AServiceModule).DBModule;
          AIntfItem.TableName:=table;
          AIntfItem.Select.Text:=select;
          AIntfItem.DeleteFieldName:=delete_field;
          AIntfItem.PKFieldName:=pkfield;
          AIntfItem.DefaultOrderBy:=orderby;

      end;

      if not AIntfItem.Init(AIntfItem.DBModule,ADesc) then
      begin
        Exit;
      end;
...
function TsrvTableCommonRestService.GetRecordList(AAppID:String;
  AUserFID: String; const AKey:String;
  APageIndex,APageSize:Integer;
  const ARestName:String;
  AWhereKeyJson:String;
  const ACustomConditionSQL, AOrderBy, AGroupBy, AGroupBySummary:String;
  ...
  const AJimuPageNo:String;
  const AJimuPageSize:String;
...
begin
  ...
  if AJimuPageNo<>'' then
  begin
    APageIndex:=StrToInt(AJimuPageNo);
    APageSize:=StrToInt(AJimuPageSize);
  end;
  ...
  try
    //ClientIdentity
    AQueryParams.AsString:=RequestTransportStream.Args[2];
    //RequestTransportStream.Args
//    hlp:=TkbmMWRESTTransportStreamHelper(Self.RequestTransportStream.Helper);
//    hlp.Params
...
//                [kbmMW_Rest('value: "$pageSize", required: false')] const AJimuPageSize:String;
    ...
    begin
      if (Copy(AQueryParams.Names[I],1,7)='filter_') and (AQueryParams.Values[I].Value<>'') then
      begin
        //是过滤参数
        AWhereKeyJsonArray:=SA(AWhereKeyJson);
        AWhereKeyJsonArray.O[AWhereKeyJsonArray.Length]:=GetFieldCondition('AND',Copy(AQueryParams.Names[I],8,MaxInt),'=',AQueryParams.Values[I].Value);
        AWhereKeyJson:=AWhereKeyJsonArray.AsJson;
      end;
  ...
  //根据ATableItem来判断是否需要appid和user_fid比较合适
...
//        end;
    ...
    if AFormat='list' then
    begin
        if ADataJson<>nil then
        begin
          Result:=ADataJson.A['RecordList'].AsJSON;
        end
        else
        begin
          Result:='[]';
...
//                [kbmMW_Rest('value: "$is_need_record_list", required: false')] const is_need_record_list:String;
  ...
  try
    ...
    Result:=Self.GetRecordList(VarToStr(APostJson.V['appid']),//AAppID,
                               ...
                               APostJson.S['group_by'],
                               APostJson.S['group_by_summary'],
...
function TsrvTableCommonRestService.save_record_post(AAppID, AUserFID:String;
  const AKey, ARestName, check_exist_field_names,
  ARemoteLocation: String): String;

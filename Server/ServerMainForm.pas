...
//  kbmMWCrypt,
  ...
  Vcl.StdCtrls, Vcl.ExtCtrls, Vcl.ComCtrls, Vcl.Grids, UniProvider, SQLiteUniProvider, Data.DB, DBAccess, Uni;
...
type
  ...
  private
    ...
    procedure DoServiceLog(Sender:TObject;AError:String);
...
//      and (not TKbmMWServiceModule(GlobalServiceProject.ServiceModuleList[I]).DBModule.DBConfig.IsEmpty)
      then
    begin
...
procedure TfrmServerMain.DoServiceLog(Sender: TObject; AError: String);
begin
  TThread.Synchronize(nil,procedure
  begin
    Self.memLog.Lines.Add(AError);
  end);
end;
...
procedure TfrmServerMain.FormCloseQuery(Sender: TObject; var CanClose: Boolean);
begin
//  CanClose:=False;
//  Hide;
end;
...
begin
  ...
  GlobalServiceProject.FOnLog:=Self.DoServiceLog;
...
procedure TfrmServerMain.Timer1Timer(Sender: TObject);
begin

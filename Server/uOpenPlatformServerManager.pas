...
//  end;
  ...
  public
    ...
    function Add(AAppID:String;AUserFID:String;AUserName:String;AAccessToken:String;AAppType:String='';AIsSystem:Boolean=False):TUserAccessToken;overload;
...
//
//
//    AIntfItem:=CommonRestServiceModule.IntfList.Find('database_config');
//    if AIntfItem=nil then
//    begin
//      ADesc:='database_config接口不存在';
//      Exit;
//    end;
//
//
...
//      {$ELSE}
          ...
          begin
            ...
            sd.RootPath[mwhfcJavascript]:=GetApplicationPath+'webfiles/files';
            sd.RootPath[mwhfcStyleSheet]:=GetApplicationPath+'webfiles/files';
            ...
            {$ENDIF}
              ...
              begin
                try
                  //启动服务
                  //提供数据服务
                  uBaseLog.HandleException(nil,'TServiceProject.DoStart kbmMWServer1.Active = True');
...
//    begin
//      DBModule.UpdateDataBase(Name,FSQLUpgradeList,ADesc);
//    end;
//  end;
...
//    ALicenseInfo:=ISuperObject;
    ...
    begin
      ...
      FServiceProject.LicenseCheckErrorMsg:=AMessage;
      FServiceProject.DoLog('检测授权失败：'+AMessage);
//    注释掉下面这两句,表示吧,授权检测失败,也能启动服务,但是登录之后功能有限制而已
//      SleepThread(15*1000);
//      continue;
    end
    else
...
function TUserAccessTokenList.Add(AAppID, AUserFID,AUserName,AAccessToken: String;AAppType:String;AIsSystem:Boolean): TUserAccessToken;
begin
  Result:=GlobalUserAccessTokenClass.Create;
  Result.AppID:=AAppID;
...
begin
  Result:='SELECT appid,fid,name,login_key FROM tbluser WHERE IFNULL(login_key,'''')<>'''' ';
end;
...
//        AUserAccessToken.AccessToken:=ASQLDBHelper.Query.FieldByName('login_key').AsString;
  ...
  Self.UserName:=AUserDataset.FieldByName('name').AsString;
  Self.AppType:='';
...
begin
  ...
  Self.UserName:=AUserJson.S['name'];
  Self.AppType:='';
